...
const PIE_COLORS = ['#2E7D32', '#4CAF50', '#81C784', '#A5D6A7', '#C8E6C9', '#E8F5E9'];
    ...
    <div className="p-6">
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="font-bold text-gray-900 mb-4">مقارنة ربحية المحاصيل</h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.byCrop} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis type="number" tick={{ fontSize: 12 }} />
                <YAxis dataKey="cropAr" type="category" tick={{ fontSize: 11 }} width={80} />
                <Tooltip
                  contentStyle={{
                    backgroundColor: '#fff',
                    border: '1px solid #e0e0e0',
                ...
                />
                <Legend />
                <Bar dataKey="revenue" fill={CHART_COLORS.primary} name="الإيرادات" radius={[0, 4, 4, 0]} />
                <Bar dataKey="costs" fill={CHART_COLORS.danger} name="التكاليف" radius={[0, 4, 4, 0]} />
                <Bar dataKey="profit" fill={CHART_COLORS.info} name="الربح" radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="mt-6 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-4 border-b border-gray-100">
          <h3 className="font-bold text-gray-900">تفاصيل الربحية حسب المحصول</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">المحصول</th>
                ...
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">التكاليف</th>
                ...
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">الربح/هكتار</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {data.byCrop.map((crop) => (
                <tr key={crop.crop} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <Wheat className="w-5 h-5 text-sahool-600" />
                      <span className="font-medium text-gray-900">{crop.cropAr}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-900">{crop.area.toFixed(1)} هكتار</td>
                  <td className="px-6 py-4 text-sm font-medium text-green-600">${crop.revenue.toFixed(2)}</td>
                  <td className="px-6 py-4 text-sm font-medium text-red-600">${crop.costs.toFixed(2)}</td>
                  <td className="px-6 py-4 text-sm font-bold text-blue-600">${crop.profit.toFixed(2)}</td>
                  <td className="px-6 py-4">
                    <span className={`text-sm font-medium ${crop.margin >= 20 ? 'text-green-600' : crop.margin >= 10 ? 'text-yellow-600' : 'text-red-600'}`}>
                      {crop.margin.toFixed(1)}%
                    </span>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-900">${(crop.profit / crop.area).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="mt-6 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {data.seasons.map((season) => (
            <div key={season.season} className="border border-gray-100 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-green-600">${(season.revenue / 1000).toFixed(1)}K</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">التكاليف:</span>
                  <span className="font-medium text-red-600">${(season.costs / 1000).toFixed(1)}K</span>
                </div>
                <div className="flex justify-between border-t border-gray-100 pt-2">
                  <span className="text-gray-900 font-medium">الربح:</span>
                  <span className="font-bold text-blue-600">${(season.profit / 1000).toFixed(1)}K</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">المحاصيل:</span>
                  <span className="font-medium text-gray-900">{season.crops}</span>
                </div>
              </div>
            </div>
          ))}
