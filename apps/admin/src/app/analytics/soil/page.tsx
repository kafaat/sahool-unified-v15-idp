...
const MOCK_TESTS: SoilTest[] = [
  {
    ...
    status: 'deficient',
    recommendations_ar: [
      'إضافة سماد يوريا 46% بمعدل 46 كجم/هكتار',
      'النيتروجين أقل من الحد الأمثل (18 < 25 ppm)',
    ],
  },
  {
    id: 's2',
    ...
    iron_ppm: 5,
    zinc_ppm: 1.8,
    texture_ar: 'رملية طينية',
    moisture_percent: 28,
    salinity_level: 'moderate',
    status: 'deficient',
    recommendations_ar: ['إضافة سوبر فوسفات (P أقل من الحد)', 'معالجة ملوحة التربة (EC = 3.2)'],
  },
  {
    id: 's3',
    field_id: 'f3',
    ...
    salinity_level: 'low',
    status: 'optimal',
    recommendations_ar: ['التربة في حالة جيدة — استمر في البرنامج الحالي'],
  },
  {
    id: 's4',
    field_id: 'f4',
    ...
    iron_ppm: 15,
    zinc_ppm: 4.2,
    texture_ar: 'طينية ثقيلة',
    moisture_percent: 52,
    salinity_level: 'low',
    status: 'optimal',
    recommendations_ar: ['التغذية ممتازة — خفض النيتروجين قليلاً لتجنب الإفراط'],
  },
  {
    id: 's5',
    field_id: 'f5',
    ...
    status: 'critical',
    recommendations_ar: [
      'معالجة ملوحة عاجلة (EC = 4.5)',
      'إضافة NPK كامل — جميع العناصر ناقصة',
      'إضافة مادة عضوية لتحسين بنية التربة',
      'غسيل ملحي بـ 200 ملم مياه نظيفة',
    ],
  },
  {
    id: 's6',
    ...
    salinity_level: 'low',
    status: 'optimal',
    recommendations_ar: ['تربة بركانية ممتازة للبن — حافظ على المادة العضوية'],
  },
];
...
function getNutrientStatus(
  ...
  if (value < threshold.low)
    return { label: 'ناقص', color: 'text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20' };
...
export default function SoilMonitoringPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const response = await apiClient.get(
        `${API_URLS.soilAnalysis}${API_PATHS.soilAnalysis.tests}`
      );
  ...
  const stats = useMemo(() => {
    ...
    const highSalinity = tests.filter(
      (t) => t.salinity_level === 'high' || t.salinity_level === 'critical'
    ).length;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4">
        {[
          ...
          {
            label: 'متوسط pH',
            value: stats.avgPh.toFixed(1),
            icon: Gauge,
            color: 'text-purple-600 dark:text-purple-400',
          },
          {
            label: 'متوسط EC',
            value: `${stats.avgEC.toFixed(1)} dS/m`,
            icon: Activity,
            color: 'text-cyan-600 dark:text-cyan-400',
          },
          {
            label: 'ملوحة عالية',
            value: stats.highSalinity,
            icon: Droplets,
            ...
            return (
              <div
                ...
                className={`bg-white dark:bg-gray-900 rounded-xl border p-4 cursor-pointer transition-all ${
                  isSelected
                    ? 'border-sahool-500 ring-1 ring-sahool-200 dark:ring-sahool-800'
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${getSalinityColor(test.salinity_level)}`}
                      >
                    ...
                    <div className="flex gap-4 mt-3">
                      {[
                        {
                          label: 'N',
                          value: test.nitrogen_ppm,
                          threshold: NUTRIENT_THRESHOLDS[0]!,
                        },
                        {
                          label: 'P',
                          value: test.phosphorus_ppm,
                          threshold: NUTRIENT_THRESHOLDS[1]!,
                        },
                        {
                          label: 'K',
                          value: test.potassium_ppm,
                          threshold: NUTRIENT_THRESHOLDS[2]!,
                        },
                      ].map((n) => {
                            ...
                            <div className="w-16 h-2 bg-gray-200 dark:bg-gray-700 rounded-full">
                              <div
                                className={`h-full rounded-full ${
                                  ns.label === 'مثالي'
                                    ? 'bg-green-500'
                                    : ns.label === 'منخفض' || ns.label === 'ناقص'
                                      ? 'bg-orange-500'
                                      : 'bg-red-500'
                                }`}
                                style={{
                                  width: `${Math.min((n.value / n.threshold.high) * 100, 100)}%`,
                                }}
                              />
                            </div>
            ...
            <div className="space-y-4">
              <div className="p-3 bg-sahool-50 dark:bg-sahool-900/20 rounded-lg">
                <p className="text-sm font-bold text-sahool-700 dark:text-sahool-300">
                  {selectedData.field_name_ar}
                </p>
                <p className="text-xs text-sahool-600 dark:text-sahool-400">
                  {selectedData.farm_name_ar} — {selectedData.governorate_ar}
                </p>
              </div>
                    ...
                    <div key={threshold.name}>
                      <div className="flex items-center justify-between text-sm mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${ns.color}`}>
                            {ns.label}
                          </span>
                        </div>
                      </div>
                ...
                <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-2">
                    {selectedData.recommendations_ar.map((rec, i) => (
                      <div
                        key={i}
                        className="flex gap-2 p-2 bg-amber-50 dark:bg-amber-900/10 rounded text-sm text-amber-800 dark:text-amber-300"
                      >
