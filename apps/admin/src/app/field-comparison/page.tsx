...
export default function FieldComparisonPage() {
  ...
  const fieldA = useMemo(() => MOCK_FIELDS.find((f) => f.id === fieldAId) ?? MOCK_FIELDS[0], [fieldAId]) as (typeof MOCK_FIELDS)[number];
  const fieldB = useMemo(() => MOCK_FIELDS.find((f) => f.id === fieldBId) ?? MOCK_FIELDS[1], [fieldBId]) as (typeof MOCK_FIELDS)[number];
