...
/**
 * Field Preparation Workflow Page
 * تجهيز الحقل — سير عمل التحضير قبل الزراعة
 *
 * Pre-season field preparation workflow with 7 steps:
 * 1. Soil Analysis (تحليل التربة)
 * 2. Primary Tillage (حراثة أولية)
 * 3. Secondary Tillage (حراثة ثانية - تنعيم)
 * 4. Land Leveling (تسوية الأرض)
 * 5. Base Fertilization (تسميد أساسي)
 * 6. Irrigation Network Setup (تجهيز شبكة الري)
 * 7. Establishment Irrigation (ري تأسيسي)
 */
...
const STATUS_CONFIG: Record<
  StepStatus,
  { labelAr: string; icon: typeof CheckCircle2; color: string; bgColor: string; borderColor: string }
> = {
  completed: {
    labelAr: 'مكتمل',
    icon: CheckCircle2,
...
const SEASONS: SeasonOption[] = [
  { id: 'winter-2026', labelAr: 'شتاء 2026', year: 2026 },
  { id: 'summer-2026', labelAr: 'صيف 2026', year: 2026 },
  { id: 'winter-2027', labelAr: 'شتاء 2027', year: 2027 },
];
...
const INITIAL_STEPS: PrepStep[] = [
  {
    ...
    details: {
      ...
      'النيتروجين (N)': '22 ppm',
      'الفوسفور (P)': '18 ppm',
      'البوتاسيوم (K)': '180 ppm',
      'الملوحة (EC)': '1.8 dS/m',
      'المادة العضوية': '1.4%',
    },
    recommendationAr: 'إضافة يوريا 46 كجم/هكتار لتعويض نقص النيتروجين',
    costSar: 350,
    linkLabel: 'عرض تقرير التحليل الكامل',
    linkHref: '/analytics/soil',
  },
  {
    id: 2,
    titleAr: 'حراثة أولية',
    titleEn: 'Primary Tillage',
    status: 'completed',
    date: '2026-09-20',
    details: {
      'العمق': '30 سم',
      'المعدة': 'جرار + محراث قرصي',
      'عدد التمريرات': '2',
      'اتجاه الحراثة': 'متقاطع',
    },
    recommendationAr: 'تفكيك التربة المتماسكة وقلب المخلفات النباتية',
    costSar: 400,
  },
  {
    id: 3,
    titleAr: 'حراثة ثانية (تنعيم)',
    titleEn: 'Secondary Tillage (Fine)',
    status: 'completed',
    date: '2026-09-25',
    details: {
      'العمق': '15 سم',
      'المعدة': 'روتوفيتور',
      'حجم الكتل': 'أقل من 5 سم',
      'حالة التربة': 'رطوبة مناسبة',
    },
    recommendationAr: 'تنعيم التربة لتهيئة مرقد بذرة مثالي',
    costSar: 300,
  },
  {
    id: 4,
    titleAr: 'تسوية الأرض',
    titleEn: 'Land Leveling',
    status: 'in_progress',
    details: {
      'الانحدار المطلوب': '0.1 - 0.2%',
      'الطريقة': 'تسوية ليزرية',
      'فرق الارتفاع الحالي': '18 سم',
      'حجم القطع': '245 م³',
      'حجم الردم': '238 م³',
    },
    costSar: 600,
    linkLabel: 'عرض خريطة التسوية',
    linkHref: '/terrain',
  },
  {
    id: 5,
    titleAr: 'تسميد أساسي',
    titleEn: 'Base Fertilization',
    status: 'pending',
    details: {
      'DAP (18-46-0)': '50 كجم/هكتار',
      'سلفات بوتاسيوم': '30 كجم/هكتار',
      'سوبر فوسفات': '25 كجم/هكتار',
      'طريقة الإضافة': 'نثر قبل الحراثة النهائية',
    },
    recommendationAr: 'إضافة الأسمدة الأساسية لتوفير العناصر الغذائية خلال المراحل الأولى من النمو',
    costSar: 450,
  },
  {
    id: 6,
    titleAr: 'تجهيز شبكة الري',
    titleEn: 'Irrigation Network Setup',
    status: 'pending',
    details: {
      'النوع': 'تنقيط',
      'عدد الخطوط': '12',
      'المسافة بين الخطوط': '30 سم',
      'المسافة بين النقاطات': '20 سم',
      'تصريف النقاط': '4 لتر/ساعة',
      'ضغط التشغيل': '1.5 بار',
    },
    costSar: 250,
  },
  {
    id: 7,
    titleAr: 'ري تأسيسي',
    titleEn: 'Establishment Irrigation',
    status: 'pending',
    details: {
      'الكمية': '40 مم',
      'المدة المتوقعة': '6 ساعات',
      'الهدف': 'ترطيب العمق الجذري (0-40 سم)',
      'رطوبة التربة المستهدفة': '75% من السعة الحقلية',
    },
    recommendationAr: 'ري عميق لتأسيس رطوبة كافية قبل الزراعة مباشرة',
    costSar: 150,
  },
];
...
export default function FieldPrepPage() {
  ...
  const totalCostPerHa = useMemo(
    () => steps.reduce((sum, s) => sum + (s.costSar ?? 0), 0),
    [steps]
  );
    ...
    setSteps((prev) =>
      prev.map((step) => {
        ...
        return {
          ...step,
          status: nextStatus,
          date: nextStatus === 'completed' ? new Date().toISOString().split('T')[0] : step.date,
        };
      })
    );
  }, []);
  ...
  const handleSave = useCallback(() => {
    // Placeholder: save progress via API
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto" dir="rtl">
      <Header title="تجهيز الحقل — Field Preparation" subtitle="الخطوات الأساسية قبل الزراعة" />

      {/* Field & Season Selectors */}
      ...
      <div className="mt-6 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
        ...
        <div className="flex items-center justify-between mt-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span>
            {field?.nameAr} — {field?.areaHa} هكتار
          </span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 text-center">
            ...
            <p className="text-lg font-bold text-gray-900 dark:text-gray-100">
              {totalCostPerHa.toLocaleString()} ريال
            </p>
          </div>
          <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-3 text-center">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">
              إجمالي الحقل ({field?.areaHa} هـ)
            </p>
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-center">
            ...
            <p className="text-lg font-bold text-blue-700 dark:text-blue-300">
              {steps
                ...
                .reduce((sum, s) => sum + (s.costSar ?? 0) * (field?.areaHa ?? 1), 0)
                .toLocaleString()}{' '}
              ريال
            </p>
          </div>
