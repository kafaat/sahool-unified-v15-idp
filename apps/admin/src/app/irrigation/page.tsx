...
function generateMockPlan(): IrrigationPlan {
  ...
  const schedules: IrrigationSchedule[] = Array.from({ length: 3 }, (_, i) => {
    ...
    return {
      ...
      urgency,
      urgency_ar: urgency === 'critical' ? 'حرج' : urgency === 'high' ? 'عالي' : urgency === 'medium' ? 'متوسط' : 'منخفض',
...
function generateMockWaterBalance(): { summary: Record<string, number>; daily_data: WaterBalance[] } {
  ...
  return {
    summary: {
      ...
      cumulative_deficit_mm: dailyData[dailyData.length - 1].cumulative_deficit_mm,
    },
...
function generateMockCrops(): CropInfo[] {
  return [
    { id: 'tomato', name_ar: 'طماطم', water_requirements_mm_day: { seedling: 2.5, vegetative: 4.5, flowering: 6.0 } },
    { id: 'wheat', name_ar: 'قمح', water_requirements_mm_day: { seedling: 2.0, vegetative: 4.0, flowering: 5.5 } },
    { id: 'coffee', name_ar: 'بن', water_requirements_mm_day: { seedling: 3.0, vegetative: 4.0, flowering: 5.0 } },
    { id: 'banana', name_ar: 'موز', water_requirements_mm_day: { seedling: 4.0, vegetative: 6.0, flowering: 7.0 } },
    { id: 'date_palm', name_ar: 'نخيل', water_requirements_mm_day: { seedling: 5.0, vegetative: 8.0, flowering: 10.0 } },
  ];
}

export default function IrrigationPage() {
  const [plan, setPlan] = useState<IrrigationPlan | null>(null);
  const [waterBalance, setWaterBalance] = useState<{ summary: Record<string, number>; daily_data: WaterBalance[] } | null>(null);
  ...
  async function loadData() {
    ...
    try {
      ...
      const [planRes, balanceRes, methodsRes, cropsRes] = await Promise.all([
        apiClient.post(`${API_URLS.irrigation}/v1/calculate`, {
          field_id: 'field-1',
          crop: 'tomato',
        ...
        apiClient.get(`${API_URLS.irrigation}/v1/water-balance/field-1`).catch(() => null),
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      schedule.urgency !== 'critical' && schedule.urgency !== 'high' && 'border-gray-100'
                    )}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className={cn(
                          'p-3 rounded-lg',
                          schedule.urgency === 'critical' ? 'bg-red-100' :
                          schedule.urgency === 'high' ? 'bg-orange-100' :
                          'bg-sahool-100'
                        )}>
                          <Droplets className={cn(
                            'w-6 h-6',
                            schedule.urgency === 'critical' ? 'text-red-600' :
                            schedule.urgency === 'high' ? 'text-orange-600' :
                            'text-sahool-600'
                          )} />
                        </div>
                        <div>
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-4 text-center">
                      <div className="bg-gray-50 rounded-lg p-3">
                        <p className="text-lg font-bold text-gray-900">{schedule.duration_minutes} دقيقة</p>
                        <p className="text-xs text-gray-500">المدة</p>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-3">
                        <p className="text-lg font-bold text-blue-600">{schedule.water_amount_m3.toFixed(1)} م³</p>
                        <p className="text-xs text-gray-500">كمية المياه</p>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-3">
                        <p className="text-lg font-bold text-gray-900">{schedule.method_ar}</p>
                        <p className="text-xs text-gray-500">الطريقة</p>
                      </div>
                    </div>

                    <p className="mt-3 text-sm text-gray-600">{schedule.reasoning_ar}</p>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-4">
                ...
                <ul className="space-y-2">
                  {plan?.recommendations_ar.map((rec, i) => (
                    <li key={i} className="text-sm text-gray-600 p-2 bg-gray-50 rounded">{rec}</li>
                  ))}
                </ul>
              </div>
              ...
              {/* Crop Info */}
              <div className="bg-white rounded-xl border border-gray-100 p-4">
                <h4 className="font-bold text-gray-900 mb-3">معلومات المحصول</h4>
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{plan?.current_water_need_mm.toFixed(1)} ملم</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-6">
              ...
              <div className="space-y-4">
                ...
                <div className={cn(
                  'p-3 rounded-lg',
                  (waterBalance?.summary.cumulative_deficit_mm || 0) > 20 ? 'bg-red-50' : 'bg-gray-50'
                )}>
                  ...
                  <p className={cn(
                    'text-2xl font-bold',
                    (waterBalance?.summary.cumulative_deficit_mm || 0) > 20 ? 'text-red-700' : 'text-gray-700'
                  )}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-6">
                  ...
                  <div key={method.id} className="flex items-center gap-4">
                    <div className="flex-1">
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="text-sm text-gray-500">{method.efficiency_percent}%</span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div
                          className={cn(
                            'h-2 rounded-full',
                            method.efficiency_percent >= 80 ? 'bg-green-500' :
                            method.efficiency_percent >= 60 ? 'bg-amber-500' :
                            'bg-red-500'
                          )}
              ...
              <div className="mt-6 p-4 bg-sahool-50 rounded-lg">
                <p className="text-sm text-sahool-700">
                  💡 <strong>نصيحة:</strong> التحويل للري بالتنقيط يمكن أن يوفر حتى 45% من استهلاك المياه مقارنة بالري التقليدي
                </p>
              </div>
            </div>
            ...
            {/* Supported Crops */}
            <div className="bg-white rounded-xl border border-gray-100 p-6">
              <h3 className="font-bold text-gray-900 mb-4">المحاصيل المدعومة</h3>
                ...
                {crops.map(crop => (
                  <div key={crop.id} className="p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-xs text-gray-500 space-y-1">
                      {Object.entries(crop.water_requirements_mm_day).slice(0, 2).map(([stage, value]) => (
                        <div key={stage} className="flex justify-between">
                          <span>{stage === 'seedling' ? 'شتلة' : stage === 'vegetative' ? 'نمو' : 'إزهار'}:</span>
