...
const MOCK_PRICES: MarketPrice[] = [
  {
    ...
    category: "حبوب",
    weekHistory: [2.60, 2.65, 2.71, 2.70, 2.74, 2.78, 2.80],
    marketComparison: [
      { market: "الرياض", price: 2.80 },
  ...
  },
  {
    ...
    category: "حبوب",
    weekHistory: [2.20, 2.18, 2.15, 2.14, 2.13, 2.11, 2.10],
    marketComparison: [
      { market: "جدة", price: 2.10 },
  ...
  },
  {
    ...
    category: "فواكه",
    weekHistory: [30.00, 31.00, 31.50, 32.41, 33.00, 34.00, 35.00],
    marketComparison: [
      { market: "القصيم", price: 35.00 },
      { market: "الرياض", price: 33.50 },
      { market: "المدينة", price: 34.00 },
    ],
  ...
  },
  {
    ...
    category: "خضروات",
    weekHistory: [5.00, 4.90, 4.80, 4.75, 4.70, 4.60, 4.50],
    marketComparison: [
      { market: "الرياض", price: 4.50 },
      { market: "جدة", price: 4.60 },
  ...
  },
  {
    ...
    category: "خضروات",
    weekHistory: [3.10, 3.12, 3.14, 3.15, 3.16, 3.18, 3.20],
    marketComparison: [
      { market: "الدمام", price: 3.20 },
  ...
  },
  {
    ...
    category: "خضروات",
    weekHistory: [3.10, 3.05, 3.00, 2.96, 2.95, 2.92, 2.90],
    marketComparison: [
      { market: "جدة", price: 2.90 },
  ...
  },
  {
    ...
    weekHistory: [100.00, 103.00, 106.67, 108.00, 112.00, 117.00, 120.00],
    marketComparison: [
      { market: "صنعاء", price: 120.00 },
      { market: "عدن", price: 115.00 },
      { market: "إب", price: 118.00 },
    ],
  ...
  },
  {
    ...
    category: "حبوب",
    weekHistory: [1.78, 1.78, 1.79, 1.79, 1.79, 1.80, 1.80],
    marketComparison: [
      { market: "الحديدة", price: 1.80 },
  ...
  },
  {
    ...
    category: "فواكه",
    weekHistory: [12.50, 13.00, 13.50, 14.11, 14.50, 14.80, 15.00],
    marketComparison: [
      { market: "عدن", price: 15.00 },
      { market: "تعز", price: 14.50 },
      { market: "إب", price: 14.80 },
    ],
  ...
  },
  {
    ...
    lastUpdated: "منذ ساعتين",
    category: "فواكه",
    weekHistory: [9.20, 9.00, 8.83, 8.70, 8.60, 8.55, 8.50],
    marketComparison: [
      { market: "تعز", price: 8.50 },
      { market: "صنعاء", price: 8.70 },
...
function QualityBadge({ quality }: { quality: Quality }) {
  const config: Record<Quality, { label: string; className: string }> = {
    PREMIUM: { label: "ممتاز", className: "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300" },
    GRADE_A: { label: "درجة أ", className: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300" },
    GRADE_B: { label: "درجة ب", className: "bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300" },
  };
...
function TrendIcon({ trend, size = 4 }: { trend: Trend; size?: number }) {
  ...
  if (trend === "down") return <TrendingDown className={cn(`w-${size} h-${size}`, "text-red-500")} />;
...
function PriceCard({ crop, onClick, isSelected }: { crop: MarketPrice; onClick: () => void; isSelected: boolean }) {
  return (
    <button
      onClick={onClick}
      className={cn(
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{crop.cropName}</p>
      ...
      >
        <div>
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
            {crop.currentPrice.toFixed(2)}{" "}
            <span className="text-sm font-normal text-gray-500 dark:text-gray-400">{crop.unit}</span>
          </p>
        </div>
...
export default function MarketPricesPage() {
  ...
  const stats = useMemo(() => {
    ...
    const avgChange =
      MOCK_PRICES.reduce((sum, p) => sum + p.changePercent, 0) / MOCK_PRICES.length;
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
            ...
            {marketFilter !== "الكل" && (
              <span className="flex items-center gap-1 px-2 py-1 bg-sahool-50 dark:bg-sahool-900/30 text-sahool-700 dark:text-sahool-300 text-xs rounded-full">
                {marketFilter}
                <button onClick={() => setMarketFilter("الكل")}><X className="w-3 h-3" /></button>
              </span>
            )}
            {categoryFilter !== "الكل" && (
              <span className="flex items-center gap-1 px-2 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs rounded-full">
                {categoryFilter}
                <button onClick={() => setCategoryFilter("الكل")}><X className="w-3 h-3" /></button>
              </span>
            )}
            {trendFilter !== "الكل" && (
              <span className="flex items-center gap-1 px-2 py-1 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-xs rounded-full">
                {trendFilter}
                <button onClick={() => setTrendFilter("الكل")}><X className="w-3 h-3" /></button>
              </span>
            )}
            <span className="text-xs text-gray-400 dark:text-gray-500">
        ...
        <div className="flex-1 min-w-0">
          {filteredPrices.length === 0 ? (
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-12 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400">لا توجد نتائج تطابق التصفية المحددة</p>
              <button
                onClick={() => {
          ...
          {filteredPrices.length > 0 && (
            <div className="mt-4 flex items-center justify-between text-xs text-gray-400 dark:text-gray-500">
              <span>عرض {filteredPrices.length} من {MOCK_PRICES.length} محصول</span>
