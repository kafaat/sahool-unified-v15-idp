  ...
  yields: [
    {
      ...
      avg_yield_ton_per_ha: 3.2,
      total_yield_tons: 384,
      target_yield_ton_per_ha: 3.5,
      achievement_percent: 91,
      comparison_last_season: 8,
    },
    {
      crop_type: 'barley',
      crop_name_ar: 'شعير',
      total_area_hectares: 85,
      avg_yield_ton_per_ha: 2.8,
      total_yield_tons: 238,
      target_yield_ton_per_ha: 3.0,
      achievement_percent: 93,
      comparison_last_season: 12,
    },
    {
      crop_type: 'tomato',
      crop_name_ar: 'طماطم',
      total_area_hectares: 35,
      avg_yield_ton_per_ha: 25.0,
      total_yield_tons: 875,
      target_yield_ton_per_ha: 28.0,
      achievement_percent: 89,
      comparison_last_season: 5,
    },
    {
      crop_type: 'potato',
      crop_name_ar: 'بطاطس',
      total_area_hectares: 45,
      avg_yield_ton_per_ha: 18.5,
      total_yield_tons: 832.5,
      target_yield_ton_per_ha: 20.0,
      achievement_percent: 92,
      comparison_last_season: -3,
    },
    {
      crop_type: 'coffee',
      crop_name_ar: 'بن يمني',
      total_area_hectares: 39.5,
      avg_yield_ton_per_ha: 0.8,
      total_yield_tons: 31.6,
      target_yield_ton_per_ha: 0.9,
      achievement_percent: 88,
      comparison_last_season: 15,
    },
  ],
  irrigation: {
...
function getSeverityBadge(severity: string) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-medium ${colors[severity] || colors.low}`}
    >
...
export default function SeasonalReportPage() {
  ...
  const loadReport = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get(`${API_URLS.indicators}/api/v1/reports/seasonal`, {
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { icon: Target, label: 'الحقول', value: season.total_fields },
            {
              ...
              value: `${formatNumber(season.total_area_hectares)} هكتار`,
            },
            {
              icon: Calendar,
              label: 'الفترة',
              value: `${season.start_date.slice(5)} — ${season.end_date.slice(5)}`,
            },
          ].map((s) => (
            ...
            <tbody>
              {yields.map((y) => (
                ...
                >
                  ...
                  <td className="py-3 px-2 font-bold text-gray-900 dark:text-gray-100">
                    {formatNumber(y.total_yield_tons)}
                  </td>
                  <td className="py-3 px-2">
                    <span
                      className={`flex items-center gap-1 text-sm font-medium ${
                        y.comparison_last_season >= 0
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              {
                label: 'إجمالي المياه',
                value: `${formatNumber(irrigation.total_water_m3)} م³`,
                color: 'text-blue-600 dark:text-blue-400',
              },
              {
                label: 'كفاءة الري',
                value: `${irrigation.avg_efficiency_percent}%`,
                color: 'text-green-600 dark:text-green-400',
              },
              {
                label: 'عمليات الري',
                value: formatNumber(irrigation.irrigation_events),
                color: 'text-gray-900 dark:text-gray-100',
              },
              {
                label: 'المياه الموفرة',
                value: `${formatNumber(irrigation.water_saved_m3)} م³ (${irrigation.water_saved_percent}%)`,
          ...
          <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/10 rounded-lg">
            ...
            <p className="text-lg font-bold text-blue-700 dark:text-blue-400">
              {formatMoney(irrigation.cost_yer)}
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div className="p-3 bg-green-50 dark:bg-green-900/10 rounded-lg text-center">
              ...
              <p className="text-base font-bold text-green-700 dark:text-green-400">
                {formatMoney(financial.total_revenue_yer)}
              </p>
            </div>
            <div className="p-3 bg-red-50 dark:bg-red-900/10 rounded-lg text-center">
              ...
              <p className="text-base font-bold text-red-700 dark:text-red-400">
                {formatMoney(financial.total_cost_yer)}
              </p>
            </div>
            <div className="p-3 bg-blue-50 dark:bg-blue-900/10 rounded-lg text-center">
              ...
              <p className="text-base font-bold text-blue-700 dark:text-blue-400">
                {formatMoney(financial.net_profit_yer)}
              </p>
            </div>
          </div>
          <div className="space-y-2">
            ...
            {financial.cost_breakdown.map((c, i) => (
              <div key={i} className="flex items-center gap-3">
                <span className="text-xs text-gray-600 dark:text-gray-400 w-40 flex-shrink-0">
                  {c.category_ar}
                </span>
                ...
                <span className="text-xs font-medium text-gray-700 dark:text-gray-300 w-10 text-left">
                  {c.percent}%
                </span>
              </div>
            ))}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                {
                  label: 'متوسط الحرارة',
                  value: `${weather.avg_temperature}°م`,
                  icon: Thermometer,
                },
                {
                  label: 'إجمالي الأمطار',
                  value: `${weather.total_rainfall_mm} ملم`,
                  icon: Droplets,
                },
                {
                  label: 'GDD المتراكمة',
                  value: formatNumber(weather.gdd_total),
                  icon: TrendingUp,
                },
              ].map((item) => (
                <div
            ...
            <div className="flex items-center gap-4 mb-3">
              <div className="flex-1">
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="font-bold text-sahool-600">
                ...
                <div className="h-3 bg-gray-200 dark:bg-gray-700 rounded-full">
                  <div
                    className="h-full bg-sahool-500 rounded-full"
                    style={{ width: `${tasks.completion_rate_percent}%` }}
                  />
                </div>
              </div>
            </div>
