...
const MOCK_LOCATIONS: WeatherLocation[] = [
  { id: "loc-1", name: "Sana'a", name_ar: "صنعاء", latitude: 15.3694, longitude: 44.191, governorate: "صنعاء" },
  { id: "loc-2", name: "Aden", name_ar: "عدن", latitude: 12.7855, longitude: 45.0187, governorate: "عدن" },
  { id: "loc-3", name: "Taiz", name_ar: "تعز", latitude: 13.5789, longitude: 44.0219, governorate: "تعز" },
  { id: "loc-4", name: "Hodeidah", name_ar: "الحديدة", latitude: 14.7979, longitude: 42.9544, governorate: "الحديدة" },
  { id: "loc-5", name: "Ibb", name_ar: "إب", latitude: 13.9667, longitude: 44.1667, governorate: "إب" },
];
...
const MOCK_FORECAST: ForecastDay[] = [
  { date: "2026-03-18", day_name_ar: "الأربعاء", temp_max: 24, temp_min: 12, humidity: 40, wind_speed: 10, rain_probability: 10, rain_amount_mm: 0, description_ar: "مشمس", icon: "sunny", uv_index: 8 },
  { date: "2026-03-19", day_name_ar: "الخميس", temp_max: 23, temp_min: 11, humidity: 45, wind_speed: 15, rain_probability: 20, rain_amount_mm: 0, description_ar: "غائم جزئياً", icon: "partly-cloudy", uv_index: 6 },
  { date: "2026-03-20", day_name_ar: "الجمعة", temp_max: 20, temp_min: 10, humidity: 60, wind_speed: 20, rain_probability: 65, rain_amount_mm: 8, description_ar: "أمطار خفيفة", icon: "rain", uv_index: 3 },
  { date: "2026-03-21", day_name_ar: "السبت", temp_max: 18, temp_min: 9, humidity: 70, wind_speed: 18, rain_probability: 80, rain_amount_mm: 15, description_ar: "أمطار متوسطة", icon: "rain", uv_index: 2 },
  { date: "2026-03-22", day_name_ar: "الأحد", temp_max: 21, temp_min: 10, humidity: 55, wind_speed: 12, rain_probability: 30, rain_amount_mm: 2, description_ar: "غائم", icon: "cloudy", uv_index: 5 },
  { date: "2026-03-23", day_name_ar: "الاثنين", temp_max: 25, temp_min: 13, humidity: 35, wind_speed: 8, rain_probability: 5, rain_amount_mm: 0, description_ar: "مشمس", icon: "sunny", uv_index: 9 },
  { date: "2026-03-24", day_name_ar: "الثلاثاء", temp_max: 26, temp_min: 14, humidity: 30, wind_speed: 10, rain_probability: 0, rain_amount_mm: 0, description_ar: "صافي", icon: "sunny", uv_index: 9 },
];
...
function getWeatherIcon(icon: string, size = 24) {
  ...
  switch (icon) {
    ...
    case "cloudy": return <Cloudy className={`${iconClass} text-gray-400`} />;
  ...
  const loadWeatherData = async (location: WeatherLocation) => {
    ...
    try {
      ...
      const [currentRes, forecastRes, agriRes, alertsRes] = await Promise.allSettled([
        ...
        apiClient.get(`${API_URLS.weather}${API_PATHS.weather.agricultural}`, { params: { location_id: location.id } }),
    ...
    <div className="p-6 space-y-6">
      ...
      {alerts.length > 0 && (
        <div className="space-y-2">
          {alerts.map((alert) => (
            <div key={alert.id} className={`flex items-start gap-3 p-4 rounded-lg border ${getSeverityColor(alert.severity)}`}>
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-700 dark:from-blue-700 dark:to-blue-900 rounded-2xl p-6 text-white shadow-lg">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm opacity-90">{selectedLocation.name_ar} — {selectedLocation.governorate}</span>
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4 mt-6 pt-4 border-t border-white/20">
            {[
              ...
              { icon: Wind, label: "الرياح", value: `${current.wind_speed} كم/س ${current.wind_direction_ar}` },
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-sm text-gray-500 dark:text-gray-400">
            إجمالي الأمطار المتوقعة: <span className="font-bold text-blue-600 dark:text-blue-400">{totalRainForecast} ملم</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-7 gap-3">
          {forecast.map((day) => (
            ...
            >
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">{day.day_name_ar}</p>
      ...
      {agriReport && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
            ...
            <div className={`p-4 rounded-lg mb-4 ${agriReport.spray_window.is_suitable
              ? "bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800"
              : "bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800"
            }`}>
              <div className="flex items-center gap-2 mb-2">
                <div className={`w-3 h-3 rounded-full ${agriReport.spray_window.is_suitable ? "bg-green-500" : "bg-red-500"}`} />
                <span className={`font-bold text-sm ${agriReport.spray_window.is_suitable
              ...
              <p className={`text-sm ${agriReport.spray_window.is_suitable
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={c.label} className={`flex items-center gap-2 p-2 rounded text-sm ${c.ok
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 bg-orange-50 dark:bg-orange-900/10 rounded-lg">
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">درجات النمو المتراكمة (GDD)</p>
                  <p className="text-2xl font-bold text-orange-700 dark:text-orange-400">{agriReport.gdd_accumulated}</p>
                </div>
                <div className="text-left">
                  <p className="text-xs text-gray-500 dark:text-gray-400">اليوم</p>
                  <p className="text-lg font-bold text-orange-600 dark:text-orange-400">+{agriReport.gdd_daily}</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/10 rounded-lg">
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">التبخر-نتح المرجعي (ET₀)</p>
                  <p className="text-2xl font-bold text-blue-700 dark:text-blue-400">{agriReport.et_reference} <span className="text-sm font-normal">ملم/يوم</span></p>
                </div>
              ...
              <div className="p-3 bg-green-50 dark:bg-green-900/10 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold text-green-700 dark:text-green-400">{agriReport.soil_moisture_estimate}%</span>
                </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
            ...
            <div className={`p-4 rounded-lg ${agriReport.irrigation_recommendation.should_irrigate
              ? "bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-200 dark:border-cyan-800"
              : "bg-gray-50 dark:bg-gray-800/50"
            }`}>
              <div className="flex items-center gap-2 mb-2">
                <div className={`w-3 h-3 rounded-full ${agriReport.irrigation_recommendation.should_irrigate ? "bg-cyan-500 animate-pulse" : "bg-gray-400"}`} />
                <span className={`font-bold text-sm ${agriReport.irrigation_recommendation.should_irrigate
                  ? "text-cyan-800 dark:text-cyan-300"
                  : "text-gray-600 dark:text-gray-400"
                }`}>
                  {agriReport.irrigation_recommendation.should_irrigate ? "يُنصح بالري" : "لا حاجة للري حالياً"}
                </span>
              </div>
              <p className="text-sm text-gray-700 dark:text-gray-300">{agriReport.irrigation_recommendation.reason_ar}</p>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
            ...
            <div className="space-y-3">
              <div className={`flex items-center justify-between p-3 rounded-lg ${getRiskColor(agriReport.frost_risk.level)}`}>
                <div className="flex items-center gap-2">
                  <CloudSnow className="w-5 h-5" />
                  <div>
                    <p className="text-sm font-medium">خطر الصقيع</p>
                    <p className="text-xs opacity-75">أدنى حرارة متوقعة: {agriReport.frost_risk.min_temp_forecast}°م</p>
                  </div>
                </div>
                <span className="text-sm font-bold">{agriReport.frost_risk.level_ar}</span>
              </div>

              <div className={`flex items-center justify-between p-3 rounded-lg ${getRiskColor(agriReport.heat_stress.level)}`}>
                <div className="flex items-center gap-2">
                  <Thermometer className="w-5 h-5" />
                  <div>
                    <p className="text-sm font-medium">الإجهاد الحراري</p>
                    <p className="text-xs opacity-75">أعلى حرارة متوقعة: {agriReport.heat_stress.max_temp_forecast}°م</p>
                  </div>
                </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
        ...
        <div className="flex items-end gap-3 h-32">
          {forecast.map((day) => {
            ...
            const height = day.rain_amount_mm > 0 ? Math.max((day.rain_amount_mm / maxRain) * 100, 8) : 4;
            return (
              <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                {day.rain_amount_mm > 0 && (
                  <span className="text-xs font-bold text-blue-600 dark:text-blue-400">{day.rain_amount_mm}</span>
                )}
                <div
                  className={`w-full rounded-t-md transition-all ${day.rain_amount_mm > 0
                    ? "bg-gradient-to-t from-blue-600 to-blue-400"
                    : "bg-gray-200 dark:bg-gray-700"
                  }`}
                  style={{ height: `${height}%` }}
                />
                <span className="text-xs text-gray-500 dark:text-gray-400">{day.day_name_ar.slice(0, 3)}</span>
              </div>
            );
          })}
        </div>
