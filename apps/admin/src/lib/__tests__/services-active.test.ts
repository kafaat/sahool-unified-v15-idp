...
describe("Hooks Active", () => {
  ...
  const requiredHooks = [
    { file: "useWebSocket.ts", exports: ["useWebSocket", "useWebSocketEvent", "useRealtimeData", "useConnectionStatus"] },
    { file: "useRealTimeAlerts.ts", exports: ["useRealTimeAlerts", "useCriticalAlerts", "useAlertStats"] },
...
describe("WebSocket Service Active", () => {
  it("websocket.ts exists", () => {
    expect(
      fs.existsSync(path.join(SRC_DIR, "lib/websocket.ts")),
    ).toBe(true);
  });

  it("exports WebSocketClient class", () => {
    const content = fs.readFileSync(
      path.join(SRC_DIR, "lib/websocket.ts"),
      "utf-8",
    );
  ...
  it("supports all event types", () => {
    ...
    const eventTypes = [
      "alert", "sensor", "irrigation", "diagnosis",
      "farm_update", "weather", "task",
    ];
...
describe("Store Services Active", () => {
  ...
  it("auth store is active with login/logout/checkAuth", () => {
    const content = fs.readFileSync(
      path.join(storesDir, "auth.store.tsx"),
      "utf-8",
    );
  ...
  it("theme store is active with theme toggling", () => {
    const content = fs.readFileSync(
      path.join(storesDir, "theme.store.tsx"),
      "utf-8",
    );
...
describe("Notification Components Active", () => {
  ...
  it("Header component integrates notifications", () => {
    const content = fs.readFileSync(
      path.join(SRC_DIR, "components/layout/Header.tsx"),
      "utf-8",
    );
  ...
  it("AlertsPanel dashboard component is active", () => {
    expect(
      fs.existsSync(
        path.join(SRC_DIR, "components/dashboard/AlertsPanel.tsx"),
      ),
    ).toBe(true);
  });
  ...
  it("RealTimeActivityFeed component exists", () => {
    expect(
      fs.existsSync(
        path.join(SRC_DIR, "components/dashboard/RealTimeActivityFeed.tsx"),
      ),
    ).toBe(true);
  });
  ...
  it("Alerts management page exists", () => {
    expect(
      fs.existsSync(path.join(SRC_DIR, "app/alerts/page.tsx")),
    ).toBe(true);
  });
});
...
describe("CSRF Protection Active", () => {
  ...
  it("useCsrf hook provides token management", () => {
    const content = fs.readFileSync(
      path.join(SRC_DIR, "hooks/useCsrf.ts"),
      "utf-8",
    );
