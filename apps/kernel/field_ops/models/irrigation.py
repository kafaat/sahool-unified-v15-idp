...
class WeatherData(BaseModel):
    ...
    temp_max: float = Field(
        ..., ge=-10, le=60, description="أقصى درجة حرارة - Max temperature"
    )
    ...
    wind_speed: float = Field(
        2.0, ge=0, le=50, description="سرعة الرياح - Wind speed at 2m height"
    )
    ...
    elevation: float = Field(
        0, ge=0, description="الارتفاع عن سطح البحر - Elevation (m)"
    )
...
class SoilProperties(BaseModel):
    ...
    root_depth: float = Field(
        0.3, ge=0.1, le=3.0, description="عمق الجذور - Root depth (m)"
    )
...
class CropCoefficient(BaseModel):
    ...
    stage_duration_days: int = Field(
        ..., ge=1, description="مدة المرحلة بالأيام - Stage duration"
    )
...
class WaterBalance(BaseModel):
    ...
    rainfall: float = Field(0.0, ge=0, description="كمية الأمطار - Rainfall amount")
    effective_rainfall: float = Field(
        0.0, ge=0, description="الأمطار الفعالة - Effective rainfall"
    )

    # المخرجات - Outputs (mm)
    et0: float = Field(
        ..., ge=0, description="التبخر المرجعي - Reference evapotranspiration"
    )
...
class IrrigationEvent(BaseModel):
    ...
    duration_minutes: int = Field(
        ..., ge=1, description="المدة بالدقائق - Duration in minutes"
    )
    ...
    irrigation_type: IrrigationType = Field(
        ..., description="نوع الري - Irrigation type"
    )
    status: IrrigationStatus = Field(
        IrrigationStatus.SCHEDULED, description="الحالة - Status"
    )
...
class IrrigationSchedule(BaseModel):
    ...
    total_water_mm: float = Field(
        0.0, ge=0, description="إجمالي المياه (مم) - Total water (mm)"
    )
    total_water_m3: float = Field(
        0.0, ge=0, description="إجمالي المياه (م³) - Total water (m³)"
    )
    ...
    def completion_rate(self) -> float:
        ...
        if not self.events:
            return 0.0
        completed = len(
            [e for e in self.events if e.status == IrrigationStatus.COMPLETED]
        )
...
class IrrigationRecommendation(BaseModel):
    ...
    urgency: str = Field(
        "low", description="الأهمية - Urgency: low/medium/high/critical"
    )
