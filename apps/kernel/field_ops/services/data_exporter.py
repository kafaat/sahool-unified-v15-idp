...
class DataExporter:
    ...
    def _recommendations_to_pdf(self, field_id: str, recommendations: list[dict]) -> bytes:
        ...
        # Title
        title = Paragraph(f"التوصيات - Recommendations<br/>Field: {field_id}", styles["Title"])
        story.append(title)
        story.append(Spacer(1, 0.3 * inch))
    ...
    def _generate_filename(self, prefix: str, field_id: str, format: ExportFormat) -> str:
...
def generate_daily_report(field_id: str, report_date: date | None = None) -> ExportResult:
    ...
    return exporter.generate_report(
        ReportType.DAILY_SUMMARY, {"field_id": field_id, "date": report_date or date.today()}
    )
