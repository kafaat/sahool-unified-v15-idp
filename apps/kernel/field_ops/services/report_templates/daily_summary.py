...
class DailySummaryReport:
    ...
    def _create_executive_summary(self, data: dict) -> list:
        ...
        # Summary text
        summary_table_data = [["عربي", summary["summary_ar"]], ["English", summary["summary_en"]]]
        ...
        # Alerts if any
        if data.get("alerts"):
            elements.append(Paragraph("تنبيهات - Alerts", self.styles["BilingualHeading"]))
    ...
    def _create_ndvi_section(self, data: dict) -> list:
        ...
        if data.get("ndvi_week") and len(data["ndvi_week"]) > 1:
            elements.append(Paragraph("اتجاه أسبوعي / Weekly Trend", self.styles["EnglishHeading"]))
    ...
    def _create_recommendations_section(self, data: dict) -> list:
        ...
        elements.append(Paragraph("التوصيات - Recommendations", self.styles["BilingualHeading"]))
