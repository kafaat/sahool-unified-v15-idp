...
def create_app_icon(output_path: str, size: int = 1024):
    ...
    draw.ellipse(
        [center - leaf_size // 2, center - leaf_size, center + leaf_size // 2, center],
        fill=WHITE,
    )
    draw.ellipse(
        [center - leaf_size, center - leaf_size // 2, center, center + leaf_size // 2],
        fill=WHITE,
    )
    draw.ellipse(
        [center, center - leaf_size // 2, center + leaf_size, center + leaf_size // 2],
        fill=WHITE,
    )

    # Stem
    draw.rectangle(
        [center - leaf_size // 8, center, center + leaf_size // 8, center + leaf_size],
        fill=WHITE,
    )
...
def create_splash_logo(output_path: str, size: int = 512):
    ...
    for angle_offset in [-60, 0, 60]:
        ...
        draw.ellipse(
            [
                center - leaf_size // 3 + x_offset,
                center - leaf_size - abs(x_offset) // 2,
                center + leaf_size // 3 + x_offset,
                center - abs(x_offset) // 2,
            ],
            fill=WHITE,
        )
