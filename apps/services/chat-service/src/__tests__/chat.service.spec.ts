...
describe("ChatService", () => {
  ...
  describe("updateTypingIndicator", () => {
    ...
    it("should set typing to false", async () => {
      ...
      await service.updateTypingIndicator(
        mockConversationId,
        mockUserId,
        false,
      );
  ...
  describe("updateOnlineStatus", () => {
    ...
    it("should update lastSeenAt timestamp", async () => {
      ...
      const callArgs =
        mockPrismaService.participant.updateMany.mock.calls[0][0];
      expect(callArgs.data.lastSeenAt).toBeInstanceOf(Date);
      expect(callArgs.data.lastSeenAt.getTime()).toBeGreaterThanOrEqual(
        beforeTime.getTime(),
      );
    });
  });
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should handle large message content", async () => {
      ...
      mockPrismaService.$transaction.mockImplementation(async (callback) => {
        return callback({
          message: {
            create: jest
              .fn()
              .mockResolvedValue({ ...mockMessage, content: largeContent }),
          },
          conversation: { update: jest.fn() },
          participant: { updateMany: jest.fn() },
        });
      ...
      await expect(
        service.sendMessage(dtoWithLargeContent),
