...
CROP_KNOWLEDGE = {
    "wheat": {
        "name": "Wheat", "name_ar": "القمح",
        "family": "Poaceae", "family_ar": "النجيليات",
        "growth_stages": ["Germination", "Tillering", "Stem elongation", "Heading", "Flowering", "Grain filling", "Maturity"],
    ...
    "date_palm": {
        "name": "Date Palm", "name_ar": "النخيل",
        "family": "Arecaceae", "family_ar": "النخيليات",
    ...
    "tomato": {
        "name": "Tomato", "name_ar": "الطماطم",
        "family": "Solanaceae", "family_ar": "الباذنجانيات",
        ...
        "common_diseases": [
            ...
            {"name": "Tomato Yellow Leaf Curl", "name_ar": "تجعد أوراق الطماطم", "ndvi_indicator": "severe NDVI decline"},
        ],
        "yemen_notes": "تُزرع في تهامة والمرتفعات. موسمان: شتوي وصيفي.",
    },
    "barley": {
        "name": "Barley", "name_ar": "الشعير",
        "family": "Poaceae", "family_ar": "النجيليات",
...
async def get_crop_encyclopedia(crop_type: str):
    ...
    if not crop:
        ...
        return {"error": f"Crop '{crop_type}' not found", "error_ar": f"المحصول '{crop_type}' غير موجود", "available_crops": available}
    ...
    for crop_type, data in CROP_KNOWLEDGE.items():
        ...
        for d in data.get("common_diseases", []):
            if q_lower in d.get("name", "").lower() or q_lower in d.get("name_ar", ""):
                results.append({"crop_type": crop_type, "match_type": "disease", "match": d["name_ar"] if lang == "ar" else d["name"]})
        # Search in pests
        for p in data.get("common_pests", []):
            if q_lower in p.get("name", "").lower() or q_lower in p.get("name_ar", ""):
                results.append({"crop_type": crop_type, "match_type": "pest", "match": p["name_ar"] if lang == "ar" else p["name"]})
    return {"query": q, "language": lang, "results": results, "count": len(results)}

@router.get("/")
async def list_crops():
    crops = [{"crop_type": k, "name": v["name"], "name_ar": v["name_ar"], "family_ar": v["family_ar"]} for k, v in CROP_KNOWLEDGE.items()]
