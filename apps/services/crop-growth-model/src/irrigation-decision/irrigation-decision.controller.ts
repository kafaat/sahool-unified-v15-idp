...
export class IrrigationDecisionController {
  ...
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        ...
        terrain: {
          type: "string",
          enum: ["plain", "mountain", "greenhouse", "terrace"],
          example: "plain",
        },
        ...
        technicalCapability: {
          type: "string",
          enum: ["advanced", "basic", "minimal"],
          example: "basic",
        },
        waterAvailability: {
          type: "string",
          enum: ["abundant", "limited", "scarce"],
          example: "limited",
        },
      },
      required: [
  ...
  selectMethod(@Body() input: ScenarioInput) {
    const recommendation =
      this.irrigationDecisionService.selectOptimalMethod(input);
  ...
  evaluateThreshold(@Body() input: ThresholdInput) {
    const recommendation =
      this.irrigationDecisionService.evaluateThresholdControl(input);

    return {
      input,
      recommendation,
      methodology:
        "Soil moisture threshold control with growth-stage specific thresholds",
    };
  }
  ...
  @ApiOperation({
    ...
    description:
      "مقارنة أساليب اتخاذ قرارات الري (FAO-56، العتبة، نماذج المحاصيل)",
  })
  ...
  quickRecommend(@Query("scenario") scenario: string) {
    ...
    const recommendation =
      this.irrigationDecisionService.selectOptimalMethod(scenarioInput);
  ...
  getSoilProperties(@Param("soilType") soilType: string) {
    const properties =
      this.irrigationDecisionService.getSoilProperties(soilType);
