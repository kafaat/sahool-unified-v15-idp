...
export class PlantingStrategyService {
  ...
  private readonly plantingMethods: Map<PlantingMethodType, PlantingMethod> =
    new Map([
      [
        "equal_row",
        {
          ...
          nameCn: "等行条播",
          description:
            "Traditional method with equal spacing between all rows, suitable for moderate yield targets",
      ...
      [
        "wide_strip",
        {
          ...
          nameCn: "宽幅条播",
          description:
            "High-density method with wide seeding strips for maximum yield in fertile conditions",
      ...
      [
        "space_broadcasting",
        {
          ...
          nameCn: "宽窄行条播",
          description:
            "Alternating wide and narrow rows for optimal light capture and ventilation",
      ...
      [
        "small_basin",
        {
          ...
          nameCn: "畦播",
          description:
            "Basin-based planting for water conservation in arid regions",
  ...
  private readonly cropParameters: Map<
    CropType,
    {
  ...
  private readonly soilFactors: Map<
    SoilType,
    {
  ...
  > = new Map([
    [
      "sandy",
      {
        waterRetention: 0.6,
        fertilityFactor: 0.7,
        workabilityFactor: 1.2,
        densityAdjustment: 0.9,
      },
    ],
    [
      "loamy",
    ...
    [
      ...
      {
        waterRetention: 1.1,
        fertilityFactor: 1.05,
        workabilityFactor: 0.9,
    ...
    [
      "saline",
      {
        waterRetention: 0.8,
        fertilityFactor: 0.6,
        workabilityFactor: 0.8,
        densityAdjustment: 0.8,
      },
    ],
  ]);

  // Climate adjustment factors
  private readonly climateFactors: Map<
    ClimateZone,
    {
  ...
  optimizePlantingStrategy(request: OptimizationRequest): OptimizationResult {
    ...
    const alternativeMethods = sortedMethods
      .slice(1, 3)
    ...
    if (fieldAnalysis.waterAvailabilityScore < 0.6) {
      warnings.push(
        "Water availability is limited. Consider drought-tolerant varieties.",
      );
    }
    if (fieldAnalysis.soilQualityScore < 0.5) {
      warnings.push(
        "Soil quality is below optimal. Soil improvement recommended.",
      );
    }
    if (request.targetYield > bestMethod.suitableYieldRange.max) {
      warnings.push(
        `Target yield exceeds typical range for ${bestMethod.nameEn}. Results may vary.`,
      );
    }

    recommendations.push(
      `Optimal planting depth: ${spacingConfig.seedDepth} cm`,
    );
    ...
    if (bestMethod.id === "wide_strip") {
      recommendations.push(
        "Use precision seeder for optimal strip width (8-12cm)",
      );
    }
    if (bestMethod.id === "small_basin") {
      recommendations.push(
        "Prepare basins 5-7 days before planting to allow settling",
      );
    }
  ...
  analyzeField(request: OptimizationRequest): FieldAnalysis {
    ...
    const waterAvailabilityScore = Math.min(
      1,
      request.availableWater / requiredWater,
    );
    ...
    if (soilFactor.fertilityFactor < 0.8) {
      ...
      improvementSuggestions.push(
        "Apply organic matter and balanced NPK fertilizer",
      );
    }
    if (waterAvailabilityScore < 0.7) {
      limitingFactors.push("Insufficient water supply");
      improvementSuggestions.push(
        "Consider drip irrigation or water-conserving methods",
      );
    }
    ...
    if (request.climateZone === "arid") {
      ...
      improvementSuggestions.push(
        "Use mulching and schedule irrigation for early morning",
      );
    }
  ...
  private scoreAllMethods(
    ...
    for (const [methodId, method] of this.plantingMethods) {
      ...
      if (
        targetYield >= method.suitableYieldRange.min &&
        targetYield <= method.suitableYieldRange.max
      ) {
        score += 0.25;
      ...
      if (
        methodId === "small_basin" &&
        fieldAnalysis.waterAvailabilityScore < 0.6
      ) {
        score += 0.1; // Bonus for water-conserving method in water-scarce conditions
      }
      if (
        methodId === "wide_strip" &&
        fieldAnalysis.waterAvailabilityScore > 0.8
      ) {
  ...
  calculateDensity(
    ...
    const baseRowSpacing =
      (methodConfig.rowSpacing.min + methodConfig.rowSpacing.max) / 2;
    ...
    const rowsPerMu = Math.floor(
      (Math.sqrt(muArea / rowSpacing) / 100) * rowSpacing,
    );
    ...
    const seedingRateKgPerMu =
      (totalPlantsPerMu * cropParams.thousandGrainWeight) /
      1000 /
      cropParams.germinationRate /
      1000;
  ...
  calculateFertilizerRecommendations(
    ...
    const organicMatterNeeded =
      organicMatterContent < 2.0
  ...
  calculateIrrigationSchedule(
    ...
    const schedule = {
      ...
      emergence: totalRequired * 0.1,
      tillering: totalRequired * 0.2,
      jointing: totalRequired * 0.25,
      heading: totalRequired * 0.2,
      grainfilling: totalRequired * 0.1,
  ...
  predictYield(
    ...
    adjustmentFactors.push({
      ...
      impact: soilImpact,
      description:
        fieldAnalysis.soilQualityScore >= 0.7
    ...
    adjustmentFactors.push({
      ...
      impact: waterImpact,
      description:
        fieldAnalysis.waterAvailabilityScore >= 0.7
    ...
    adjustmentFactors.push({
      ...
      description:
        fieldAnalysis.climateCompatibilityScore >= 0.9
          ? "Favorable climate"
    ...
    const methodBonus =
      method === "wide_strip" ? 10 : method === "space_broadcasting" ? 5 : 0;
    if (methodBonus > 0) {
      adjustmentFactors.push({
    ...
    const baseYield = Math.min(
      targetYield,
      methodConfig.suitableYieldRange.max,
    );
  ...
  generatePlantingPlan(
    ...
    const cropParams = this.cropParameters.get(request.cropType)!;
    const seedCost =
      optimization.spacingConfiguration.seedingRate * 5 * request.fieldArea; // $5/kg estimate

    const resourceRequirements: ResourceRequirements = {
      seeds: {
        quantity:
          optimization.spacingConfiguration.seedingRate * request.fieldArea,
        unit: "kg",
        cost: seedCost,
      },
      fertilizer: [
        {
          type: "Urea (46% N)",
          quantity:
            optimization.fertilizerRecommendations.baseApplication.n *
            2.17 *
            request.fieldArea,
          unit: "kg",
          cost:
        ...
        },
        {
          type: "DAP (18-46-0)",
          quantity:
            optimization.fertilizerRecommendations.baseApplication.p *
            2.17 *
            request.fieldArea,
          unit: "kg",
          cost:
        ...
        },
        {
          ...
          quantity:
            optimization.fertilizerRecommendations.baseApplication.k *
            2 *
            request.fieldArea,
          unit: "kg",
          cost:
      ...
      water: {
        quantity:
          optimization.irrigationSchedule.totalWater * request.fieldArea,
        unit: "m³",
      },
      labor: {
      ...
      machinery: [
        {
          type: "Tractor",
          hours: (20 * request.fieldArea) / 10,
          cost: ((20 * request.fieldArea) / 10) * 30,
        },
        {
          type: "Seeder",
          hours: (8 * request.fieldArea) / 10,
          cost: ((8 * request.fieldArea) / 10) * 20,
        },
      ],
      totalCost: 0,
    };
    ...
    const timeline: TimelineEvent[] = tasks.map((task) => ({
      event: task.name,
      date: new Date(
        plantingDate.getTime() - task.daysBeforePlanting * 24 * 60 * 60 * 1000,
      ),
      description: task.description,
      critical: ["planting", "base_fertilizer", "pre_irrigation"].includes(
        task.id,
      ),
    }));
    ...
    growthMilestones.forEach((milestone) => {
      timeline.push({
        ...
        date: new Date(
          plantingDate.getTime() + milestone.days * 24 * 60 * 60 * 1000,
        ),
  ...
  compareMethods(
    ...
    for (const [methodId, method] of this.plantingMethods) {
      ...
      const irrigation = this.calculateIrrigationSchedule(
        cropType,
        methodId,
        climateZone,
        500,
      ...
      const suitabilityScore =
        scores.find((s) => s.method === methodId)?.score || 0;
    ...
    const recommendation =
      `Based on your conditions (${soilType} soil, ${climateZone} climate, target ${targetYield} kg/mu), ` +
  ...
  getMethodGuidance(method: PlantingMethodType): {
      ...
      equal_row: [
        {
          ...
          titleAr: "تحضير الحقل",
          description: "Plow to 20-25cm depth and level the field",
          tips: ["Ensure good drainage", "Remove crop residues"],
        },
        {
          step: 2,
          title: "Mark Rows",
          titleAr: "تحديد الصفوف",
          description: "Mark rows at 15-20cm intervals",
          tips: [
            "Use string lines for straight rows",
        ...
        },
        {
          ...
          title: "Seeding",
          titleAr: "البذر",
          description: "Sow seeds at 3-5cm depth",
          tips: ["Check seeder calibration", "Maintain uniform depth"],
        },
        {
          step: 4,
          title: "Cover and Press",
          titleAr: "التغطية والضغط",
          description: "Cover seeds and lightly compress soil",
          tips: ["Ensure seed-soil contact", "Avoid over-compaction"],
        },
        {
          step: 5,
          title: "Initial Irrigation",
          titleAr: "الري الأولي",
          description: "Apply light irrigation immediately",
          tips: ["Avoid waterlogging", "Irrigate early morning"],
        },
      ],
      wide_strip: [
        {
          ...
          title: "Field Preparation",
          titleAr: "تحضير الحقل",
          description: "Deep plow and fine tilth preparation",
          tips: ["Achieve fine soil structure", "Level precisely"],
        },
        {
          step: 2,
          title: "Calibrate Seeder",
          titleAr: "معايرة البذارة",
          description: "Set strip width to 8-12cm, row spacing 20-30cm",
          tips: ["Test on sample area first", "Check seed rate"],
        },
        {
          step: 3,
          title: "Create Strips",
          titleAr: "إنشاء الأشرطة",
          description: "Plant in wide strips with precision seeder",
          tips: ["Maintain consistent strip width", "Overlap prevention"],
        },
        {
          step: 4,
          title: "Firm Soil",
          titleAr: "ضغط التربة",
          description: "Roll to ensure seed-soil contact",
          tips: ["Use appropriate roller weight", "Roll when soil is moist"],
        },
        {
          step: 5,
          title: "Monitor Closely",
          titleAr: "المراقبة عن كثب",
          description: "Check emergence uniformity",
          tips: [
            "Reseed thin areas if needed",
            "Watch for lodging signs later",
          ],
        },
      ],
      space_broadcasting: [
        {
          ...
          title: "Field Preparation",
          titleAr: "تحضير الحقل",
          description: "Standard tillage with good leveling",
          tips: ["Ensure uniform soil moisture", "Break clods thoroughly"],
        },
        {
          step: 2,
          title: "Plan Row Pattern",
          titleAr: "تخطيط نمط الصفوف",
          description: "Alternate wide (20-30cm) and narrow (10-20cm) rows",
          tips: ["Mark pattern before planting", "Consider machinery width"],
        },
        {
          step: 3,
          title: "Adjust Seeder",
          titleAr: "ضبط البذارة",
          description: "Configure for alternating row spacing",
          tips: ["Use adjustable seeder", "Test pattern accuracy"],
        },
        {
          step: 4,
          title: "Plant Systematically",
          titleAr: "الزراعة المنتظمة",
          description: "Follow the marked pattern precisely",
          tips: ["Maintain consistent speed", "Check alignment regularly"],
        },
        {
          step: 5,
          title: "Post-plant Care",
          titleAr: "العناية بعد الزراعة",
          description: "Light irrigation and monitoring",
          tips: ["Wide rows allow cultivation", "Good air circulation"],
        },
      ],
      small_basin: [
        {
          step: 1,
          title: "Field Leveling",
          titleAr: "تسوية الحقل",
          description: "Precise leveling is critical for basin irrigation",
          tips: ["Use laser leveler if available", "Slope <0.1%"],
        },
        {
          step: 2,
          title: "Basin Formation",
          titleAr: "تشكيل الأحواض",
          description: "Create basins 20-25cm wide, 10-15cm deep",
          tips: ["Use basin-making equipment", "Compact basin walls"],
        },
        {
          step: 3,
          title: "Pre-irrigation",
          titleAr: "الري المسبق",
          description: "Fill basins to check level and moisten soil",
          tips: ["Identify low spots", "Allow settling"],
        },
        {
          step: 4,
          title: "Seeding in Basins",
          titleAr: "البذر في الأحواض",
          description: "Plant seeds in bottom of basins",
          tips: ["Deeper planting (10-15cm)", "2-3 seeds per spot"],
        },
        {
          step: 5,
          title: "Basin Irrigation",
          titleAr: "ري الأحواض",
          description: "Flood irrigate to basin capacity",
          tips: ["Water conservation benefit", "Avoid overflow"],
        },
      ],
    };
    ...
    const commonMistakes: Record<
      PlantingMethodType,
      { mistake: string; solution: string }[]
    > = {
      equal_row: [
        {
          mistake: "Inconsistent row spacing",
          solution: "Use mechanical markers or GPS guidance",
        },
        {
          mistake: "Planting too deep",
          solution: "Calibrate seeder depth regularly",
        },
        {
          mistake: "Over-seeding",
          solution: "Follow recommended rates for yield target",
        },
      ],
      wide_strip: [
        {
          mistake: "Strips too wide causing competition",
          solution: "Keep strips 8-12cm maximum",
        },
        {
          mistake: "Seed bunching in strips",
          solution: "Check seed metering mechanism",
        },
        {
          mistake: "Lodging from over-density",
          solution: "Reduce seeding rate on fertile soils",
        },
      ],
      space_broadcasting: [
        {
          mistake: "Losing the wide-narrow pattern",
          solution: "Mark edges clearly before planting",
        },
        {
          mistake: "Uneven distribution",
          solution: "Maintain consistent planting speed",
        },
        {
          mistake: "Wrong row ratio",
          solution: "Plan pattern mathematically before field work",
        },
      ],
      small_basin: [
        {
          mistake: "Basins not level",
          solution: "Invest time in precision leveling",
        },
        {
          mistake: "Basin walls eroding",
          solution: "Compact walls and avoid over-filling",
        },
        {
          mistake: "Waterlogging",
          solution: "Ensure drainage and don't over-irrigate",
        },
      ],
    };
    ...
    const equipment: Record<
      PlantingMethodType,
      { name: string; purpose: string; alternatives: string[] }[]
    > = {
      equal_row: [
        {
          name: "Seed drill",
          purpose: "Precision row seeding",
          alternatives: ["Manual seeder", "Broadcast spreader with rake"],
        },
        {
          name: "Row marker",
          purpose: "Consistent spacing",
          alternatives: ["String lines", "GPS guidance"],
        },
        {
          name: "Roller",
          purpose: "Seed-soil contact",
          alternatives: ["Cultipacker", "Manual tamping"],
        },
      ],
      wide_strip: [
        {
          name: "Wide-strip seeder",
          purpose: "Create uniform strips",
          alternatives: ["Modified seed drill"],
        },
        {
          name: "Precision depth control",
          purpose: "Consistent seed depth",
          alternatives: ["Depth wheels"],
        },
        {
          name: "Heavy roller",
          purpose: "Firm seed bed",
          alternatives: ["Cambridge roller"],
        },
      ],
      space_broadcasting: [
        {
          name: "Adjustable row seeder",
          purpose: "Variable spacing",
          alternatives: ["Multiple pass with standard drill"],
        },
        {
          name: "Pattern marker",
          purpose: "Maintain alternating pattern",
          alternatives: ["GPS mapping"],
        },
        {
          name: "Standard cultivator",
          purpose: "Inter-row cultivation",
          alternatives: ["Manual hoe"],
        },
      ],
      small_basin: [
        {
          name: "Basin maker",
          purpose: "Form uniform basins",
          alternatives: ["Manual basin hoe", "Furrow opener"],
        },
        {
          name: "Laser leveler",
          purpose: "Precision field leveling",
          alternatives: ["Traditional leveling board"],
        },
        {
          name: "Basin irrigator",
          purpose: "Controlled water delivery",
          alternatives: ["Flood gates", "Siphon tubes"],
        },
      ],
    };
  ...
  integrateWithDigitalTwin(
    ...
    const stageActions: Record<
      string,
      { action: string; timing: string; priority: "high" | "medium" | "low" }
    > = {
      tillering: {
      ...
      jointing: {
        action: "Second nitrogen application",
        timing: "Now",
        priority: "high",
      },
      heading: {
        action: "Foliar micronutrient spray",
        timing: "Next week",
        priority: "medium",
      },
      grainfilling: {
        action: "Maintain irrigation, reduce nitrogen",
        timing: "Ongoing",
        priority: "medium",
      },
    };
    ...
    const currentStatus = alerts.some((a) => a.level === "critical")
