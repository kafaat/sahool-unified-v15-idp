...
export interface RootParams {
  ...
  nutrientUptakeKm: {
    // Michaelis-Menten Km values (μmol L⁻¹)
    nitrogen: number;
    phosphorus: number;
    potassium: number;
  };
...
export class RootGrowthService {
  ...
  calculateRootDepth(
    ...
    const tempFactor = this.temperatureResponseFunction(
      soilTemperature,
      10,
      25,
      35,
    ...
    const sigmoidFactor =
      1 / (1 + Math.exp(-k * (daysAfterEmergence - midpoint)));
  ...
  calculateRootLengthDensity(
    ...
    for (
      let depth = 5;
      depth <= Math.min(currentDepth, params.maxRootDepth);
    ...
    for (
      ...
      const rld =
        (totalRootLength * 100 * (relativeRLD / cumulativeRLD)) /
  ...
  calculateWaterUptake(
    ...
    const rldProfile = this.calculateRootLengthDensity(
      cropType,
      100,
      rootDepth,
    );
    ...
    soilLayers.forEach((layer) => {
      ...
      // Find matching RLD
      const rldLayer = rldProfile.find(
        (r) => Math.abs(r.depth - layer.depth) < 5,
      );
      const relativeRLD = rldLayer ? rldLayer.relativeRLD : 0;

      // Water availability factor (0-1)
      const availableWater =
        (layer.waterContent - layer.wiltingPoint) /
  ...
  simulateRootArchitecture(
    ...
    const lateralSpread =
      currentDepth * Math.tan((params.branchingAngle * Math.PI) / 180);
  ...
  private temperatureResponseFunction(
    ...
    const response =
      (2 * Math.pow(temp - tmin, alpha) * Math.pow(topt - tmin, alpha) -
        Math.pow(temp - tmin, 2 * alpha)) /
      Math.pow(topt - tmin, 2 * alpha);
  ...
  getCropParameters(
    cropType?: string,
  ): RootParams | Record<string, RootParams> | null {
    if (cropType) {
