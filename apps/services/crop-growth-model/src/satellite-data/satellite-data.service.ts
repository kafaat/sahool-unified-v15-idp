...
export class SatelliteDataService {
  ...
  selectOptimalDataSource(
    ...
    return {
      ...
      estimatedDataAvailability: this.estimateAvailability(
        primary,
        requirements,
      ),
    };
  ...
  private scoreCandidates(
    ...
    return candidates.map((sat) => {
      ...
      const spatialScore = this.scoreSpatialResolution(
        sat.spatialResolution,
        requirements.spatialResolution,
      );
      score += spatialScore;

      // Temporal resolution score (0-30 points)
      const temporalScore = this.scoreTemporalResolution(
        sat.temporalResolution,
        requirements.temporalResolution,
      );
      score += temporalScore;
  ...
  private scoreSpatialResolution(
    resolution: number,
  ...
  private scoreTemporalResolution(
    resolution: number,
    requirement: "daily" | "weekly" | "biweekly" | "monthly",
  ): number {
    switch (requirement) {
      case "daily":
  ...
  private generateReasoning(
    ...
    if (!satellite)
      return "No suitable satellite found for the given requirements.";

    return (
      `${satellite.nameEn} is recommended for ${requirements.application} application because it offers ` +
  ...
  private generateReasoningAr(
    satellite: SatelliteSource,
    requirements: DataRequirements,
  ): string {
    if (!satellite)
      return "لم يتم العثور على قمر صناعي مناسب للمتطلبات المحددة.";
    ...
    return (
      `يُوصى باستخدام ${satellite.nameAr} لتطبيق ${appNames[requirements.application] || requirements.application} ` +
  ...
  private suggestCombinedApproach(
    ...
    | { description: string; descriptionAr: string; sources: SatelliteSource[] }
    | undefined {
    // Suggest combining high temporal with high spatial
    if (
      requirements.temporalResolution === "daily" &&
      requirements.spatialResolution === "high"
    ) {
      ...
      if (modis && sentinel2) {
        return {
          description:
            "Combine MODIS daily coverage with Sentinel-2 high resolution using data fusion techniques (e.g., STARFM algorithm)",
          descriptionAr:
            "دمج التغطية اليومية من MODIS مع الدقة العالية من Sentinel-2 باستخدام تقنيات دمج البيانات (مثل خوارزمية STARFM)",
    ...
    if (requirements.cloudFree && requirements.application !== "soil") {
      ...
      const optical = scored.find(
        (s) => !s.satellite.sensor.includes("SAR"),
      )?.satellite;
  ...
  private getProcessingNotes(
    ...
    if (satellite.id.startsWith("LANDSAT")) {
      notes.push(
        "Apply surface reflectance correction (already in Collection 2 L2)",
      );
    ...
    if (satellite.id === "MODIS") {
      notes.push(
        "Use ready-made products when available (MOD13, MOD15, MOD16)",
      );
  ...
  getBandCombinationByIndex(indexName: string): BandCombination | undefined {
    return this.bandCombinations.find(
      (b) => b.name.toLowerCase() === indexName.toLowerCase(),
    );
  }
  ...
  compareSatellites(ids: string[]): {
    ...
    return {
      ...
      comparison: {
        ...
        summary:
          `Best spatial: ${bestSpatial.nameEn} (${bestSpatial.spatialResolution}m), ` +
          `Best temporal: ${bestTemporal.nameEn} (${bestTemporal.temporalResolution} days)`,
        summaryAr:
          `أفضل دقة مكانية: ${bestSpatial.nameAr} (${bestSpatial.spatialResolution}م)، ` +
  ...
  getRecommendationForModule(
    module: "phenology" | "photosynthesis" | "biomass" | "roots" | "water",
  ): {
    ...
    switch (module) {
      case "phenology":
        return {
          ...
          indices: this.bandCombinations.filter((b) =>
            ["NDVI", "EVI", "NDRE"].includes(b.name),
          ),
          notes: [
      ...
      case "biomass":
        return {
          ...
          indices: this.bandCombinations.filter((b) =>
            ["NDVI", "EVI", "LAI (from NDVI)"].includes(b.name),
          ),
          notes: [
