...
export class VoiceGuidanceController {
  ...
  getVoice(@Param("id") id: string) {
    ...
    if (!voice) {
      return {
        ...
        availableVoices: this.voiceGuidanceService
          .getVoiceProfiles()
          .map((v) => ({
            id: v.id,
            name: v.nameEn,
  ...
  getScript(@Param("id") id: string) {
    ...
    if (!script) {
      return {
        ...
        availableScripts: this.voiceGuidanceService
          .getAllScripts()
          .map((s) => s.id),
      };
    }
    ...
    return {
      script,
      suggestedVoices: this.voiceGuidanceService
        .getVoiceProfiles()
        .filter((v) =>
    ...
    schema: {
      ...
      properties: {
        ...
        soilMoisture: {
          type: "number",
          example: 0.22,
          description: "Volumetric (0-1)",
        },
        cropStage: { type: "string", example: "flowering" },
        ndvi: {
          type: "number",
          example: 0.65,
          description: "Vegetation index (optional)",
        },
        alerts: {
          type: "array",
          items: { type: "string" },
  ...
  generateBriefing(@Body() input: FieldBriefingInput) {
    ...
    return {
      ...
      apiNoteAr:
        "يمكن تحويل هذا الملخص إلى صوت باستخدام تكامل تحويل النص إلى كلام",
    };
  }
  ...
  @ApiOperation({
    ...
    description:
      "توليد حلقة بأسلوب البودكاست حول موضوع زراعي - مستوحى من SoulX-Podcast",
  })
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        ...
        cropType: {
          type: "string",
          example: "wheat",
          description: "Specific crop (optional)",
        },
        language: { type: "string", enum: ["ar", "en"], example: "ar" },
        voiceStyle: {
          type: "string",
        ...
        includeLocalWisdom: {
          type: "boolean",
          example: true,
          description: "Include traditional sayings",
        },
      },
      required: ["topic"],
    },
  ...
  getQuickTip(
    ...
    const tip = this.voiceGuidanceService.getQuickTip(
      situation,
      language || "ar",
    );
  ...
  @ApiQuery({
    ...
    required: false,
    type: "boolean",
    description: "Get all wisdom quotes",
  })
