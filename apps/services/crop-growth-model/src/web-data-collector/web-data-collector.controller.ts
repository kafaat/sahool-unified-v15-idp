...
export class WebDataCollectorController {
  ...
  @ApiQuery({ name: 'type', required: false, enum: ['weather', 'market', 'research', 'satellite', 'commodity', 'news', 'government'] })
  ...
  listSources(
    ...
    if (region) {
      sources = sources.filter(s => s.region.includes(region) || s.region.includes('global'));
    }
  ...
  getSource(@Param('id') id: string) {
    ...
    if (!source) {
      return {
        ...
        availableSources: this.collectorService.getAllDataSources().map(s => s.id),
      };
    }
  ...
  createCollectionJob(@Body() input: CollectionJobInput) {
    ...
    return {
      ...job,
      apiNote: 'In production, this would trigger actual web scraping/API calls',
      apiNoteAr: 'في بيئة الإنتاج، سيؤدي هذا إلى تشغيل عمليات استخراج فعلية من الويب',
    };
  }
  ...
  getWeatherAlerts(@Query('region') region?: string) {
    ...
    return {
      alerts,
      total: alerts.length,
      highSeverityCount: alerts.filter(a => a.severity === 'high' || a.severity === 'extreme').length,
  ...
  getNews(
    ...
    return {
      news,
      total: news.length,
      categories: ['policy', 'market', 'technology', 'weather', 'research', 'pest'],
  ...
  getIntelligenceReport(@Query('region') region?: string) {
    const report = this.collectorService.getAgriculturalIntelligence(region || 'saudi_arabia');

    return {
      ...report,
      apiNote: 'Comprehensive agricultural intelligence from multiple data sources',
  ...
  runDemo() {
    ...
    const intelligence = this.collectorService.getAgriculturalIntelligence('saudi_arabia');
