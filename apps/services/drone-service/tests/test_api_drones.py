...
class TestHelpers:
    ...
    def test_drone_to_response(self):
        d = {"id": "123", "name": "D1", "model": "M1", "serial_number": "S1", "drone_type": "custom", "status": "active", "extra_field": "ignore"}
...
async def test_register_and_list_drone(app):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        create_resp = await client.post("/api/v1/drones/", json={
            "name": "TestDrone",
            "model": "DJI-T30",
            "serial_number": "SN001",
        })
...
async def test_get_drone(app):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        create_resp = await client.post("/api/v1/drones/", json={
            "name": "D1", "model": "M1", "serial_number": "S1",
        })
...
async def test_update_drone(app):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        update_resp = await client.put(f"/api/v1/drones/{drone_id}", json={
            "name": "Updated", "model": "M2", "serial_number": "S2",
        })
...
async def test_update_drone_not_found(app):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        resp = await client.put("/api/v1/drones/nonexistent", json={
...
async def test_list_drones_with_status_filter(app):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        await client.post("/api/v1/drones/", json={
            "name": "D1", "model": "M1", "serial_number": "S1",
        })
