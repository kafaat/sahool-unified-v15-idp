...
async def test_weather_check_safe(app):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        resp = await client.post("/api/v1/flights/weather-check", json={
            "lat": 24.7, "lng": 46.7,
            "wind_speed_ms": 3.0,
            "temperature_c": 25.0,
...
async def test_estimate_resources(app):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        resp = await client.post("/api/v1/flights/estimate", json={
            "area_ha": 10.0,
...
async def test_spray_flight_plan_success(app):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        resp = await client.post("/api/v1/flights/plan/spray", json={
            "field_id": "f1",
            "boundary": [
                {"lat": 24.70, "lng": 46.70},
                {"lat": 24.71, "lng": 46.70},
...
async def test_mapping_flight_plan_success(app):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        resp = await client.post("/api/v1/flights/plan/mapping", json={
            "field_id": "f1",
            "boundary": [
                {"lat": 24.70, "lng": 46.70},
                {"lat": 24.71, "lng": 46.70},
