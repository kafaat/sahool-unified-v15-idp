...
export function getBilingualTranslation(
    ...
    return {
        en: defaultEn || errorCode,
        ar: defaultAr || errorCode,
    };
}
 ...
 * - "en"
 */
export function parseAcceptLanguage(acceptLanguageHeader?: string): "en" | "ar" {
    ...
    for (const langItem of acceptLanguageHeader.split(",")) {
        const parts = langItem.trim().split(";");
        ...
        if (parts.length > 1) {
            for (const part of parts.slice(1)) {
                if (part.trim().startsWith("q=")) {
                    const qValue = parseFloat(part.trim().substring(2));
                    if (!isNaN(qValue)) {
                        quality = qValue;
                    }
                    break;
        ...
        languages.push({ lang: baseLang, quality });
    }
