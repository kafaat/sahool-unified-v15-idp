...
class NDVIData(BaseModel):
    ...
    ndvi_value: float = Field(
        ..., ge=-1, le=1, description="قيمة مؤشر NDVI - NDVI value (-1 to 1)"
    )
...
class FieldHealthRequest(BaseModel):
    ...
    sensor_data: SensorData = Field(
        ..., description="بيانات أجهزة الاستشعار - Sensor readings"
    )
    ndvi_data: NDVIData = Field(..., description="بيانات الغطاء النباتي - NDVI data")
    weather_data: WeatherData = Field(
        ..., description="بيانات الطقس - Weather information"
    )
...
class RiskFactor(BaseModel):
    ...
    severity: str = Field(
        ..., description="شدة الخطر - Severity: low, medium, high, critical"
    )
    description_ar: str = Field(
        ..., description="وصف الخطر بالعربية - Description in Arabic"
    )
    description_en: str = Field(
        ..., description="وصف الخطر بالإنجليزية - Description in English"
    )
    impact_score: float = Field(
        ..., ge=0, le=100, description="تأثير الخطر - Impact score"
    )
...
class FieldHealthResponse(BaseModel):
    ...
    ndvi_score: float = Field(
        ..., ge=0, le=100, description="درجة الغطاء النباتي - NDVI score"
    )
    soil_moisture_score: float = Field(
        ..., ge=0, le=100, description="درجة رطوبة التربة - Soil moisture score"
    )
    weather_score: float = Field(
        ..., ge=0, le=100, description="درجة الطقس - Weather score"
    )
    ...
    metadata: dict[str, Any] | None = Field(
        None, description="بيانات إضافية - Additional metadata"
    )
...
def calculate_soil_moisture_score(sensor_data: SensorData, crop_type: str) -> float:
    ...
    optimal_min, optimal_max = optimal_ranges.get(
        crop_type.lower(), optimal_ranges["default"]
    )
...
async def analyze_field_health(request: FieldHealthRequest) -> FieldHealthResponse:
    ...
    try:
        ...
        soil_moisture_score = calculate_soil_moisture_score(
            request.sensor_data, request.crop_type
        )
        ...
        response = FieldHealthResponse(
            ...
            metadata={
                ...
                "critical_risks": len(
                    [r for r in risk_factors if r.severity == "critical"]
                ),
