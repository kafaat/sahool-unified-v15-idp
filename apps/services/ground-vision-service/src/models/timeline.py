...
class CropTimelineEntry(BaseModel):
    ...
    analysis_method: str = Field(
        default="mllm", description="Method used: mllm, cv_model, manual"
    )
    ...
    def __init__(self, **data):
        ...
        if not self.growth_stage_ar:
            self.growth_stage_ar = GROWTH_STAGE_AR.get(
                self.growth_stage, "غير معروف"
            )
...
class CropTimelineAnalysis(BaseModel):
    ...
    operations_detected: list[dict] = Field(
        ...
        description="List of detected agricultural operations"
    )
    ...
    recommendations_ar: list[str] = Field(
        default_factory=list, description="الإجراءات الموصى بها"
    )
...
class FieldContext(BaseModel):
    ...
    rotation_history: list[dict] = Field(
        default_factory=list,
        description="Previous crop rotations"
    )
