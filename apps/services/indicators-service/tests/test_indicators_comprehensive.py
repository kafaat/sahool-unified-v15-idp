...
for _mod in [
    ...
    "shared.middleware.tenant_context", "shared.auth",
    "shared.auth.dependencies", "shared.auth.models",
    "nats", "asyncpg", "structlog",
]:
...
class TestModels:
    def test_indicator_model(self):
        ind = Indicator(
            id="ndvi", name_ar="ن", name_en="NDVI",
            category=IndicatorCategory.VEGETATION,
            value=0.65, unit="index",
            min_value=-1.0, max_value=1.0,
            optimal_min=0.4, optimal_max=0.8,
            trend=TrendDirection.UP, trend_percent=5.2,
            status="optimal", last_updated=datetime.now(UTC),
        )
        assert ind.id == "ndvi"
    ...
    def test_field_indicators_model(self):
            ...
            field_id="f1", field_name="T", area_hectares=10.0,
            crop_type="wheat", indicators=[], overall_score=85.0, alerts=[],
        )
        assert fi.field_id == "f1"

    def test_dashboard_summary_model(self):
        ds = DashboardSummary(
            tenant_id="t1", total_fields=5, total_area_hectares=50.0,
            average_health_score=72.0, indicators_summary={},
            active_alerts=3, critical_alerts=1,
            top_performing_fields=[], attention_needed_fields=[],
    ...
    def test_indicator_alert_model(self):
        alert = IndicatorAlert(
            alert_id="a1", field_id="f1", indicator_id="ndvi",
            indicator_name_ar="ن", severity=AlertSeverity.WARNING,
            message_ar="m", message_en="m",
            current_value=0.3, threshold_value=0.4,
            recommended_action_ar="a", recommended_action_en="a",
    ...
    def test_indicator_input_with_all_fields(self):
        inp = IndicatorInput(
            indicator_type="ndvi", value=0.65,
...
class TestGetTenantIndicators:
    ...
    async def test_with_rows(self):
        ...
        conn.fetch.return_value = [
            {
                "field_id": "f1", "indicator_type": "ndvi",
                "value": json.dumps({"v": 0.7}), "calculated_at": datetime.now(UTC),
            },
        ]
        app.state.db_pool = _make_mock_pool(conn)
...
class TestStoreFieldIndicator:
    ...
    def test_with_trend(self, client):
        conn = AsyncMock()
        app.state.db_pool = _make_mock_pool(conn)
        r = client.post("/v1/field/f1/indicators", json={
            "indicator_type": "soil_moisture", "value": 55.0,
            "trend": "up", "trend_percent": 3.5, "tenant_id": "t1",
        })
        assert r.status_code == 200
