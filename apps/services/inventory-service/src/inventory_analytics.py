...
class InventoryAnalytics:
    ...
    async def get_all_forecasts(
        ...
        if category:
            stmt = stmt.join(ItemCategory).where(
                ItemCategory.code == category
            )
    ...
    async def get_turnover_analysis(
        ...
        for item in items:
            ...
            cogs_stmt = select(
                func.sum(InventoryMovement.total_cost)
            ).where(
                and_(
                    InventoryMovement.tenant_id == self.tenant_id,
            ...
            metrics.append(TurnoverMetrics(
                item_id=str(item.id),
                item_name=item.name_en,
    ...
    async def get_reorder_recommendations(self) -> List[Dict]:
        ...
        stmt = select(InventoryItem, Supplier).outerjoin(
            Supplier,
        ...
        for row in rows:
            ...
            recommendations.append({
                ...
                'urgency': 'critical' if item.available_stock <= 0 else 'high' if forecast and forecast.days_until_stockout <= 7 else 'medium'
            })
    ...
    async def get_abc_analysis(self) -> Dict:
        ...
        return {
            ...
            'a_class': {
                ...
                'percentage_of_value': round(sum(i['value'] for i in a_items) / total_value * 100, 1) if total_value > 0 else 0
            },
            'b_class': {
                'items': b_items,
                'count': len(b_items),
                ...
                'percentage_of_value': round(sum(i['value'] for i in b_items) / total_value * 100, 1) if total_value > 0 else 0
            },
            'c_class': {
                'items': c_items,
                'count': len(c_items),
                ...
                'percentage_of_value': round(sum(i['value'] for i in c_items) / total_value * 100, 1) if total_value > 0 else 0
            }
        }
    ...
    async def get_waste_analysis(
        ...
        expired_items = []
        expired_stmt = select(InventoryItem, ItemCategory).join(
            ItemCategory
        ).where(
            and_(
                InventoryItem.tenant_id == self.tenant_id,
        ...
        return {
            ...
            'by_category': {k: {'quantity': round(v['quantity'], 2), 'value': round(v['value'], 2), 'count': v['count']} for k, v in by_category.items()},
    ...
    async def generate_dashboard_metrics(self) -> Dict:
        ...
        ).group_by(InventoryItem.name_en).order_by(desc('total_consumed')).limit(5)
        ...
        recent_movements_stmt = select(
            ...
            InventoryMovement.tenant_id == self.tenant_id
        ).order_by(desc(InventoryMovement.movement_date)).limit(10)
