  ...
  beforeEach(async () => {
    const mockIotService = {
      ...
      getSensorReading: jest.fn().mockImplementation((fieldId, sensorType) => {
        return mockSensorReadings.find((r) => r.sensorType === sensorType) || null;
      }),
      togglePump: jest.fn().mockReturnValue({ success: true, message: 'تم تشغيل المضخة' }),
      toggleValve: jest.fn().mockReturnValue({ success: true, message: 'تم فتح الصمام' }),
      setIrrigationSchedule: jest.fn().mockReturnValue({ success: true, message: 'تم تفعيل جدولة الري' }),
      getFieldActuatorStates: jest.fn().mockReturnValue(mockActuatorStates),
      getConnectedDevices: jest.fn().mockReturnValue(mockDevices),
      getDeviceStats: jest.fn().mockReturnValue({ online: 5, offline: 2, error: 1 }),
    };
  ...
  describe('Sensor Data', () => {
    ...
    describe('GET /iot/field/:fieldId/sensor/:sensorType', () => {
      it('should return specific sensor reading', () => {
        const result = controller.getSensorReading('field_001', SensorType.SOIL_MOISTURE);
        expect(result).toBeDefined();
        expect(result?.sensorType).toBe(SensorType.SOIL_MOISTURE);
        expect(service.getSensorReading).toHaveBeenCalledWith('field_001', SensorType.SOIL_MOISTURE);
      });

      it('should return null for non-existent sensor', () => {
        jest.spyOn(service, 'getSensorReading').mockReturnValue(null);
        const result = controller.getSensorReading('field_001', SensorType.PH_LEVEL);
  ...
  describe('Actuator Control', () => {
    describe('POST /iot/field/:fieldId/pump', () => {
      it('should toggle pump ON', () => {
        ...
        expect(service.togglePump).toHaveBeenCalledWith('field_001', 'ON', { duration: undefined });
      });

      it('should toggle pump ON with duration', () => {
        const result = controller.togglePump('field_001', { status: 'ON', duration: 30 });
        expect(result.success).toBe(true);
        expect(service.togglePump).toHaveBeenCalledWith('field_001', 'ON', { duration: 30 });
      });

      it('should toggle pump OFF', () => {
        const result = controller.togglePump('field_001', { status: 'OFF' });
        expect(result.success).toBe(true);
        expect(service.togglePump).toHaveBeenCalledWith('field_001', 'OFF', { duration: undefined });
      });
    });

    describe('POST /iot/field/:fieldId/valve/:valveId', () => {
      it('should toggle valve ON', () => {
        const result = controller.toggleValve('field_001', 'valve_1', { status: 'ON' });
        expect(result.success).toBe(true);
        expect(service.toggleValve).toHaveBeenCalledWith('field_001', 'valve_1', 'ON');
      });

      it('should toggle valve OFF', () => {
        const result = controller.toggleValve('field_001', 'valve_1', { status: 'OFF' });
        expect(result.success).toBe(true);
        expect(service.toggleValve).toHaveBeenCalledWith('field_001', 'valve_1', 'OFF');
      });
    });
    ...
    describe('POST /iot/field/:fieldId/irrigation/schedule', () => {
      it('should set irrigation schedule', () => {
        ...
        expect(result.success).toBe(true);
        expect(service.setIrrigationSchedule).toHaveBeenCalledWith('field_001', schedule);
      });
  ...
  describe('Actuator States', () => {
    describe('GET /iot/field/:fieldId/actuators', () => {
      it('should return actuator states for a field', () => {
        ...
        expect(service.getFieldActuatorStates).toHaveBeenCalledWith('field_001');
      });
    });
  });
