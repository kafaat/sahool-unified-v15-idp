...
describe("IotService - MQTT Message Processing", () => {
  ...
  describe("MQTT Connection", () => {
    ...
    it("should handle reconnection attempts", () => {
      ...
      expect(loggerSpy).toHaveBeenCalledWith("Reconnecting to MQTT broker...");
    });
  ...
  describe("Topic Pattern Parsing", () => {
    it("should parse tenant ID from topic", async () => {
      ...
      const topic =
        "sahool/tenant-123/farm/farm-1/field/field-001/sensor/soil_moisture";
      const message = Buffer.from(JSON.stringify({ value: 65 }));
    ...
    it("should parse farm ID from topic", async () => {
      ...
      const topic =
        "sahool/tenant-1/farm/farm-456/field/field-001/sensor/soil_moisture";
      const message = Buffer.from(JSON.stringify({ value: 65 }));
    ...
    it("should parse field ID from topic", async () => {
      ...
      const topic =
        "sahool/tenant-1/farm/farm-1/field/field-789/sensor/soil_moisture";
      const message = Buffer.from(JSON.stringify({ value: 65 }));
