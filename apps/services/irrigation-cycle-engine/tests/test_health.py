...
class TestIrrigationSchedule:
    def test_schedule_basic(self, client):
        ...
        response = client.post("/api/v1/irrigation/schedule", json={
            "crop": "wheat",
...
class TestSalinityAssessment:
    ...
    def test_salinity_assessment_with_sar(self, client):
        ...
        response = client.post(
            "/api/v1/irrigation/salinity-assessment",
            params={
                "ec_water": 2.0, "crop": "wheat", "kc": 1.15,
                "na": 10.0, "ca": 4.0, "mg": 2.0,
            },
        )
        assert response.status_code == 200
        data = response.json()
...
class TestYemenData:
    ...
    def test_list_soils_by_region(self, client):
        ...
        data = response.json()
        assert data["total"] > 0
        assert all("highland" in p["name"].lower() or p["region"] == "highlands"
