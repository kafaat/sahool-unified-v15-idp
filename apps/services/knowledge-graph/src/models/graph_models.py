...
class Crop(BaseModel):
    ...
    growing_season: Optional[str] = Field(None, description="Growing season (spring/summer/fall/winter)")
    family: Optional[str] = Field(None, description="Botanical family")
...
class Disease(BaseModel):
    ...
    description_en: Optional[str] = Field(None, description="English description")
    description_ar: Optional[str] = Field(None, description="Arabic description")
    pathogen_type: Optional[str] = Field(None, description="Type of pathogen (fungal, bacterial, viral, etc.)")
    symptoms_en: Optional[List[str]] = Field(None, description="List of English symptoms")
    symptoms_ar: Optional[List[str]] = Field(None, description="List of Arabic symptoms")
    severity_level: Optional[int] = Field(None, ge=1, le=10, description="Severity level (1-10)")
    incubation_days: Optional[int] = Field(None, description="Incubation period in days")
    attributes: Dict[str, Any] = Field(default_factory=dict, description="Additional attributes")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
...
class Treatment(BaseModel):
    ...
    description_en: Optional[str] = Field(None, description="English description")
    description_ar: Optional[str] = Field(None, description="Arabic description")
    treatment_type: Optional[str] = Field(None, description="Type (fungicide, bactericide, insecticide, etc.)")
    active_ingredient: Optional[str] = Field(None, description="Active ingredient")
    concentration: Optional[str] = Field(None, description="Concentration/dosage (e.g., '0.2%')")
    application_method: Optional[str] = Field(None, description="Application method (spray, drench, dust, etc.)")
    safety_level: Optional[int] = Field(None, ge=1, le=5, description="Safety level (1=very safe, 5=dangerous)")
    environmental_impact: Optional[str] = Field(None, description="Environmental impact")
    cost_per_liter: Optional[float] = Field(None, description="Cost per liter/unit")
    attributes: Dict[str, Any] = Field(default_factory=dict, description="Additional attributes")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
...
class Relationship(BaseModel):
    ...
    evidence: Optional[List[str]] = Field(None, description="List of evidence references")
    attributes: Dict[str, Any] = Field(default_factory=dict, description="Additional attributes")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
