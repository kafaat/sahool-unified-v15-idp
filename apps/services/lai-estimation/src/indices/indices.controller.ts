...
export class VegetationIndicesController {
  ...
  @ApiBody({
    ...
    schema: {
      type: 'object',
      properties: {
        green: { type: 'number', example: 0.08, description: 'Green band (0-1)' },
        red: { type: 'number', example: 0.05, description: 'Red band (0-1)' },
        redEdge: { type: 'number', example: 0.15, description: 'Red Edge band (0-1)' },
        nir: { type: 'number', example: 0.45, description: 'NIR band (0-1)' },
      },
      required: ['green', 'red', 'redEdge', 'nir'],
    },
  })
  @ApiResponse({ status: 200, description: 'All vegetation indices calculated' })
