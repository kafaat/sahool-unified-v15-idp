...
export class CreditService {
  ...
  async calculateCreditScore(userId: string, farmData: FarmData) {
    ...
    if (totalPayments > 0) {
      ...
      else if (onTimeRatio >= 0.7) score += 50;
      else if (onTimeRatio >= 0.5) score += 25;
      else score -= 50;
    }
    ...
    return {
      ...
      scoreBreakdown: {
        ...
        riskScore:
          farmData.diseaseRisk === "Low"
            ? 100
            : farmData.diseaseRisk === "Medium"
              ? 50
              : 0,
  ...
  async getCreditFactors(userId: string): Promise<CreditFactors> {
    ...
    const factors: CreditFactors = {
      ...
      irrigationType: "drip",
      yieldScore: 80,
      paymentHistory: Math.min(
        100,
        (paidLoans / Math.max(totalLoans, 1)) * 100,
      ),
      cropDiversity: 3,
  ...
  private generateRecommendations(
    ...
    if (factors.cropDiversity < 3) {
      recommendations.push({
        action:
          "زد من تنوع المحاصيل (حاليًا: " + factors.cropDiversity + " محاصيل)",
        impact: 12,
        priority: "low",
        category: "farming",
      });
    ...
    if (
      factors.irrigationType === "rainfed" ||
      factors.irrigationType === "flood"
    ) {
      recommendations.push({
