...
describe('FintechService (Facade)', () => {
  ...
  describe('Wallet Operations (delegated)', () => {
    it('should delegate getWallet to WalletService', async () => {
      ...
      expect(mockWalletService.getWallet).toHaveBeenCalledWith('user-123', 'farmer');
      expect(result).toEqual(mockResult);
    });

    it('should delegate deposit to WalletService', async () => {
      const mockResult = { wallet: { balance: 1500 }, transaction: { id: 'tx-1' } };
      mockWalletService.deposit.mockResolvedValue(mockResult);

      const result = await service.deposit('wallet-1', 500, 'Test', 'key-1', 'user-1', '127.0.0.1');

      expect(mockWalletService.deposit).toHaveBeenCalledWith(
        'wallet-1', 500, 'Test', 'key-1', 'user-1', '127.0.0.1'
      );
      expect(result).toEqual(mockResult);
    });

    it('should delegate withdraw to WalletService', async () => {
      const mockResult = { wallet: { balance: 500 }, transaction: { id: 'tx-1' } };
      ...
      expect(mockWalletService.withdraw).toHaveBeenCalledWith(
        'wallet-1', 500, undefined, undefined, undefined, undefined
      );
      expect(result).toEqual(mockResult);
    });
    ...
    it('should delegate getTransactions to WalletService', async () => {
      ...
      expect(mockWalletService.getTransactions).toHaveBeenCalledWith('wallet-1', 10);
      expect(result).toEqual(mockResult);
    });
    ...
    it('should delegate getWalletDashboard to WalletService', async () => {
      ...
      expect(mockWalletService.getWalletDashboard).toHaveBeenCalledWith('wallet-1');
      expect(result).toEqual(mockResult);
    });
  });
  ...
  describe('Credit Operations (delegated)', () => {
    it('should delegate calculateCreditScore to CreditService', async () => {
      ...
      const result = await service.calculateCreditScore('user-123', farmData as any);

      expect(mockCreditService.calculateCreditScore).toHaveBeenCalledWith('user-123', farmData);
      expect(result).toEqual(mockResult);
    });
    ...
    it('should delegate getCreditReport to CreditService', async () => {
      ...
      expect(mockCreditService.getCreditReport).toHaveBeenCalledWith('user-123');
      expect(result).toEqual(mockResult);
    });

    it('should delegate recordCreditEvent to CreditService', async () => {
      const eventData = { walletId: 'wallet-1', eventType: 'ORDER_COMPLETED', description: 'Test' };
      ...
      expect(mockCreditService.recordCreditEvent).toHaveBeenCalledWith(eventData);
      expect(result).toEqual(mockResult);
    });
  });
  ...
  describe('Loan Operations (delegated)', () => {
    it('should delegate requestLoan to LoanService', async () => {
      const loanData = { walletId: 'wallet-1', amount: 10000, termMonths: 12, purpose: 'SEEDS' };
    ...
    it('should delegate repayLoan to LoanService', async () => {
      ...
      const result = await service.repayLoan('loan-1', 5000, 'key-1', 'user-1', '127.0.0.1');

      expect(mockLoanService.repayLoan).toHaveBeenCalledWith(
        'loan-1', 5000, 'key-1', 'user-1', '127.0.0.1'
      );
      expect(result).toEqual(mockResult);
    });
    ...
    it('should delegate scheduled payment operations to LoanService', async () => {
      ...
      mockLoanService.createScheduledPayment.mockResolvedValue({ scheduledPayment: {} });
      mockLoanService.getScheduledPayments.mockResolvedValue([]);
      mockLoanService.cancelScheduledPayment.mockResolvedValue({ isActive: false });

      await service.createScheduledPayment('wallet-1', 1000, 'MONTHLY', nextDate);
      ...
      expect(mockLoanService.getScheduledPayments).toHaveBeenCalledWith('wallet-1', true);
      expect(mockLoanService.cancelScheduledPayment).toHaveBeenCalledWith('sp-1');
    });
  });
  ...
  describe('Escrow Operations (delegated)', () => {
    it('should delegate createEscrow to EscrowService', async () => {
      ...
      const result = await service.createEscrow(
        'order-1', 'buyer-wallet', 'seller-wallet', 1000, 'Notes', 'key-1', 'user-1', '127.0.0.1'
      );
    ...
    it('should delegate releaseEscrow to EscrowService', async () => {
      ...
      const result = await service.releaseEscrow('escrow-1', 'Notes', 'key-1', 'user-1', '127.0.0.1');

      expect(mockEscrowService.releaseEscrow).toHaveBeenCalledWith(
        'escrow-1', 'Notes', 'key-1', 'user-1', '127.0.0.1'
      );
      expect(result).toEqual(mockResult);
    });
    ...
    it('should delegate refundEscrow to EscrowService', async () => {
      ...
      expect(mockEscrowService.refundEscrow).toHaveBeenCalledWith(
        'escrow-1', 'Cancelled', undefined, undefined, undefined
      );
      expect(result).toEqual(mockResult);
    });
    ...
    it('should delegate getWalletEscrows to EscrowService', async () => {
      ...
      expect(mockEscrowService.getWalletEscrows).toHaveBeenCalledWith('wallet-1');
      expect(result).toEqual(mockResult);
    });
  });
