"""
Unit Tests for NDVI Processor Service
اختبارات وحدة خدمة معالج NDVI
...
# Load ndvi_calculation directly to avoid importing the whole tasks package
# (which pulls in PIL, rasterio, etc. that are not needed for NDVI tests).
...
sys.modules[_spec.name] = _ndvi
_spec.loader.exec_module(_ndvi)

HEALTH_THRESHOLDS = _ndvi.HEALTH_THRESHOLDS
NDVI_THRESHOLDS = _ndvi.NDVI_THRESHOLDS
NDVIAlert = _ndvi.NDVIAlert
NDVIStatistics = _ndvi.NDVIStatistics
ZoneClassification = _ndvi.ZoneClassification
calculate_health_score = _ndvi.calculate_health_score
calculate_ndvi_from_bands = _ndvi.calculate_ndvi_from_bands
calculate_ndvi_simple = _ndvi.calculate_ndvi_simple
calculate_statistics = _ndvi.calculate_statistics
classify_zones = _ndvi.classify_zones
generate_alerts = _ndvi.generate_alerts
get_vegetation_class = _ndvi.get_vegetation_class
handle_ndvi_calculation = _ndvi.handle_ndvi_calculation
simulate_band_data = _ndvi.simulate_band_data
...
class TestHandleNDVICalculation:
    ...
    def test_handle_ndvi_calculation_missing_fields(self):
        ...
        payload = {"image_url": "s3://test/image.tif"}
