...
async def get_farmer_notifications(
    ...
    # Security: Authenticated user can only access their own notifications
...
async def mark_notification_read(
    ...
    """تحديد إشعار كمقروء"""
    try:
        # Security: Always use the authenticated user ID to prevent cross-user access
        # الأمان: استخدام معرف المستخدم المصادق دائماً لمنع الوصول العابر للمستخدمين
...
async def update_preferences(
    ...
    # Security: Authenticated user can only update their own preferences
    if str(user.id) != farmer_id:
        raise HTTPException(
            status_code=403,
