...
async def send_sms_notification(notification, farmer_id: str):
    ...
    try:
        ...
        if message_sid:
            ...
            logger.info(f"✅ SMS sent to ***{farmer_profile.phone[-4:] if farmer_profile.phone else '****'}: {message_sid}")
        else:
...
async def send_email_notification(notification, farmer_id: str):
    ...
    try:
        ...
        if message_id:
            ...
            logger.info(f"✅ Email sent to ***@{farmer_profile.email.split('@')[-1] if farmer_profile.email and '@' in farmer_profile.email else '***'}: {message_id}")
        else:
...
async def send_whatsapp_notification(notification, farmer_id: str):
    ...
    try:
        ...
        logger.info(f"WhatsApp notification placeholder for ***{farmer_profile.phone[-4:] if farmer_profile.phone else '****'}")
