...
class SendOTPRequest(BaseModel):
    ...
    channel: OTPChannel = Field(..., description="Delivery channel: sms, whatsapp, telegram, email | قناة الإرسال")
    purpose: OTPPurpose = Field(
        ...,
        description="OTP purpose: login, password_reset, verify_phone, verify_email, two_factor | غرض OTP",
    )
    language: Language = Field(default=Language.ARABIC, description="Preferred language: ar, en | اللغة المفضلة")
...
class VerifyOTPRequest(BaseModel):
    ...
    otp_code: str = Field(..., min_length=4, max_length=8, description="OTP code received | رمز التحقق")
    ...
    class Config:
        json_schema_extra = {"example": {"identifier": "+967771234567", "otp_code": "123456", "purpose": "login"}}
...
class VerifyOTPResponse(BaseModel):
    ...
    message_en: str = Field(..., description="English message")
    message_ar: str = Field(..., description="Arabic message")
    token: str | None = Field(None, description="Reset token (only for password_reset purpose) | رمز إعادة التعيين")
...
class OTPStorage:
    ...
    def _init_redis(self):
        ...
        if redis_url:
            try:
                ...
                logger.info("OTP storage using Redis (hashed)")
    ...
    def _verify_otp_hash(self, otp_code: str, stored_hash: str) -> bool:
    ...
    def create_otp(
        ...
        """
        Create a new OTP. If otp_code is provided, use it (already sent).
        ...
        otp_data = {
            "otp_hash": self._hash_otp(otp_code),  # Store hash, never plaintext
            "created_at": now,
        ...
        if self._use_redis and self._redis_client:
            try:
                ...
                # Track resend cooldown in Redis (survives pod restarts)
                self._redis_client.setex(f"{key}:cooldown", self.RESEND_COOLDOWN_SECONDS, "1")
            except Exception as e:
    ...
    def verify_otp(
        ...
        # Verify code against stored hash (timing-safe)
        stored_hash = otp_data.get("otp_hash", "")
        if not stored_hash or not self._verify_otp_hash(otp_code, stored_hash):
            # Increment attempts
    ...
    def can_resend(
        ...
        if self._use_redis and self._redis_client:
            try:
                ttl = self._redis_client.ttl(cooldown_key)
                if ttl > 0:
                    return False, ttl
                return True, 0
            except Exception:
    ...
    def cleanup_expired(self):
        ...
        now = time.time()
        expired_keys = [key for key, data in self._storage.items() if data.get("expires_at", 0) < now]
...
async def send_otp(request: Request, body: SendOTPRequest, tenant_id: str = Depends(get_tenant_id)):
    ...
    # Generate OTP code (but don't store yet — send first)
    otp_code = _otp_storage._generate_otp()

    # Send OTP via channel BEFORE storing (Fix: don't start timer until send succeeds)
    ...
    if not send_success:
        # Channel unavailable — no OTP was stored, user can retry immediately
        raise HTTPException(
            status_code=503,
            detail={
    ...
    # Store OTP only AFTER successful send (timer starts now)
    _, expires_in = _otp_storage.create_otp(
        identifier=body.identifier,
        purpose=body.purpose.value,
        tenant_id=tenant_id,
        otp_code=otp_code,
