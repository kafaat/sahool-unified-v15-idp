...
def mock_db():
    ...
    with patch('src.database.init_db', new=AsyncMock()):
        with patch('src.database.check_db_health', new=AsyncMock(return_value={
            'status': 'healthy',
            'connected': True
        })):
            with patch('src.database.get_db_stats', new=AsyncMock(return_value={
...
class TestNotificationActions:
    ...
    async def test_mark_notification_as_read_invalid_id(self, app_client):
        ...
        response = app_client.patch(
            '/v1/notifications/invalid-id/read',
            params={'farmer_id': 'farmer-123'}
        )
...
class TestNotificationStats:
    ...
    async def test_get_notification_stats(self, app_client):
        ...
        with patch('src.main.get_db_stats', new=AsyncMock(return_value={
            ...
            with patch('src.models.Notification') as mock_model:
