...
class TestHealthEndpoint:
    ...
    async def test_health_check_success(self, async_client):
        ...
        with patch('src.main.check_db_health', new=AsyncMock(return_value={
            'status': 'healthy',
            'connected': True
        })):
...
class TestNotificationCreation:
    ...
    async def test_create_custom_notification(self, async_client, mock_notification_data):
        ...
        with patch('src.main.create_notification', new=AsyncMock(return_value=MagicMock(**mock_notification_data))):
...
class TestNotificationRetrieval:
    ...
    async def test_get_farmer_notifications(self, async_client, mock_notification_data):
        ...
        with patch('src.repository.NotificationRepository.get_by_user', new=AsyncMock(return_value=[mock_notification])):
            with patch('src.repository.NotificationRepository.get_unread_count', new=AsyncMock(return_value=1)):
    ...
    async def test_get_broadcast_notifications(self, async_client, mock_notification_data):
        ...
        with patch('src.repository.NotificationRepository.get_broadcast_notifications',
...
class TestNotificationUpdates:
    ...
    async def test_mark_notification_as_read(self, async_client, mock_notification_data):
        ...
        with patch('src.repository.NotificationRepository.get_by_id',
                   new=AsyncMock(return_value=mock_notification)):
            with patch('src.repository.NotificationRepository.mark_as_read',
                       new=AsyncMock(return_value=True)):
                response = await async_client.patch(
                    f'/v1/notifications/{notification_id}/read',
                    params={'farmer_id': 'farmer-123'}
                )
...
class TestFarmerRegistration:
    ...
    async def test_update_notification_preferences(self, async_client):
        ...
        with patch('src.repository.NotificationPreferenceRepository.create_or_update',
                   new=AsyncMock(return_value=MagicMock())):
            response = await async_client.put('/v1/farmers/farmer-123/preferences', json=preferences)
