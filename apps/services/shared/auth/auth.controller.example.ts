...
export class AuthController {
  ...
  @ApiOperation({
    summary: 'User login',
    description: 'Authenticate user with email and password. Rate limited to 5 attempts per minute.'
  })
  @ApiResponse({
    status: 200,
    description: 'Login successful',
    headers: {
      'X-RateLimit-Limit': { description: 'Request limit per window', schema: { type: 'number' } },
      'X-RateLimit-Remaining': { description: 'Remaining requests', schema: { type: 'number' } },
      'X-RateLimit-Reset': { description: 'Time until reset (seconds)', schema: { type: 'number' } },
    },
  })
  ...
  async login(@Body() loginDto: LoginDto, @Req() request: Request) {
    ...
    console.log(`Login attempt from IP: ${request.ip} for email: ${loginDto.email}`);
  ...
  @ApiOperation({
    ...
    description: 'Create a new user account. Rate limited to 10 attempts per minute.'
  })
  @ApiResponse({
    status: 201,
  ...
  async register(@Body() registerDto: RegisterDto, @Req() request: Request) {
    // Log the attempt
    console.log(`Registration attempt from IP: ${request.ip} for email: ${registerDto.email}`);
  ...
  @ApiOperation({
    ...
    description: 'Send password reset email. Rate limited to 3 attempts per minute.'
  })
  @ApiResponse({
    status: 200,
    description: 'Password reset email sent (or message displayed for security)',
    headers: {
      'X-RateLimit-Limit': { description: 'Request limit per window', schema: { type: 'number' } },
      'X-RateLimit-Remaining': { description: 'Remaining requests', schema: { type: 'number' } },
      'X-RateLimit-Reset': { description: 'Time until reset (seconds)', schema: { type: 'number' } },
  ...
  async forgotPassword(@Body() forgotPasswordDto: ForgotPasswordDto, @Req() request: Request) {
    // Log the attempt
    console.log(`Password reset requested from IP: ${request.ip} for email: ${forgotPasswordDto.email}`);
  ...
  @ApiOperation({
    ...
    description: 'Reset password using the token from email. Rate limited to 5 attempts per minute.'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Get a new access token using refresh token. Rate limited to 10 attempts per minute.'
  })
  @ApiResponse({
    status: 200,
