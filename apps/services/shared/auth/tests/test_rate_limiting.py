...
class TestAuthKeyGeneration:
    ...
    def test_get_auth_key_whitespace_handling_in_forwarded_for(
...
class TestCheckLoginLimit:
    ...
    @pytest.mark.asyncio
    async def test_login_limit_tracks_attempts(
        self, auth_rate_limiter, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_login_limit_exceeded_raises_exception(
        self, auth_rate_limiter, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_login_limit_exception_has_retry_after(
        self, auth_rate_limiter, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_login_limit_different_users_independent(
        self, auth_rate_limiter, mock_request
    ):
    ...
    async def test_login_limit_different_ips_independent(self, auth_rate_limiter):
        ...
        for _ in range(2):
            allowed1, _, _, _ = await auth_rate_limiter.check_login_limit(
                request1, username
            )
            allowed2, _, _, _ = await auth_rate_limiter.check_login_limit(
                request2, username
            )
...
class TestCheckPasswordResetLimit:
    ...
    @pytest.mark.asyncio
    async def test_password_reset_limit_allows_first_request(
        self, auth_rate_limiter, mock_request
    ):
        """Test that first password reset request is allowed."""
        allowed, remaining, limit, reset = (
            await auth_rate_limiter.check_password_reset_limit(
                mock_request, "user@example.com"
            )
        )
    ...
    @pytest.mark.asyncio
    async def test_password_reset_limit_tracks_attempts(
        self, auth_rate_limiter, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_password_reset_has_stricter_limits_than_login(
        self, auth_rate_limiter
    ):
...
class TestCheckRegistrationLimit:
    ...
    @pytest.mark.asyncio
    async def test_registration_limit_allows_first_request(
        self, auth_rate_limiter, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_registration_limit_with_email(
        self, auth_rate_limiter, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_registration_limit_tracks_attempts(
        self, auth_rate_limiter, mock_request
    ):
        ...
        for attempt in range(1, 6):
            allowed, remaining, limit, reset = (
                await auth_rate_limiter.check_registration_limit(mock_request, email)
            )
            assert allowed is True
...
class TestCheckTokenRefreshLimit:
    ...
    @pytest.mark.asyncio
    async def test_token_refresh_limit_allows_first_request(
        self, auth_rate_limiter, mock_request
    ):
        """Test that first token refresh is allowed."""
        allowed, remaining, limit, reset = (
            await auth_rate_limiter.check_token_refresh_limit(mock_request, "user123")
        )
    ...
    @pytest.mark.asyncio
    async def test_token_refresh_limit_tracks_attempts(
        self, auth_rate_limiter, mock_request
    ):
    ...
    async def test_token_refresh_limit_different_users_independent(
        ...
        allowed, _, _, _ = await auth_rate_limiter.check_token_refresh_limit(
            mock_request, "user2"
        )
        assert allowed is True
...
class TestRateLimitHeaders:
    ...
    @pytest.mark.asyncio
    async def test_login_limit_exception_includes_headers(
        self, auth_rate_limiter, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_password_reset_exception_includes_headers(
        self, auth_rate_limiter, mock_request
    ):
...
class TestIntegrationAndEdgeCases:
    ...
    async def test_concurrent_requests_same_user(self, auth_rate_limiter, mock_request):
        ...
        tasks = [
            auth_rate_limiter.check_login_limit(mock_request, username)
            for _ in range(3)
        ]
    ...
    async def test_multiple_auth_operations_different_methods(
        ...
        # Login attempts
        login_allowed, _, _, _ = await auth_rate_limiter.check_login_limit(
            mock_request, "user1"
        )
    ...
    @pytest.mark.asyncio
    async def test_rate_limit_config_strictness_hierarchy(
        self, auth_rate_limiter
    ):
