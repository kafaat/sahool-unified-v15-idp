...
// Simple success
return createSuccessResponse(data, "Operation successful", "العملية نجحت");
...
const result = await retryWithBackoff(() => externalApi.call(), {
  maxRetries: 3,
  initialDelay: 1000,
});
```
...
const result = await withTimeout(slowOperation(), 5000, "Operation timeout");
```
