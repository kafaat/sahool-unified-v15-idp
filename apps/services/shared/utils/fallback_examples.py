...
for i in range(5):
    try:
        result = fm.execute_with_fallback(
            "weather", weather_api_call, location="صنعاء - Sana'a"
        )
        print(
            f"  {i+1}. ✅ نجاح - Success: {result['temperature']}°C, {result['condition']}"
        )
...
for field_id in ["F001", "F002", "F003"]:
    ...
    print(
        f"  {field_id}: NDVI={result['ndvi']:.2f}, المصدر - Source={result['source']}"
    )
...
test_fm.register_fallback(
    "test_service", test_fallback, failure_threshold=3, recovery_timeout=2
)
