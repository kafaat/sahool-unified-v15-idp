...
async def record_harvest_event(
    ...
    if nc:
        await nc.publish(
            SAHOOL_TRACEABILITY_HARVEST_RECORDED, json.dumps({"batch_id": batch_id, "tenant_id": tenant_id}).encode()
        )
...
async def record_processing_event(
    ...
    if nc:
        await nc.publish(
            SAHOOL_TRACEABILITY_PROCESSING_RECORDED, json.dumps({"batch_id": batch_id, "tenant_id": tenant_id}).encode()
        )
...
async def record_storage_event(
    ...
    if nc:
        await nc.publish(
            SAHOOL_TRACEABILITY_STORAGE_RECORDED, json.dumps({"batch_id": batch_id, "tenant_id": tenant_id}).encode()
        )
...
async def record_transport_event(
    ...
    if nc:
        await nc.publish(
            SAHOOL_TRACEABILITY_TRANSPORT_RECORDED, json.dumps({"batch_id": batch_id, "tenant_id": tenant_id}).encode()
        )
...
async def get_product_journey(
    ...
    batch = await pool.fetchrow(
        "SELECT * FROM produce_batches WHERE batch_code = $1 AND tenant_id = $2", batch_code, uuid.UUID(tenant_id)
    )
    if not batch:
