...
-- Idempotent: all statements use IF NOT EXISTS / exception-safe blocks
-- ═══════════════════════════════════════════════════════════════════════════════

-- Step 1: Create enums (safe: no-op if already exist)
DO $$ BEGIN
...
-- Step 8: Create unique constraints (safe: no-op if already exist)
...
-- Step 15: Add foreign key constraints (safe: no-op if already exist)
DO $$ BEGIN
...
DO $$ BEGIN
  ALTER TABLE "_UserAssignedRoles" ADD CONSTRAINT "_UserAssignedRoles_A_fkey"
    FOREIGN KEY ("A") REFERENCES "users"("id") ON DELETE CASCADE ON UPDATE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE "_UserAssignedRoles" ADD CONSTRAINT "_UserAssignedRoles_B_fkey"
