...
class NDVITimeSeriesAnalyzer:
    ...
    def calculate_trend(self, ndvi_values: list[float], dates: list[date]) -> TrendResult:
        """
    ...
    def identify_peak_greenness(self, ndvi_series: list[NDVIPoint]) -> tuple[date, float] | None:
        """
                    ...
                    if cloudy_mask[i, j]:
                        ...
                        for di in range(-half_k, half_k + 1):
                            for dj in range(-half_k, half_k + 1):
                                ni, nj = i + di, j + dj
                                if 0 <= ni < height and 0 <= nj < width and clear_mask[ni, nj]:
