...
def client():
    ...
    def get_historical(lat: float, lon: float, from_date: str, to_date: str):
        return {
            "location": {"lat": lat, "lon": lon},
            "data": [{"date": "2025-12-20", "temp_avg": 25, "precipitation_mm": 0}],
        }
...
class TestHistorical:
    def test_get_historical(self, client):
        response = client.get(
            "/api/v1/historical?lat=15.35&lon=44.20&from_date=2025-12-01&to_date=2025-12-20"
        )
        assert response.status_code == 200
