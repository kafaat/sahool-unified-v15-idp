...
class SatinBowerbirdOptimizer:
    ...
    def optimize(
        ...
        if self.verbose:
            logger.info(f"SBO starting: population={self.population_size}, iterations={self.max_iterations}")
        ...
        for iteration in range(self.max_iterations):
            ...
            if self.verbose and (iteration + 1) % 10 == 0:
                logger.info(f"Iteration {iteration + 1}/{self.max_iterations}: best={best_fitness:.4f}")
        ...
        if self.verbose:
            logger.info(f"SBO completed in {execution_time:.2f}s")
            logger.info(f"Best parameters: {best_params}")
            logger.info(f"Best score: {best_fitness:.4f}")
