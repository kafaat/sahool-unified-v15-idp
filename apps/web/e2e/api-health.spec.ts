...
test.describe("API Integration Verification", () => {
  ...
  for (const svc of serviceApiTests) {
    test(`${svc.label} page (${svc.route}) triggers API call`, async ({
      ...
      // Wait for data fetching to trigger
      await page.waitForTimeout(3000);

      // The page should have made at least one API call
      // (auth/me is always called, plus service-specific calls)
      ...
      if (!serviceApiCalled) {
        ...
        console.log(
          `[${svc.label}] Expected pattern: ${svc.expectApi}`,
        );
      }

      // At minimum, auth API should be called
      const authCalled = apiCalls.some((url) => /auth/i.test(url));
      expect(authCalled || serviceApiCalled).toBeTruthy();
    });
  }
});
