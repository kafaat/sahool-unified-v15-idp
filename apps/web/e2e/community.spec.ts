...
test.describe("Community Page", () => {
  ...
  test.describe("Page Load and Basic Structure", () => {
    ...
    test("should display create post button", async ({ page }) => {
      ...
      const createPostButton = page.locator(
        'button:has-text("انشر سؤالاً أو تجربة")',
      );
  ...
  test.describe("Posts/Discussions Display", () => {
    test("should display posts in feed", async ({ page }) => {
      ...
      if (count === 0) {
        const emptyState = page.locator(
          "text=/لا توجد منشورات|No posts found/i",
        );
    ...
    test("should display post type badges", async ({ page }) => {
      ...
      if (count > 0) {
        ...
        const typeBadge = firstPost
          .locator("span.rounded-full.text-xs")
          .filter({
            hasText: /سؤال|نصيحة|تجربة|نقاش|تحديث/i,
          });

        const hasBadge = await typeBadge
    ...
    test("should display user badges if present", async ({ page }) => {
      ...
      if (count > 0) {
        ...
        const verifiedBadges = page.locator(
          "svg.text-blue-500, svg.text-yellow-500",
        );
  ...
  test.describe("User Interactions - Like", () => {
    ...
    test("should toggle like on post when clicked", async ({ page }) => {
      ...
      if (count > 0) {
        ...
        console.log(
          `Like button clicked - classes changed: ${initialClass !== newClass}`,
        );
      }
    });
    ...
    test("should show liked state with different styling", async ({ page }) => {
      ...
      if (count > 0) {
        ...
        const isLiked =
          buttonClass?.includes("bg-green-50") ||
          buttonClass?.includes("text-green-600");

        console.log(`Like button shows liked state: ${isLiked}`);
      }
    });

    test("should disable like button while request is pending", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
      ...
      if (count > 0) {
        ...
        console.log(
          `Like button disabled state works: ${isDisabled !== undefined}`,
        );
      }
    });
  });
  ...
  test.describe("User Interactions - Comment", () => {
    ...
    test("should toggle comments section when comment button clicked", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
    ...
    test("should display comments count", async ({ page }) => {
      ...
      if (count > 0) {
        ...
        const commentsCount = firstPost.locator("text=/\\d+.*تعليق/i");
        const hasCount = await commentsCount
          .first()
          .isVisible()
          .catch(() => false);
  ...
  test.describe("User Interactions - Share and Save", () => {
    ...
    test("should display save/bookmark button on each post", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
    ...
    test("should toggle save state when bookmark clicked", async ({ page }) => {
      ...
      if (count > 0) {
        ...
        if (hasButton) {
          ...
          console.log(
            `Save button clicked - state changed: ${initialClass !== newClass}`,
          );
        }
      }
    });
  ...
  test.describe("Create Post Functionality", () => {
    test("should open create post modal when button clicked", async ({
      page,
    }) => {
      const createPostButton = page.locator(
        'button:has-text("انشر سؤالاً أو تجربة")',
  ...
  test.describe("Arabic/English Labels", () => {
    ...
    test("should display Arabic filter labels", async ({ page }) => {
      ...
      for (const option of arabicOptions) {
        const optionElement = typeFilter.locator(
  ...
  test.describe("Error Handling and Edge Cases", () => {
    ...
    test("should maintain scroll position during interactions", async ({
      ...
      console.log(
        `Scroll position maintained during filtering: ${initialScroll}px`,
      );
    });
  ...
  test.describe("Accessibility and Usability", () => {
    test("should have focusable interactive elements", async ({ page }) => {
      ...
      const isFocused = await createPostButton.evaluate(
    ...
    test("should display all content without horizontal scroll", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
