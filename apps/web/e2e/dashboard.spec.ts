...
test.describe('Dashboard Page', () => {
  ...
  test.describe('Recent Activity Widget', () => {
    test('should display recent activity section', async ({ page }) => {
      ...
      const activityHeading = page.locator('text=/النشاط الأخير|Recent Activity/i');
      const isVisible = await activityHeading.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    test('should display activity items', async ({ page }) => {
      ...
      const activitySection = page.locator('text=/النشاط الأخير|Recent Activity/i');
      const isSectionVisible = await activitySection.isVisible({ timeout: 3000 }).catch(() => false);

      if (isSectionVisible) {
        // Look for activity items (lists, cards, etc.)
        const activityItems = page.locator('ul li, [class*="activity"] [class*="item"]');
  ...
  test.describe('Weather Widget', () => {
    test('should display weather section', async ({ page }) => {
      ...
      const isVisible = await weatherHeading.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    test('should display weather data', async ({ page }) => {
      ...
      const isSectionVisible = await weatherSection.isVisible({ timeout: 3000 }).catch(() => false);

      if (isSectionVisible) {
        // Look for temperature or weather icons
        const weatherIcons = page.locator('[class*="weather"] svg, [data-testid*="weather"]');
  ...
  test.describe('Tasks Summary Widget', () => {
    test('should display tasks summary section', async ({ page }) => {
      const tasksHeading = page.locator('text=/المهام القادمة|Upcoming Tasks|Tasks/i');
    ...
    test('should display upcoming tasks', async ({ page }) => {
      ...
      const tasksSection = page.locator('text=/المهام القادمة|Upcoming Tasks/i');
      const isSectionVisible = await tasksSection.isVisible({ timeout: 3000 }).catch(() => false);

      if (isSectionVisible) {
        // Look for task items
        const taskItems = page.locator('ul li, [class*="task"] [class*="item"]');
  ...
  test.describe('Quick Actions Widget', () => {
    test('should display quick actions section', async ({ page }) => {
      const quickActionsHeading = page.locator('text=/إجراءات سريعة|Quick Actions/i');
      const isVisible = await quickActionsHeading.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    test('should display action buttons', async ({ page }) => {
      ...
      const quickActionsSection = page.locator('text=/إجراءات سريعة|Quick Actions/i');
      const isSectionVisible = await quickActionsSection.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test.describe('Dashboard Interactions', () => {
    ...
    test('should refresh data without page reload', async ({ page }) => {
      ...
      const refreshButton = page.locator('button:has-text("Refresh"), button:has-text("تحديث"), [aria-label*="refresh"]');
  ...
  test.describe('Dashboard Charts and Visualizations', () => {
    ...
    test('should display chart legends', async ({ page }) => {
      ...
      const legends = page.locator('[class*="legend"], [class*="recharts-legend"]');
    ...
    test('should handle chart interactions', async ({ page }) => {
      ...
      if (await chartElements.count() > 0) {
        ...
        await chartElements.first().hover().catch(() => {});
