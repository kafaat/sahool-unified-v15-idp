...
test.describe("Equipment Page", () => {
  ...
  test.describe("Page Loading and Structure", () => {
    ...
    test("should display add equipment button", async ({ page }) => {
      const addButton = page.locator(
        'button:has-text("إضافة معدة"), button:has-text("Add Equipment")',
      );
    ...
    test("should display equipment statistics", async ({ page }) => {
      ...
      const isVisible = await statsCards
      ...
      if (isVisible) {
        ...
        const maintenanceStats = page.locator(
          "text=/قيد الصيانة|Maintenance/i",
        );
    ...
    test("should display maintenance schedule section", async ({ page }) => {
      ...
      const maintenanceSchedule = page.locator(
        "text=/جدول الصيانة|Maintenance Schedule/i",
      );
      const isVisible = await maintenanceSchedule
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
  ...
  test.describe("Equipment List Display", () => {
    test("should display equipment list", async ({ page }) => {
      ...
      const equipmentCards = page.locator(
        '[class*="equipment"], .equipment-card, .bg-white.rounded',
      );
    ...
    test("should display filter options", async ({ page }) => {
      ...
      const isVisible = await filterSection
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
    ...
    test("should filter equipment by type", async ({ page }) => {
      ...
      const tractorFilter = page
        .locator('button:has-text("جرار"), button:has-text("Tractor")')
        .first();
      const isVisible = await tractorFilter
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
    ...
    test("should filter equipment by status", async ({ page }) => {
      ...
      const activeFilter = page
        .locator('button:has-text("نشط"), button:has-text("Active")')
        .first();
      const isVisible = await activeFilter
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
    ...
    test("should search for equipment", async ({ page }) => {
      ...
      const searchInput = page
        .locator('input[placeholder*="بحث"], input[placeholder*="Search"]')
        .first();
      const isVisible = await searchInput
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
      ...
      if (isVisible) {
        ...
        const searchButton = page
          .locator('button:has-text("بحث"), button:has-text("Search")')
          .first();
        if (
          await searchButton
  ...
  test.describe("Equipment Details", () => {
    test("should display equipment cards with details", async ({ page }) => {
      ...
      const equipmentNames = page.locator(
        '[class*="card"] h3, [class*="equipment"] h3',
      );
    ...
    test("should display operating hours if available", async ({ page }) => {
      ...
      const operatingHours = page.locator(
        "text=/ساعات التشغيل|Operating Hours/i",
      );
  ...
  test.describe("Status Indicators", () => {
    test("should display active status indicator", async ({ page }) => {
      ...
      const isVisible = await activeStatus
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
    ...
    test("should display maintenance status indicator", async ({ page }) => {
      ...
      const maintenanceStatus = page
        .locator("text=/صيانة/i, text=/Maintenance/i")
        .first();
      const isVisible = await maintenanceStatus
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
    ...
    test("should display inactive/idle status indicator", async ({ page }) => {
      ...
      const isVisible = await idleStatus
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
    ...
    test("should highlight overdue maintenance in red", async ({ page }) => {
      ...
      const isVisible = await overdueWarning
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
  ...
  test.describe("Add Equipment Form", () => {
    test("should open add equipment form", async ({ page }) => {
      ...
      const formHeading = page.locator(
        "text=/إضافة معدة جديدة|Add New Equipment/i",
      );
    ...
    test("should display all required form fields", async ({ page }) => {
      ...
      const nameEnLabel = page.locator(
        "text=/Name \\(English\\)|English Name/i",
      );
      if (
        await nameEnLabel
          .isVisible({ timeout: timeouts.short })
    ...
    test("should display bilingual labels (Arabic/English)", async ({
      page,
    }) => {
      const addButton = page.locator('button:has-text("إضافة معدة")').first();
      await addButton.click();
      ...
      const arabicLabels = page.locator(
        "text=/الاسم بالعربية|النوع|الحالة|الرقم التسلسلي/i",
      );
      const arabicCount = await arabicLabels.count();
      expect(arabicCount).toBeGreaterThanOrEqual(0);

      // Check for English labels
      const englishLabels = page.locator(
        "text=/Name \\(English\\)|Type|Status|Serial/i",
      );
    ...
    test("should have type dropdown with options", async ({ page }) => {
      ...
      const isVisible = await typeSelect
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
      ...
      if (isVisible) {
        ...
        const tractorOption = page.locator(
          'option:has-text("جرار"), option:has-text("Tractor")',
        );
        const harvesterOption = page.locator(
          'option:has-text("حصادة"), option:has-text("Harvester")',
        );
    ...
    test("should have status dropdown with options", async ({ page }) => {
      ...
      if (count >= 2) {
        ...
        const activeOption = page.locator(
          'option:has-text("نشط"), option:has-text("Active")',
        );
        const maintenanceOption = page.locator(
          'option:has-text("صيانة"), option:has-text("Maintenance")',
        );
    ...
    test("should close form on cancel", async ({ page }) => {
      ...
      const isVisible = await formHeading
        .isVisible({ timeout: timeouts.short })
        .catch(() => false);
  ...
  test.describe("Edit Equipment Form", () => {
    test("should open equipment details when clicking an item", async ({
      page,
    }) => {
      await page.waitForTimeout(timeouts.medium);
      ...
      const firstEquipment = page
        .locator('[class*="card"], a[href*="/equipment/"]')
        .first();
      const isVisible = await firstEquipment
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
    ...
    test("should display back button in details view", async ({ page }) => {
      ...
      if (isVisible) {
        ...
        const backButton = page.locator(
          'button:has-text("العودة"), button:has-text("Back"), text=/العودة للقائمة/i',
        );
  ...
  test.describe("Loading States", () => {
    ...
    test("should display loading text in Arabic", async ({ page }) => {
      ...
      const hasText = await loadingText
        .isVisible({ timeout: timeouts.short })
        .catch(() => false);
  ...
  test.describe("Responsive Design", () => {
    ...
    test("should stack statistics on mobile", async ({ page }) => {
      ...
      const stats = page
        .locator("text=/إجمالي المعدات|Total Equipment/i")
        .first();
    ...
    test("should show/hide maintenance schedule based on viewport", async ({
      page,
    }) => {
      // Desktop - should show
      await page.setViewportSize({ width: 1920, height: 1080 });
      ...
      const maintenanceDesktop = page.locator(
        "text=/جدول الصيانة|Maintenance Schedule/i",
      );
      const visibleDesktop = await maintenanceDesktop
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
      ...
      const maintenanceMobile = page.locator(
        "text=/جدول الصيانة|Maintenance Schedule/i",
      );
      const visibleMobile = await maintenanceMobile
        .isVisible({ timeout: timeouts.medium })
        .catch(() => false);
  ...
  test.describe("Arabic/English Labels", () => {
    ...
    test("should display form labels in both languages", async ({ page }) => {
      ...
      const hasArabic = await arabicLabel
        .isVisible({ timeout: timeouts.medium })
        .catch(() => false);
      ...
      const hasEnglish = await englishLabel
        .isVisible({ timeout: timeouts.medium })
        .catch(() => false);
    ...
    test("should display maintenance schedule labels in Arabic", async ({
      page,
    }) => {
      await page.waitForTimeout(timeouts.medium);

      const maintenanceLabel = page.locator("text=/جدول الصيانة/i");
      const isVisible = await maintenanceLabel
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
  ...
  test.describe("Error Handling", () => {
    test("should handle empty equipment list gracefully", async ({ page }) => {
      ...
      const emptyMessage = page.locator(
        "text=/لا توجد معدات|No equipment|Empty/i",
      );
    ...
    test("should display error message on load failure", async ({ page }) => {
      ...
      const errorMessage = page.locator(
        'text=/خطأ|Error|Failed/i, [role="alert"]',
      );
  ...
  test.describe("Maintenance Warnings", () => {
    test("should display maintenance due count", async ({ page }) => {
      ...
      const maintenanceDue = page.locator(
        "text=/بحاجة لصيانة|Maintenance Due/i",
      );
      const isVisible = await maintenanceDue
        .isVisible({ timeout: timeouts.long })
        .catch(() => false);
    ...
    test("should show next maintenance date", async ({ page }) => {
      ...
      const maintenanceDate = page.locator(
        "text=/الصيانة القادمة|Next Maintenance/i",
      );
  ...
  test.describe("Navigation", () => {
    test("should navigate between list and details", async ({ page }) => {
      ...
      if (isVisible) {
        ...
        // Look for back button
        const backButton = page.locator(
          'button:has-text("العودة"), text=/العودة للقائمة/i',
        );
