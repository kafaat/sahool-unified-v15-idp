...
test.describe('IoT & Sensors Page', () => {
  ...
  test.describe('Sensors Dashboard', () => {
    ...
    test('should display sensor names and types', async ({ page }) => {
      ...
      const sensorTypes = page.locator('text=/soil|temperature|humidity|الرطوبة|درجة الحرارة/i');
    ...
    test('should display sensor status indicators', async ({ page }) => {
      ...
      const statusBadges = page.locator('text=/online|offline|active|متصل|غير متصل|نشط/i');
    ...
    test('should allow clicking on a sensor', async ({ page }) => {
      ...
      const sensorCard = page.locator('[class*="cursor-pointer"], button').filter({
        has: page.locator('text=/sensor|مستشعر/i')
      }).first();

      const isClickable = await sensorCard.isVisible({ timeout: 3000 }).catch(() => false);
      ...
      if (isClickable) {
        ...
        const hasReadings = await readingsSection.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test.describe('Sensor Readings Display', () => {
    test('should display sensor readings section when sensor is selected', async ({ page }) => {
      await page.waitForTimeout(3000);
      ...
      if (hasClickable) {
        ...
        const isVisible = await readingsHeading.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    test('should display readings table', async ({ page }) => {
      ...
      if (hasClickable) {
        ...
        const hasList = await readingsTable.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    test('should display chart statistics', async ({ page }) => {
      ...
      if (hasClickable) {
        ...
        const statsLabels = page.locator('text=/minimum|maximum|average|min|max|avg|أدنى|أعلى|متوسط/i');
    ...
    test('should handle empty readings gracefully', async ({ page }) => {
      ...
      const noReadingsMsg = page.locator('text=/لا توجد قراءات|No readings available|No data/i');
      const hasNoReadings = await noReadingsMsg.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test.describe('Device Status Indicators', () => {
    ...
    test('should show offline status with red/gray indicator', async ({ page }) => {
      await page.waitForTimeout(3000);

      // Look for offline status with red/gray color
      const offlineStatus = page.locator('[class*="red"], [class*="gray"]').filter({
    ...
    test('should show error status with warning indicator', async ({ page }) => {
      await page.waitForTimeout(3000);

      // Look for error/warning status
      const errorStatus = page.locator('[class*="red"], [class*="orange"]').filter({
    ...
    test('should display actuator status toggles', async ({ page }) => {
      ...
      const isVisible = await actuatorsSection.isVisible({ timeout: 3000 }).catch(() => false);
      ...
      if (isVisible) {
        ...
        const actuatorStatus = page.locator('text=/on|off|auto|تشغيل|إيقاف|تلقائي/i');
  ...
  test.describe('Real-time Data Updates', () => {
    test('should handle data refresh', async ({ page }) => {
      ...
      const initialCount = await page.locator('h3').filter({ hasText: /^\d+$/ }).first().textContent();
    ...
    test('should update readings when sensor is selected', async ({ page }) => {
      ...
      if (hasClickable) {
        ...
        const readingsSection = page.locator('text=/قراءات|Readings/i');
        const hasReadings = await readingsSection.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    test('should handle websocket connection if available', async ({ page }) => {
      await page.waitForTimeout(3000);
  ...
  test.describe('Actuator Controls', () => {
    ...
    test('should display actuator list', async ({ page }) => {
      ...
      const actuatorItems = page.locator('[class*="actuator"], [data-testid*="actuator"]');
      ...
      if (count === 0) {
        ...
        const noActuatorsMsg = page.locator('text=/لا توجد مشغلات|No actuators/i');
        const hasMsg = await noActuatorsMsg.isVisible({ timeout: 2000 }).catch(() => false);
    ...
    test('should display actuator status', async ({ page }) => {
      ...
      if (isVisible) {
        ...
        const statusIndicators = page.locator('text=/on|off|auto|تشغيل|إيقاف|تلقائي/i');
    ...
    test('should show actuator types', async ({ page }) => {
      ...
      const actuatorTypes = page.locator('text=/valve|pump|fan|heater|صمام|مضخة|مروحة/i');
  ...
  test.describe('Alert Rules', () => {
    ...
    test('should display alert rules list', async ({ page }) => {
      ...
      if (count === 0) {
        ...
        const noAlertsMsg = page.locator('text=/لا توجد قواعد|No rules|No alerts/i');
        const hasMsg = await noAlertsMsg.isVisible({ timeout: 2000 }).catch(() => false);
    ...
    test('should display alert severity levels', async ({ page }) => {
      ...
      const severityLabels = page.locator('text=/info|warning|critical|معلومات|تحذير|حرج/i');
    ...
    test('should display alert conditions', async ({ page }) => {
      ...
      const conditions = page.locator('text=/above|below|between|أعلى من|أقل من|بين/i');
    ...
    test('should display alert threshold values', async ({ page }) => {
      ...
      const alertSection = page.locator('h2:has-text("قواعد التنبيه")');
      const isVisible = await alertSection.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    test('should show enabled/disabled state for rules', async ({ page }) => {
      ...
      const stateIndicators = page.locator('text=/enabled|disabled|مفعّل|معطّل/i');
  ...
  test.describe('Responsive Design', () => {
    ...
    test('should adapt layout grid on different screens', async ({ page }) => {
      ...
      const isVisible = await mainGrid.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    test('should scroll properly on mobile', async ({ page }) => {
      ...
      const alertSection = page.locator('h2:has-text("قواعد التنبيه")');
      const isVisible = await alertSection.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test.describe('Bilingual Labels', () => {
    ...
    test('should display English statistics labels', async ({ page }) => {
      ...
      const englishLabels = ['Active Sensors', 'Active Actuators', 'Alert Rules'];
    ...
    test('should display bilingual status labels', async ({ page }) => {
      ...
      const statusLabels = page.locator('text=/متصل|مفعّل|online|active|connected/i');
      const count = await statusLabels.count();
    ...
    test('should display Arabic sensor type names', async ({ page }) => {
      ...
      const arabicTypes = page.locator('text=/رطوبة التربة|درجة الحرارة|الرطوبة/i');
    ...
    test('should have proper RTL direction', async ({ page }) => {
      ...
      const isVisible = await mainContainer.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test.describe('Error Handling', () => {
    test('should handle no sensors gracefully', async ({ page }) => {
      ...
      const noSensorsMsg = page.locator('text=/لا توجد مستشعرات|No sensors available|No devices/i');
      const hasMsg = await noSensorsMsg.isVisible({ timeout: 2000 }).catch(() => false);
    ...
    test('should handle no alert rules gracefully', async ({ page }) => {
      ...
      const noRulesMsg = page.locator('text=/لا توجد قواعد|No rules|No alerts/i');
      const hasMsg = await noRulesMsg.isVisible({ timeout: 2000 }).catch(() => false);
    ...
    test('should handle sensor selection errors', async ({ page }) => {
      ...
      const sensorCard = page.locator('[class*="cursor-pointer"]').first();
      const exists = await sensorCard.isVisible({ timeout: 2000 }).catch(() => false);
