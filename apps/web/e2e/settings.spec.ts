...
test.describe("Settings Page", () => {
    ...
    test.skip("should save profile changes", async ({ page }) => {
      ...
      if (await nameInput.isVisible({ timeout: 3000 })) {
        ...
        const saveButton = page
          .locator('button:has-text("Save"), button:has-text("حفظ")')
          .first();
  ...
  test.describe("Password Settings", () => {
    ...
    test("should validate password strength", async ({ page }) => {
      const newPasswordInput = page
        .locator('input[name*="new"], input[placeholder*="New Password"]')
        .first();
      const isVisible = await newPasswordInput
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      ...
      if (isVisible) {
        ...
        const strengthIndicator = page.locator(
          '[class*="strength"], text=/weak|ضعيف/i',
        );
    ...
    test("should require current password to change", async ({ page }) => {
      const currentPasswordInput = page
        .locator(
          'input[name*="current"], input[placeholder*="Current Password"]',
        )
        .first();
      const isVisible = await currentPasswordInput
        .isVisible({ timeout: 3000 })
        .catch(() => false);
  ...
  test.describe("Notification Settings", () => {
    test("should display notification preferences", async ({ page }) => {
      ...
      const notificationHeading = page.locator(
        'h2:has-text("إعدادات الإشعارات")',
      );
    ...
    test("should toggle notification setting", async ({ page }) => {
      ...
      const firstToggle = page
        ...
        .first();
      const isVisible = await firstToggle
        .isVisible({ timeout: 3000 })
        .catch(() => false);
  ...
  test.describe("Language Settings", () => {
    test("should display language options", async ({ page }) => {
      ...
      const isVisible = await languageSection
    ...
    test("should have language selector", async ({ page }) => {
      ...
      const languageSelector = page
        .locator('select[name*="language"], [role="combobox"]')
        .first();
      const isVisible = await languageSelector
        .isVisible({ timeout: 3000 })
        .catch(() => false);
  ...
  test.describe("Theme Settings", () => {
    test("should display theme options", async ({ page }) => {
      const themeSection = page.locator(
        "text=/Theme|المظهر|Dark Mode|الوضع الداكن/i",
      );
      const isVisible = await themeSection
        .isVisible({ timeout: 5000 })
        .catch(() => false);
    ...
    test("should have theme toggle", async ({ page }) => {
      ...
      const themeToggle = page
        .locator(
          'input[type="checkbox"][name*="theme"], input[type="checkbox"][name*="dark"], [role="switch"]',
        )
        .first();
    ...
    test.skip("should toggle dark mode", async ({ page }) => {
      const darkModeToggle = page
        .locator('input[name*="dark"], [aria-label*="dark"]')
        .first();
  ...
  test.describe("Privacy Settings", () => {
    test("should display privacy options", async ({ page }) => {
      const privacySection = page.locator(
        "text=/Privacy|الخصوصية|Security|الأمان/i",
      );
      const isVisible = await privacySection
        .isVisible({ timeout: 5000 })
        .catch(() => false);
  ...
  test.describe("Account Management", () => {
    test("should display account actions", async ({ page }) => {
      ...
      const dangerActions = page.locator(
        'button:has-text("Delete"), button:has-text("حذف"), button:has-text("Deactivate")',
      );
    ...
    test("should show confirmation before dangerous actions", async ({
      page,
    }) => {
      await page.waitForTimeout(1000);
      ...
      const deleteButton = page
        .locator(
          'button:has-text("Delete Account"), button:has-text("حذف الحساب")',
        )
        .first();
  ...
  test.describe("Settings Persistence", () => {
    test("should persist settings after page reload", async ({ page }) => {
      ...
      const isVisible = await firstCheckbox
        .isVisible({ timeout: 3000 })
        .catch(() => false);
