...
test.describe("Wallet Page", () => {
  ...
  test.describe("Page Load and Display", () => {
    test("should display wallet page correctly", async ({ page }) => {
      ...
      const heading = page.locator(
        'h1:has-text("المحفظة"), h1:has-text("Wallet")',
      );
  ...
  test.describe("Balance Display", () => {
    ...
    test("should display balance amount with currency", async ({ page }) => {
      ...
      const balanceAmount = page
        .locator("text=/\\d+\\.\\d+\\s+(SAR|ريال|SR)/i")
        .first();
    ...
    test("should display pending balance if exists", async ({ page }) => {
      ...
      const hasPending = await pendingBalance
    ...
    test("should display balance in gradient card", async ({ page }) => {
      ...
      const balanceCard = page
        .locator('.bg-gradient-to-br, [class*="gradient"]')
        .first();
      const hasGradient = await balanceCard
        .isVisible({ timeout: 3000 })
        .catch(() => false);

      console.log(
        `Gradient balance card: ${hasGradient ? "found" : "not found"}`,
      );
    });
  });
  ...
  test.describe("Statistics Cards", () => {
    test("should display statistics cards", async ({ page }) => {
      ...
      const hasIncome = await incomeLabel
        .isVisible({ timeout: 5000 })
        .catch(() => false);

      // Look for expenses label
      const expensesLabel = page.locator(
        "text=/إجمالي المصروفات|Total Expenses/i",
      );
      const hasExpenses = await expensesLabel
        .isVisible({ timeout: 5000 })
        .catch(() => false);

      // Look for transactions count
      const transactionsLabel = page.locator(
        "text=/عدد المعاملات|Transactions/i",
      );
      const hasTransactions = await transactionsLabel
        .isVisible({ timeout: 5000 })
        .catch(() => false);

      console.log(
        `Statistics cards - Income: ${hasIncome}, Expenses: ${hasExpenses}, Transactions: ${hasTransactions}`,
      );
    ...
    test("should display monthly income statistics", async ({ page }) => {
      ...
      const monthlyIncomeLabel = page.locator(
        "text=/إجمالي الإيداعات|Total Income/i",
      );
    ...
    test("should display monthly expenses statistics", async ({ page }) => {
      ...
      const monthlyExpensesLabel = page.locator(
        "text=/إجمالي المصروفات|Total Expenses/i",
      );
    ...
    test("should display transaction count", async ({ page }) => {
      ...
      const transactionCountLabel = page.locator(
        "text=/عدد المعاملات|Transactions/i",
      );
    ...
    test("should display statistics with appropriate icons", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
  ...
  test.describe("Wallet Information Section", () => {
    test("should display wallet info section", async ({ page }) => {
      ...
      const walletInfoHeading = page.locator(
        "text=/معلومات المحفظة|Wallet Info/i",
      );
      const hasInfo = await walletInfoHeading
        .isVisible({ timeout: 5000 })
        .catch(() => false);
    ...
    test("should display wallet ID", async ({ page }) => {
      ...
      const hasWalletId = await walletIdLabel
        .isVisible({ timeout: 3000 })
        .catch(() => false);
    ...
    test("should display total deposits and withdrawals", async ({ page }) => {
      ...
      const hasDeposits = await depositsLabel
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      const hasWithdrawals = await withdrawalsLabel
        .isVisible({ timeout: 3000 })
        .catch(() => false);

      console.log(
        `Total deposits visible: ${hasDeposits}, Total withdrawals visible: ${hasWithdrawals}`,
      );
    });
    ...
    test("should display last transaction date", async ({ page }) => {
      ...
      const hasLastTransaction = await lastTransactionLabel
        .isVisible({ timeout: 3000 })
        .catch(() => false);
  ...
  test.describe("Quick Action Buttons", () => {
    test("should display deposit button", async ({ page }) => {
      ...
      const depositButton = page.locator(
        'button:has-text("إيداع"), button:has-text("Deposit")',
      );
    ...
    test("should display withdraw button", async ({ page }) => {
      ...
      const withdrawButton = page.locator(
        'button:has-text("سحب"), button:has-text("Withdraw")',
      );
    ...
    test("should display transfer button", async ({ page }) => {
      ...
      const transferButton = page.locator(
        'button:has-text("تحويل"), button:has-text("Transfer")',
      );
    ...
    test("should show alert when clicking withdraw button", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
    ...
    test("should navigate to transfer form when clicking transfer", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
  ...
  test.describe("Transaction History", () => {
    test("should display transaction history section", async ({ page }) => {
      ...
      const historyHeading = page.locator(
        'h2:has-text("سجل المعاملات"), h2:has-text("Transaction History")',
      );
      await expect(historyHeading).toBeVisible({ timeout: 10000 });
    });

    test("should display transaction history with bilingual labels", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);

      // Check for both Arabic and English labels
      const bilingualHeading = page.locator(
        "text=/سجل المعاملات.*Transaction History/i",
      );
    ...
    test("should display filter button", async ({ page }) => {
      ...
      const filterButton = page.locator(
        'button:has-text("فلتر"), button:has-text("Filter")',
      );
      await expect(filterButton.first()).toBeVisible({ timeout: 10000 });
    });

    test("should display transaction items or empty state", async ({
      page,
    }) => {
      ...
      const emptyStateMessage = page.locator(
        "text=/لا توجد معاملات|No transactions/i",
      );
      ...
      console.log(
        `Transactions: ${hasTransactions}, Empty state: ${hasEmptyState}`,
      );
    ...
    test("should display transaction count if transactions exist", async ({
      page,
    }) => {
      await page.waitForTimeout(3000);

      const countLabel = page.locator(
        "text=/عرض.*معاملة|Showing.*transactions/i",
      );
  ...
  test.describe("Transaction Filters", () => {
    test("should toggle filter panel when clicking filter button", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
      ...
      if (await filterButton.isVisible({ timeout: 3000 })) {
        ...
        const filterPanel = page
          .locator('button:has-text("الكل"), button:has-text("All")')
          .first();
        const isPanelVisible = await filterPanel
          .isVisible({ timeout: 3000 })
          .catch(() => false);
    ...
    test("should display all transaction type filters", async ({ page }) => {
      ...
      if (await filterButton.isVisible({ timeout: 3000 })) {
        ...
        const hasAllFilter = await allFilter
          .isVisible({ timeout: 2000 })
          .catch(() => false);
        const hasDepositFilter = await depositFilter
          .isVisible({ timeout: 2000 })
          .catch(() => false);
        const hasWithdrawalFilter = await withdrawalFilter
          .isVisible({ timeout: 2000 })
          .catch(() => false);
        const hasPaymentFilter = await paymentFilter
          .isVisible({ timeout: 2000 })
          .catch(() => false);
        const hasTransferFilter = await transferFilter
          .isVisible({ timeout: 2000 })
          .catch(() => false);

        console.log(
          `Filter options - All: ${hasAllFilter}, Deposit: ${hasDepositFilter}, Withdrawal: ${hasWithdrawalFilter}, Payment: ${hasPaymentFilter}, Transfer: ${hasTransferFilter}`,
        );
    ...
    test("should filter transactions by type - withdrawal", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
    ...
    test("should hide filter panel when clicking filter button again", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
      ...
      if (await filterButton.isVisible({ timeout: 3000 })) {
        ...
        if (isPanelVisible) {
          ...
          const isPanelHidden = !(await filterPanel
            .isVisible({ timeout: 2000 })
            .catch(() => true));
  ...
  test.describe("Transaction Details", () => {
    ...
    test("should display transaction amounts with +/- indicators", async ({
      page,
    }) => {
      await page.waitForTimeout(3000);
    ...
    test("should show bilingual descriptions for transactions", async ({
      page,
    }) => {
      await page.waitForTimeout(3000);
  ...
  test.describe("Loading States", () => {
    test("should show loading skeleton for balance", async ({ page }) => {
      ...
      const stillLoading = await loadingSkeleton
        .isVisible({ timeout: 1000 })
        .catch(() => false);
    ...
    test("should display error state gracefully if data fails to load", async ({
      page,
    }) => {
      await page.waitForTimeout(3000);
  ...
  test.describe("Responsive Design", () => {
    ...
    test("should adjust grid layout on different screen sizes", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
  ...
  test.describe("Arabic/English Labels", () => {
    test("should display all key labels in both Arabic and English", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
      ...
      // Balance label
      const balanceLabel = page.locator("text=/رصيد المحفظة.*Wallet Balance/i");
      const hasBalance = await balanceLabel
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      ...
      // Transaction history
      const historyLabel = page.locator(
        "text=/سجل المعاملات.*Transaction History/i",
      );
    ...
    test("should display Arabic labels for action buttons", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
    ...
    test("should display Arabic labels for statistics", async ({ page }) => {
      ...
      console.log(
        `Arabic stats labels - Income: ${hasIncome}, Expenses: ${hasExpenses}`,
      );
    });
    ...
    test("should display English labels alongside Arabic", async ({ page }) => {
      ...
      for (const label of englishLabels) {
        ...
        const isVisible = await element
          ...
          .catch(() => false);
        console.log(
          `English label "${label}": ${isVisible ? "found" : "not found"}`,
        );
      }
    });
    ...
    test("should display Arabic filter labels", async ({ page }) => {
      ...
      if (await filterButton.first().isVisible({ timeout: 3000 })) {
        ...
        const arabicFilters = ["الكل", "إيداع", "سحب", "دفع", "تحويل"];
        ...
        for (const filter of arabicFilters) {
          ...
          const isVisible = await element
            ...
            .catch(() => false);
          console.log(
            `Arabic filter "${filter}": ${isVisible ? "found" : "not found"}`,
          );
        }
      }
    });
    ...
    test("should display Arabic status labels", async ({ page }) => {
      ...
      const statusLabels = ["مكتمل", "قيد الانتظار", "فشل", "ملغي"];
  ...
  test.describe("Navigation and Interactions", () => {
    test("should navigate back to dashboard view after transfer", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
    ...
    test("should maintain state after navigation away and back", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
    ...
    test("should handle hover states on transaction items", async ({
      page,
    }) => {
      await page.waitForTimeout(3000);
      ...
      const transactionItems = page
        .locator('[class*="rounded-xl"][class*="border-2"][class*="hover"]')
        .first();
  ...
  test.describe("Edge Cases", () => {
    ...
    test("should handle filter with no matching transactions", async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
