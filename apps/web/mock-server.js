  ...
  fields: [
    {
      ...
      boundary: {
        type: 'Polygon',
        coordinates: [[[44.19, 15.37], [44.195, 15.37], [44.195, 15.375], [44.19, 15.375], [44.19, 15.37]]]
      },
    ...
    },
    {
      ...
      boundary: {
        type: 'Polygon',
        coordinates: [[[44.16, 13.96], [44.17, 13.96], [44.17, 13.97], [44.16, 13.97], [44.16, 13.96]]]
      },
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    },
    {
      ...
      boundary: {
        type: 'Polygon',
        coordinates: [[[44.02, 13.58], [44.03, 13.58], [44.03, 13.59], [44.02, 13.59], [44.02, 13.58]]]
      },
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    },
    {
      ...
      boundary: {
        type: 'Polygon',
        coordinates: [[[42.95, 14.80], [42.97, 14.80], [42.97, 14.82], [42.95, 14.82], [42.95, 14.80]]]
      },
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    }
  ...
  sensors: [
    {
      ...
      status: 'online',
      batteryLevel: 85,
      lastReading: { sensorId: 'sensor_001', value: 45, unit: '%', timestamp: new Date().toISOString() }
    },
    {
      id: 'sensor_002',
      fieldId: 'field_001',
      ...
      status: 'online',
      batteryLevel: 92,
      lastReading: { sensorId: 'sensor_002', value: 28.5, unit: '°C', timestamp: new Date().toISOString() }
    }
  ]
};
...
const routes = {
  ...
  // NDVI API
  'GET /api/v1/ndvi/summary/:tenantId': () => ({ success: true, data: mockData.ndviSummary }),
  'GET /api/v1/ndvi/:fieldId': (params) => {
    ...
    return {
      ...
      data: {
        ...
        current: { value: field?.ndviValue || 0.5, category: { name: 'Moderate', nameAr: 'متوسط', color: '#f59e0b' }, date: new Date().toISOString() },
        statistics: { average: field?.ndviValue || 0.5, min: 0.3, max: 0.8, trend: 0.02, trendDirection: 'stable' },
        history: [],
  ...
  // Weather API
  'GET /api/v1/weather/current': () => ({ success: true, data: mockData.weather }),
  'GET /api/v1/weather/forecast': () => ({
    success: true,
    data: {
      location: { lat: 15.37, lng: 44.19 },
      daily: [
        { date: new Date().toISOString(), tempMax: 32, tempMin: 18, humidity: 40, precipitation: 0, precipitationProbability: 5, windSpeed: 10, description: 'مشمس', icon: '01d' },
        { date: new Date(Date.now() + 86400000).toISOString(), tempMax: 34, tempMin: 20, humidity: 35, precipitation: 0, precipitationProbability: 10, windSpeed: 12, description: 'مشمس جزئياً', icon: '02d' },
        { date: new Date(Date.now() + 86400000 * 2).toISOString(), tempMax: 30, tempMin: 19, humidity: 50, precipitation: 2, precipitationProbability: 40, windSpeed: 8, description: 'غائم جزئياً', icon: '03d' }
      ]
    }
  }),
  ...
  'POST /api/v1/advisor/analyze': (params, body) => ({
    ...
    data: {
      recommendations: [
        { type: 'irrigation', priority: 'high', message: 'يُنصح بالري خلال الـ 24 ساعة القادمة', messageAr: 'يُنصح بالري خلال الـ 24 ساعة القادمة' },
        { type: 'fertilizer', priority: 'medium', message: 'أضف سماد NPK بمعدل 50 كجم/هكتار', messageAr: 'أضف سماد NPK بمعدل 50 كجم/هكتار' }
      ]
    }
  }),
