...
function groupByDate<T extends { createdAt: string }>(
  items: T[]
): { label: string; labelAr: string; items: T[] }[] {
  const now = new Date();
...
export default function NotificationsClient() {
  ...
  // Query hooks
  const { data: notifications = [], isLoading, isError, refetch } = useNotifications(apiFilters);
    ...
    <div className="space-y-6">
      ...
      {isError && !isLoading && (
        <div className="bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800 p-6 text-center">
          ...
          <p className="text-red-700 dark:text-red-400 font-medium">فشل في تحميل الإشعارات</p>
