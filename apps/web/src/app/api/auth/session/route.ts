...
const ACCESS_TOKEN_MAX_AGE = parseMaxAge(process.env.JWT_ACCESS_TOKEN_EXPIRE_SECONDS, 1800); // 30 minutes default - aligned with admin app
const REFRESH_TOKEN_MAX_AGE = parseMaxAge(process.env.JWT_REFRESH_TOKEN_EXPIRE_SECONDS, 604800); // 7 days default
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.access_token || !isValidToken(body.access_token)) {
      return NextResponse.json({ success: false, error: 'Invalid access token' }, { status: 400 });
    }
