...
function isWeatherAlertPayload(
  payload: EventPayload,
): payload is WeatherAlertPayload {
  return "type" in payload && "location" in payload;
}

function isDiagnosisPayload(
  payload: EventPayload,
): payload is DiagnosisPayload {
...
function PayloadDetails({
  ...
  if (event.event_type === "ndvi_processed" && isNdviPayload(payload)) {
    ...
    return (
      <span>
        🛰️ NDVI: {payload.ndvi}{" "}
        <span className={`px-1 rounded ${statusClass}`}>{statusLabel}</span>
      </span>
    );
  }
...
const EventCard = React.memo<{ event: TimelineEvent }>(function EventCard({
  event,
}) {
...
export const EventTimeline = React.memo<EventTimelineProps>(
  function EventTimeline({ tenantId }) {
