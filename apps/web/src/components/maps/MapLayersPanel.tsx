...
// Toggle visibility of multiple data layers with opacity control and NDVI legend
...
/** NDVI color scale labels for the legend */
const NDVI_LEGEND_STOPS = [
  { value: '-1.0', color: '#d73027', labelAr: 'تربة / ماء' },
  { value: '0.0', color: '#f46d43', labelAr: 'منخفض جداً' },
  { value: '0.1', color: '#fdae61', labelAr: 'منخفض' },
  { value: '0.2', color: '#fee08b', labelAr: 'متناثر' },
  { value: '0.3', color: '#d9ef8b', labelAr: 'معتدل منخفض' },
  { value: '0.4', color: '#a6d96a', labelAr: 'معتدل' },
  { value: '0.6', color: '#66bd63', labelAr: 'صحي' },
  { value: '0.8', color: '#1a9850', labelAr: 'صحي جداً' },
] as const;
...
/** Detailed NDVI color scale legend */
function NDVIColorScaleLegend() {
  return (
    <div className="mt-2 rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-900">
      <p
        className="mb-2 text-xs font-semibold text-gray-600 dark:text-gray-300"
        dir="rtl"
      >
        مقياس ألوان NDVI
      </p>
      <div className="flex items-center gap-0.5" dir="ltr">
        {NDVI_LEGEND_STOPS.map((stop) => (
          <div key={stop.value} className="flex flex-1 flex-col items-center">
            <div
              className="h-3 w-full rounded-sm"
              style={{ backgroundColor: stop.color }}
              title={`${stop.value}: ${stop.labelAr}`}
            />
            <span className="mt-0.5 text-[8px] leading-tight text-gray-400">
              {stop.value}
            </span>
          </div>
        ))}
      </div>
      <div className="mt-1 flex justify-between text-[9px] text-gray-400" dir="rtl">
        <span>صحي جداً</span>
        <span>تربة / ماء</span>
      </div>
    </div>
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          aria-label={`ضبط شفافية ${layer.nameAr}`}
        >
          {layer.enabled ? (
      ...
      {showOpacity && (
        <div className="mt-2 flex items-center gap-2">
          <span className="text-[10px] text-gray-400 dark:text-gray-500" dir="rtl">
            الشفافية
          </span>
          <input
            type="range"
            ...
            aria-label={`شفافية ${layer.nameAr}`}
          />
...
export default function MapLayersPanel({
  ...
  /** Whether any NDVI-type layer is currently enabled */
  const showNDVILegend = useMemo(
    () => layers.some((l) => l.enabled && l.colorScale === 'ndvi'),
    [layers],
  );
  ...
  return (
    <div className="fixed right-0 top-16 z-30 flex h-[calc(100vh-4rem)] w-72 flex-col border-l border-gray-200 bg-white shadow-xl dark:border-gray-700 dark:bg-gray-800">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
        ...
        {showNDVILegend && <NDVIColorScaleLegend />}
      </div>
      ...
      <div className="border-t border-gray-200 px-4 py-2 dark:border-gray-700">
        <p
          ...
          dir="rtl"
        >
          {layers.length} طبقة متاحة &middot; {enabledCount} نشطة
        </p>
      </div>
    </div>
  );
