...
// Side-by-side NDVI comparison for two dates with 8-color scale and metadata

import { useMemo } from 'react';
import { Calendar, Cloud, Database, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
export interface SplitScreenNDVIProps {
  fieldId: string;
  fieldName: string;
...
function getTrendIcon(delta: number) {
  if (delta > 0.02) return TrendingUp;
  if (delta < -0.02) return TrendingDown;
  return Minus;
}
...
function NDVIPanel({
  ...
  return (
    <div className="flex flex-1 flex-col gap-3 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          ...
          <input
            ...
            className="rounded-lg border border-gray-300 bg-gray-50 px-3 py-1.5 text-sm text-gray-700 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200"
            dir="ltr"
          />
        </div>
        <span
      ...
      {/* Metadata: cloud cover + data source */}
      <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
        <span className="inline-flex items-center gap-1" dir="ltr">
          <Cloud className="h-3 w-3" />
          {snapshot.cloudCoverPct}%
          <span className="mr-1 text-gray-400" dir="rtl">
            غطاء سحابي
          </span>
        </span>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-200 px-5 py-3 dark:border-gray-700">
        ...
        <span
          className="rounded-md bg-gray-200 px-2 py-0.5 text-xs text-gray-600 dark:bg-gray-700 dark:text-gray-300"
          dir="ltr"
        >
          {fieldId}
        </span>
      ...
      <div className="mx-4 mb-4 mt-2 rounded-xl border border-gray-200 bg-white px-5 py-3 dark:border-gray-700 dark:bg-gray-800">
        <div className="flex flex-col items-center gap-1 text-center sm:flex-row sm:justify-between sm:text-start">
          {/* Arabic summary */}
          <p className="text-sm font-medium text-gray-800 dark:text-gray-200 inline-flex items-center gap-1">
            <TrendIcon className={`h-4 w-4 ${trendColor}`} />
            <span className="ml-1 text-gray-500">التغيير:</span>
