...
import type { ActionRecommendation as ActionRecommendationType, ActionType } from '../types/action-windows';
...
const getPriorityLabel = (priority: ActionRecommendationType['priority']): { en: string; ar: string } => {
  switch (priority) {
    case 'urgent':
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 flex-1">
            <div className={`p-2 rounded-lg ${getPriorityColor(recommendation.priority).replace('text-', 'bg-').replace('-800', '-100')}`}>
            ...
            <div className="flex-1" dir="rtl">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs border ${getPriorityColor(recommendation.priority)}`}>
                  {priorityLabel.ar}
                </span>
              </div>
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-start gap-3">
          <div className={`p-3 rounded-xl ${getPriorityColor(recommendation.priority).replace('text-', 'bg-').replace('-800', '-100')}`}>
            {getActionIcon(recommendation.actionType)}
          </div>
        ...
        <div className="text-right">
          <div className="flex items-center gap-1">
            ...
            <span className="text-2xl font-bold text-green-600">{Math.round(recommendation.confidence)}%</span>
          </div>
          <p className="text-xs text-gray-500" dir="rtl">الثقة</p>
        </div>
      </div>
      ...
      {/* Window Information */}
      <div className="bg-blue-50 rounded-lg border border-blue-200 p-4 mb-4" dir="rtl">
        ...
        <div className="space-y-1 text-sm text-blue-800">
          <p>
            <span className="font-medium">البداية:</span> {formatDateTime(recommendation.window.startTime)}
          </p>
          <p>
            <span className="font-medium">النهاية:</span> {formatDateTime(recommendation.window.endTime)}
          </p>
        </div>
      </div>
      ...
      {/* Reason */}
      <div className="mb-4">
        <h4 className="font-semibold text-gray-900 mb-2" dir="rtl">السبب</h4>
        <p className="text-sm text-gray-700" dir="rtl">{recommendation.reasonAr}</p>
      </div>

      {/* Benefits */}
      {recommendation.benefitsAr && recommendation.benefitsAr.length > 0 && (
        <div className="mb-4">
          <h4 className="font-semibold text-gray-900 mb-2" dir="rtl">الفوائد</h4>
          <ul className="space-y-1">
            {recommendation.benefitsAr.map((benefit, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-gray-700" dir="rtl">
      ...
      {recommendation.warningsAr && recommendation.warningsAr.length > 0 && (
        <div className="mb-4">
          <h4 className="font-semibold text-gray-900 mb-2" dir="rtl">تحذيرات</h4>
          <ul className="space-y-1">
            {recommendation.warningsAr.map((warning, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-orange-700" dir="rtl">
      ...
      {showWeather && (
        <div className="mb-4">
          <h4 className="font-semibold text-gray-900 mb-3" dir="rtl">الظروف الجوية</h4>
      ...
      {/* Footer */}
      <div className="mt-4 pt-4 border-t border-gray-200 flex items-center justify-between text-xs text-gray-500" dir="rtl">
