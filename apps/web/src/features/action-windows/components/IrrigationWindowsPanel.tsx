...
import { Droplets, TrendingDown, Calendar, AlertCircle, RefreshCw, Plus, Droplet, Thermometer, Wind, Clock } from 'lucide-react';
...
const getSoilMoistureStatusBadge = (status: IrrigationWindow['soilMoisture']['status']) => {
  switch (status) {
    case 'critical':
      return (
...
export const IrrigationWindowsPanel = React.memo<IrrigationWindowsPanelProps>(({
  ...
  const [selectedWindow, setSelectedWindow] = useState<IrrigationWindow | null>(null);
  const { data: windows, isLoading, error, refetch, isRefetching } = useIrrigationWindows({
    fieldId,
    days,
  });
  ...
  const timelineBlocks = useMemo(() => {
    ...
    return windows.map((window) => ({
      ...
      score: window.priority === 'urgent' ? 100 : window.priority === 'high' ? 80 : window.priority === 'medium' ? 60 : 40,
      ...
      <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl border border-blue-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3" dir="rtl">
            <div className="p-3 bg-blue-100 rounded-lg">
              <Droplets className="w-6 h-6 text-blue-700" aria-hidden="true" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-gray-900">نوافذ الري</h2>
              <p className="text-sm text-gray-600">توصيات الري لـ {days} أيام</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-red-50 rounded-lg p-3 border border-red-300">
            <p className="text-sm text-red-700 mb-1" dir="rtl">عاجل</p>
            <p className="text-2xl font-bold text-red-700">
              {windows.filter((w) => w.priority === 'urgent').length}
            </p>
          </div>
          <div className="bg-orange-50 rounded-lg p-3 border border-orange-300">
            <p className="text-sm text-orange-700 mb-1" dir="rtl">عالي</p>
            <p className="text-2xl font-bold text-orange-700">
              {windows.filter((w) => w.priority === 'high').length}
            </p>
          </div>
          <div className="bg-green-50 rounded-lg p-3 border border-green-300">
            <p className="text-sm text-green-700 mb-1" dir="rtl">متوسط/منخفض</p>
            <p className="text-2xl font-bold text-green-700">
              {windows.filter((w) => w.priority === 'medium' || w.priority === 'low').length}
            </p>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1" dir="rtl">
                ...
                <p className="text-sm text-gray-700 font-medium" dir="rtl">
                  كمية الماء: {window.waterAmount} ملم | المدة: {window.duration} ساعة
                </p>
              </div>
            ...
            <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 mb-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <p className="text-xs text-gray-600 mb-1" dir="rtl">الحالي</p>
                  <p className="text-lg font-bold text-gray-900">
                    {window.soilMoisture.current.toFixed(1)}%
                  </p>
                </div>
                <div>
                  <p className="text-xs text-gray-600 mb-1" dir="rtl">المستهدف</p>
                  <p className="text-lg font-bold text-blue-600">
                    {window.soilMoisture.target.toFixed(1)}%
                  </p>
                </div>
                <div>
                  <p className="text-xs text-gray-600 mb-1" dir="rtl">العجز</p>
              ...
              <div className="mt-3">
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div
                    ...
                    style={{ width: `${(window.soilMoisture.current / window.soilMoisture.target) * 100}%` }}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-50 rounded-lg border border-blue-200 p-3 mb-4">
              <h4 className="font-medium text-blue-900 mb-2 text-sm" dir="rtl">
                معدل التبخر والنتح (ET)
              </h4>
              <div className="grid grid-cols-3 gap-3 text-sm">
                <div>
                  <p className="text-xs text-blue-700 mb-1" dir="rtl">ET₀</p>
                  <p className="font-semibold text-blue-900">{window.et.et0.toFixed(2)} ملم/يوم</p>
                </div>
                <div>
                  <p className="text-xs text-blue-700 mb-1" dir="rtl">ETc</p>
                  <p className="font-semibold text-blue-900">{window.et.etc.toFixed(2)} ملم/يوم</p>
                </div>
                <div>
                  <p className="text-xs text-blue-700 mb-1" dir="rtl">Kc</p>
                  <p className="font-semibold text-blue-900">{window.et.kc.toFixed(2)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <div className="p-3 rounded-lg border bg-gray-50 border-gray-200">
                <div className="flex items-center gap-2 mb-1">
                  <Thermometer className="w-4 h-4 text-gray-600" aria-hidden="true" />
                  <span className="text-xs font-medium text-gray-700">الحرارة</span>
                </div>
                <p className="text-sm font-semibold text-gray-900">{Math.round(window.weather.temperature)}°C</p>
              </div>

              <div className="p-3 rounded-lg border bg-gray-50 border-gray-200">
                <div className="flex items-center gap-2 mb-1">
                  <Wind className="w-4 h-4 text-gray-600" aria-hidden="true" />
                  <span className="text-xs font-medium text-gray-700">الرياح</span>
                </div>
                <p className="text-sm font-semibold text-gray-900">{window.weather.windSpeed} km/h</p>
              </div>

              <div className="p-3 rounded-lg border bg-gray-50 border-gray-200">
                <div className="flex items-center gap-2 mb-1">
                  <Droplet className="w-4 h-4 text-gray-600" aria-hidden="true" />
                  <span className="text-xs font-medium text-gray-700">الرطوبة</span>
                </div>
                <p className="text-sm font-semibold text-gray-900">{Math.round(window.weather.humidity)}%</p>
              </div>

              <div className="p-3 rounded-lg border bg-gray-50 border-gray-200">
                <div className="flex items-center gap-2 mb-1">
                  <Clock className="w-4 h-4 text-gray-600" aria-hidden="true" />
                  <span className="text-xs font-medium text-gray-700">المطر</span>
                </div>
                <p className="text-sm font-semibold text-gray-900">{Math.round(window.weather.rainProbability)}%</p>
              </div>
            </div>

            {/* Reason */}
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-3">
              <h4 className="font-medium text-yellow-900 mb-2 text-sm" dir="rtl">السبب:</h4>
              <p className="text-sm text-yellow-800" dir="rtl">{window.reasonAr}</p>
            </div>

            {/* Recommendations */}
            {window.recommendationsAr.length > 0 && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                <h4 className="font-medium text-green-900 mb-2 text-sm" dir="rtl">توصيات:</h4>
                <ul className="space-y-1">
                  {window.recommendationsAr.map((rec, idx) => (
                    <li key={idx} className="text-sm text-green-800" dir="rtl">
                      • {rec}
                    </li>
                  ))}
                </ul>
