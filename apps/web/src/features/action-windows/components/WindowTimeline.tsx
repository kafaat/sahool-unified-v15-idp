...
import type { Timeline, TimelineBlock, WindowStatus } from '../types/action-windows';
...
export const WindowTimeline = React.memo<WindowTimelineProps>(({
    ...
    if (Array.isArray(timeline)) {
      ...
      return {
        ...
        summary: {
          ...
          optimalWindows: timeline.filter((b) => b.status === 'optimal').length,
          marginalWindows: timeline.filter((b) => b.status === 'marginal').length,
  ...
  const handleBlockKeyPress = (event: React.KeyboardEvent, block: TimelineBlock) => {
  ...
  if (blocks.length === 0) {
    return (
      <div
        ...
        role="status"
      >
        <Calendar className="w-12 h-12 mx-auto mb-3 text-gray-400" aria-hidden="true" />
  ...
  return (
    <div className="space-y-4">
      ...
      {showDetails && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white rounded-lg border border-gray-200 p-3">
            ...
            <p className="text-2xl font-bold text-gray-900">{summary.totalWindows}</p>
          </div>
          ...
          <div className="bg-green-50 rounded-lg border border-green-200 p-3">
            ...
            <p className="text-2xl font-bold text-green-700">{summary.optimalWindows}</p>
          </div>
          ...
          <div className="bg-yellow-50 rounded-lg border border-yellow-200 p-3">
            ...
            <p className="text-2xl font-bold text-yellow-700">{summary.marginalWindows}</p>
          </div>
          ...
          <div className="bg-red-50 rounded-lg border border-red-200 p-3">
            ...
            <p className="text-2xl font-bold text-red-700">{summary.avoidWindows}</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        <h3 className="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2" dir="rtl">
          ...
          {blocks.map((block) => {
            ...
            return (
              <div
                ...
                onKeyPress={(e) => block.actionable && handleBlockKeyPress(e, block)}
                className={`
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold">{Math.round(block.score)}</p>
                  ...
                  <div className="mt-3 pt-3 border-t border-white/20">
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        ...
                        <span className="font-medium">{Math.round(block.details.temperature)}°C</span>
                      </div>
                      <div>
                        <span className="opacity-75">الرياح:</span>{' '}
                        <span className="font-medium">{block.details.windSpeed} km/h</span>
                      </div>
                      <div>
                        <span className="opacity-75">الرطوبة:</span>{' '}
                        <span className="font-medium">{Math.round(block.details.humidity)}%</span>
                      </div>
                      <div>
                        <span className="opacity-75">المطر:</span>{' '}
                        <span className="font-medium">{Math.round(block.details.rainProbability)}%</span>
                      </div>
                    </div>
                  </div>
                )}
      ...
      {selectedBlock && (
        <div className="bg-blue-50 rounded-lg border border-blue-200 p-4" dir="rtl">
          <h4 className="font-semibold text-blue-900 mb-2">تفاصيل النافذة المحددة</h4>
          <div className="space-y-2 text-sm text-blue-800">
            <p>
              <span className="font-medium">الوقت:</span> {formatTime(selectedBlock.startTime)} -{' '}
              {formatTime(selectedBlock.endTime)}
            </p>
            <p>
              <span className="font-medium">الحالة:</span> {selectedBlock.labelAr}
            </p>
            <p>
              <span className="font-medium">النتيجة:</span> {Math.round(selectedBlock.score)}/100
            </p>
      ...
      {/* Legend */}
      <div className="flex items-center justify-center gap-6 text-sm" dir="rtl">
