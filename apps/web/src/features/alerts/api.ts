...
const MOCK_ALERTS: Alert[] = [
  {
    ...
    message: 'Temperature expected to exceed 40°C tomorrow. Consider adjusting irrigation schedule.',
    messageAr: 'من المتوقع أن تتجاوز درجة الحرارة 40 درجة مئوية غداً. يُنصح بتعديل جدول الري.',
    severity: 'warning',
    category: 'weather',
    status: 'active',
    fieldId: 'field-1',
  ...
  },
  {
    ...
    titleAr: 'اكتشاف آفات',
    message: 'Early signs of aphid infestation detected in Field #3. Immediate action recommended.',
    messageAr: 'تم اكتشاف علامات مبكرة لإصابة بالمن في الحقل رقم 3. يوصى باتخاذ إجراء فوري.',
    severity: 'critical',
    category: 'pest',
    status: 'active',
    fieldId: 'field-3',
  ...
  },
  {
    ...
    message: 'Scheduled maintenance for irrigation system in Field #5 tomorrow at 8 AM.',
  ...
  },
  {
    ...
    message: 'Soil moisture levels below optimal range in Field #2. Irrigation needed.',
    messageAr: 'مستويات رطوبة التربة أقل من النطاق الأمثل في الحقل رقم 2. يلزم الري.',
    severity: 'warning',
    category: 'irrigation',
    status: 'active',
    fieldId: 'field-2',
  ...
  },
  {
    ...
    message: 'Coffee prices increased by 15% in the local market. Good time to sell.',
...
export const alertsApi = {
   ...
   */
  bulkDismiss: async (alertIds: string[], reason?: string): Promise<{ success: boolean; updated: number }> => {
    try {
      const response = await api.post('/api/v1/alerts/bulk/dismiss', { alertIds, reason });
      return response.data;
    } catch (error) {
