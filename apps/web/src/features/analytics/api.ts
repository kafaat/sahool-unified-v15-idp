...
export const analyticsApi = {
  ...
  getComparison: async (
    ...
    try {
      ...
      logger.warn(
        "Failed to fetch comparison data from API, using mock data:",
        error,
      );
  ...
  generateReport: async (
    ...
    try {
      ...
      const errorMessage =
        ...
        ERROR_MESSAGES.GENERATE_REPORT_FAILED.en;
      const errorMessageAr =
        axiosError.response?.data?.message_ar ||
        ERROR_MESSAGES.GENERATE_REPORT_FAILED.ar;
  ...
  downloadReport: async (reportId: string): Promise<Blob> => {
    try {
      ...
      const errorMessage =
        axiosError.response?.data?.message ||
        ERROR_MESSAGES.DOWNLOAD_REPORT_FAILED.en;
      const errorMessageAr =
        axiosError.response?.data?.message_ar ||
        ERROR_MESSAGES.DOWNLOAD_REPORT_FAILED.ar;
