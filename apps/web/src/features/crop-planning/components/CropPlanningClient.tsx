...
/**
 * Crop Planning Client Component
...
const PREP_ITEMS = [
  'حراثة أولية (عمق 30 سم)',
  'حراثة ثانية (تنعيم)',
  'تسوية الأرض',
  'تحليل التربة',
  'تسميد أساسي (DAP + بوتاسيوم)',
  'تجهيز شبكة الري',
  'ري تأسيسي',
];
...
export default function CropPlanningClient() {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gray-50 p-6 space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">تخطيط الموسم الزراعي</h1>
        <p className="text-sm text-gray-500 mt-1">خطة شاملة من تحضير الحقل إلى الحصاد</p>
      </div>
      ...
      <div className="flex gap-6">
        ...
        {/* Step Content */}
        <div className="flex-1 bg-white rounded-xl border shadow-sm p-6 min-h-[500px]">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {CROPS.map((c) => (
                  <button
                    key={c.code}
                    onClick={() => setPlan((p) => ({ ...p, cropCode: c.code }))}
                    className={`p-4 rounded-xl border-2 text-center transition-all ${
                      plan.cropCode === c.code
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-500">
                      {c.daysToHarvest} يوم | {c.yieldTonsHa} طن/هـ
                    </p>
                  </button>
                ))}
              </div>
              ...
              <div className="space-y-2">
                {PREP_ITEMS.map((item, i) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={plan.prepChecklist[i] ?? false}
                      onChange={() =>
                        setPlan((p) => {
                          const cl = [...p.prepChecklist];
                          cl[i] = !cl[i];
                          return { ...p, prepChecklist: cl };
                        })
                      }
                      className="w-5 h-5 rounded text-green-600"
                    />
                    <span className={plan.prepChecklist[i] ? 'line-through text-gray-400' : ''}>
                      {item}
                    </span>
                  </label>
                ))}
            ...
            <div className="space-y-4">
              ...
              {selectedCrop && (
                <div className="p-4 bg-blue-50 rounded-lg text-sm">
                  <p>
                    <strong>توصية:</strong> {selectedCrop.nameAr} يُزرع في{' '}
                    {selectedCrop.plantingMonths}
                  </p>
                  <p>
                    معدل البذار الموصى:{' '}
                    {selectedCrop.code === 'wheat'
                      ? '120-140'
                      : selectedCrop.code === 'tomato'
                        ? '0.3-0.5'
                        : '100-120'}{' '}
                    كجم/هكتار
                  </p>
          ...
          {step === 5 && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { key: 'drip', label: 'تنقيط', icon: '💧' },
                  { key: 'sprinkler', label: 'رشاش', icon: '🌧️' },
                  { key: 'flood', label: 'غمر', icon: '🌊' },
                  { key: 'pivot', label: 'محوري', icon: '🔄' },
                ].map((m) => (
                  <button
                    key={m.key}
                    onClick={() => setPlan((p) => ({ ...p, irrigationMethod: m.key }))}
                    className={`p-3 rounded-lg border-2 text-center ${
                      plan.irrigationMethod === m.key
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={plan.irrigationFrequency}
                    onChange={(e) =>
                      setPlan((p) => ({ ...p, irrigationFrequency: +e.target.value }))
                    }
                    className="w-full p-2 border rounded-lg"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={plan.irrigationAmount}
                    onChange={(e) =>
                      setPlan((p) => ({ ...p, irrigationAmount: +e.target.value }))
                    }
                    className="w-full p-2 border rounded-lg"
                  />
                </div>
              ...
              {selectedCrop && (
                <p className="text-sm text-gray-500">
                  الاحتياج المائي الكلي: {selectedCrop.waterMm} مم/موسم
                </p>
              )}
            </div>
          )}
          ...
          {step === 6 && (
            <div className="space-y-4">
              ...
              {[
                {
                  key: 'ndvi' as const,
                  label: 'مراقبة NDVI بالأقمار الصناعية',
                  desc: 'كل 5 أيام — Sentinel-2',
                  icon: '🛰️',
                },
                {
                  key: 'scouting' as const,
                  label: 'فحص ميداني أسبوعي',
                  desc: 'زيارة الحقل وتسجيل الملاحظات',
                  icon: '👁️',
                },
                {
                  key: 'pest' as const,
                  label: 'مراقبة الآفات والأمراض',
                  desc: 'كل أسبوعين — فحص بصري + مصائد',
                  icon: '🐛',
                },
                {
                  key: 'soil' as const,
                  label: 'مراقبة رطوبة التربة',
                  desc: 'مستشعر IoT أو SAR رادار',
                  icon: '💧',
                },
              ].map((item) => (
                <label
                ...
                >
                  <input
                    type="checkbox"
                    checked={plan.monitoringPlan[item.key]}
                    onChange={() =>
                      setPlan((p) => ({
                        ...p,
                        monitoringPlan: {
                          ...p.monitoringPlan,
                          [item.key]: !p.monitoringPlan[item.key],
                        },
                      }))
                    }
                    className="w-5 h-5 rounded text-green-600"
            ...
            <div className="space-y-6">
              ...
              {selectedCrop && plan.plantingDate ? (
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg">
                    ...
                    <ul className="text-sm text-gray-600 space-y-1">
                      ...
                      <li>
                        🌱 تاريخ الزراعة:{' '}
                        {new Date(plan.plantingDate).toLocaleDateString('ar-YE')}
                      </li>
                      <li>
                        💧 الري:{' '}
                        {plan.irrigationMethod === 'drip'
                          ? 'تنقيط'
                          : plan.irrigationMethod === 'sprinkler'
                            ? 'رشاش'
                            : plan.irrigationMethod === 'flood'
                              ? 'غمر'
                              : 'محوري'}{' '}
                        — كل {plan.irrigationFrequency} أيام
                      </li>
                      <li>
                        📊 المراقبة:{' '}
                        {Object.values(plan.monitoringPlan).filter(Boolean).length} برامج نشطة
                      </li>
                ...
                <p className="text-gray-500">
                  يرجى اختيار المحصول وتاريخ الزراعة أولاً (الخطوات 2 و 4)
                </p>
              )}
            </div>
          )}
      ...
      <div className="flex justify-between">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={step === 7}
            className="flex items-center gap-2 px-6 py-2 bg-green-600 text-white rounded-lg disabled:opacity-30 hover:bg-green-700"
          >
