...
// FieldBoundaryMap - Interactive boundary drawing for field creation
// خريطة رسم حدود الحقل التفاعلية
// Supports: polygon (click-to-add vertices) + rectangle (two-click drag)
// Ported from apps/admin/src/components/maps/DrawableMap.tsx
...
export interface FieldBoundaryMapProps {
  onBoundaryChange: (
...
// ---------------------------------------------------------------------------
// Inner component with map event access
// ---------------------------------------------------------------------------
...
function verticesToGeoPolygon(vertices: Vertex[]): GeoPolygon {
...
export default function FieldBoundaryMap({
  onBoundaryChange,
  initialCenter = [15.5527, 48.5164], // Yemen center
  initialZoom = 6,
  height = '420px',
    ...
    (v: Vertex) => {
      ...
      if (mode === 'rectangle') {
        if (rectStage.current === 'first') {
          ...
          onBoundaryChange(verticesToGeoPolygon(rectVertices), bbox);
        }
      } else if (mode === 'polygon') {
        setVertices((prev) => [...prev, v]);
      }
    },
    [mode, completed, vertices, onBoundaryChange]
  );
  ...
  const handleComplete = useCallback(() => {
    ...
    onBoundaryChange(verticesToGeoPolygon(vertices), bbox);
  }, [vertices, onBoundaryChange]);
    ...
    >
      ...
      {completed && (
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-[1000] bg-green-50 border border-green-300 px-4 py-2 rounded-md shadow-md text-sm font-medium text-green-800 flex flex-col items-center gap-1">
          <div>✓ تم تحديد المنطقة — {vertices.length} نقطة</div>
