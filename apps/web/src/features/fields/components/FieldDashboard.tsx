 ...
 */
function calculateNDVITrend(
  timeSeriesData?: Array<{ date: string; ndvi: number }>,
): NDVITrendData {
 ...
 */
function getTodayTasksCount(tasks?: Task[]): {
  total: number;
  completed: number;
  pending: number;
} {
 ...
 */
const LoadingSkeleton: React.FC<{ height?: string }> = ({
  height = "h-24",
...
export const FieldDashboard: React.FC<FieldDashboardProps> = ({
  ...
  const { data: ndviTimeSeries, isLoading: timeSeriesLoading } =
    useNDVITimeSeries(fieldId);
  const {
    data: tasks,
          ...
          <div className="flex items-center space-x-4 space-x-reverse">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                ...
                {isArabic ? field.cropAr : field.crop}
              </p>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    size="sm"
                    variant={
                      selectedView === "satellite" ? "primary" : "outline"
                    }
            ...
            <CardContent>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge
                    variant={irrigation.recommended ? "warning" : "success"}
                  >
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-gray-900">
                        {isArabic
                          ? astronomical.lunar_mansion?.name
                          : astronomical.lunar_mansion?.name_en}
                      </p>
                      <p className="text-xs text-gray-500">
                    ...
                    <Badge variant="info">
                      {isArabic
                        ? astronomical.moon_phase?.name
                        : astronomical.moon_phase?.name_en}
                    </Badge>
                  </div>
                  {astronomical.recommendations?.[0] && (
                    <p className="text-sm text-gray-600">
                      {isArabic
                        ? astronomical.recommendations[0].reason
                        : astronomical.recommendations[0].activity}
                    </p>
                  )}
                </div>
              ) : (
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-700">
                {zoneSummary.healthyZones}{" "}
                {isArabic ? "مناطق صحية" : "Healthy Zones"}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-yellow-500" />
              <span className="text-sm text-gray-700">
                {zoneSummary.attentionZones}{" "}
                {isArabic ? "مناطق تحتاج انتباه" : "Attention Zones"}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-red-500" />
              <span className="text-sm text-gray-700">
                {zoneSummary.criticalZones}{" "}
                {isArabic ? "مناطق حرجة" : "Critical Zones"}
              </span>
            </div>
          </div>
          <p className="text-sm text-gray-500">
            {isArabic ? "إجمالي المناطق" : "Total Zones"}:{" "}
