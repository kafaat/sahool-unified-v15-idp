...
export const LayerControl: React.FC<LayerControlProps> = ({
      ...
      if (saved) {
        try {
          ...
          return {
            ...DEFAULT_NDVI,
            ...parsed,
            historicalDate: parsed.historicalDate ? new Date(parsed.historicalDate) : null,
            ...initialNDVI,
          };
        } catch (e) {
  ...
  return (
    <div className={clsx('absolute z-[1000] max-w-sm', positionClasses[position], className)}>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              <span className="text-sm font-normal text-gray-500">Map Layers</span>
            </CardTitle>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setShowLegend(!showLegend)}
                title={showLegend ? 'إخفاء المفتاح / Hide Legend' : 'عرض المفتاح / Show Legend'}
        ...
        {isExpanded && (
          <CardContent className="space-y-4">
            ...
            <div className="space-y-3">
              {(Object.keys(LAYER_CONFIG) as Array<keyof LayerSettings>).map((layerKey) => {
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">{config.descriptionAr}</p>
                      </div>
                    </div>
                    <Switch
                      id={`layer-${layerKey}`}
            ...
            {layers.ndvi && (
              <div className="border-t pt-4 space-y-4">
                <h4 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                  ...
                  <span className="text-xs font-normal text-gray-500">NDVI Settings</span>
                </h4>

                {/* Opacity Slider / منزلق الشفافية */}
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label htmlFor="ndvi-opacity" className="text-sm text-gray-700 font-medium">
                ...
                <div className="space-y-2">
                  ...
                  {ndviSettings.historicalDate && (
                    <p className="text-xs text-gray-500">
                      عرض بيانات {ndviSettings.historicalDate.toLocaleDateString('ar-EG')}
                      {' / '}
                      Showing data from {ndviSettings.historicalDate.toLocaleDateString('en-US')}
                    </p>
                  )}
                </div>
              </div>
            ...
            {showLegend && layers.ndvi && (
              <div className="border-t pt-4 space-y-3">
                <h4 className="text-sm font-bold text-gray-900">
                  مفتاح ألوان NDVI <span className="text-gray-500">/ Color Legend</span>
                </h4>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {NDVI_COLOR_SCALE.filter((_, i) => i % 2 === 0).map((stop) => (
                    <div key={stop.value} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-gray-900 truncate">{stop.labelAr}</div>
                        <div className="text-gray-500 truncate">{stop.labelEn}</div>
                      </div>
                    </div>
                  ))}
                </div>
