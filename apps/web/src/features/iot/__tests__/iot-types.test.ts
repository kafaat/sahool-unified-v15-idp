...
describe('IoT Types', () => {
  describe('SensorType', () => {
    ...
    it('should include environmental sensors', () => {
      const environmentalSensors: SensorType[] = ['temperature', 'humidity', 'pressure', 'rain', 'wind'];
      environmentalSensors.forEach(sensor => {
        expect(['soil_moisture', 'temperature', 'humidity', 'ph', 'light', 'pressure', 'rain', 'wind']).toContain(sensor);
      });
    });
  });
  ...
  describe('SensorStatus', () => {
    it('should support all status values', () => {
      const statuses: SensorStatus[] = ['online', 'offline', 'error', 'maintenance', 'active', 'inactive'];
      expect(statuses).toHaveLength(6);
    });

    it('should indicate sensor availability', () => {
      const availableStatuses: SensorStatus[] = ['online', 'active'];
      const unavailableStatuses: SensorStatus[] = ['offline', 'error', 'maintenance', 'inactive'];
  ...
  describe('ActuatorType', () => {
    it('should support all actuator types', () => {
      const actuatorTypes: ActuatorType[] = ['valve', 'pump', 'fan', 'heater', 'light', 'sprinkler'];
      expect(actuatorTypes).toHaveLength(6);
    });

    it('should include irrigation actuators', () => {
      const irrigationActuators: ActuatorType[] = ['valve', 'pump', 'sprinkler'];
      irrigationActuators.forEach(actuator => {
        expect(['valve', 'pump', 'fan', 'heater', 'light', 'sprinkler']).toContain(actuator);
      });
    });
  });
  ...
  describe('ActuatorStatus', () => {
    it('should support all actuator status values', () => {
      const statuses: ActuatorStatus[] = ['online', 'offline', 'error', 'on', 'off', 'auto'];
      expect(statuses).toHaveLength(6);
    });
  ...
  describe('Actuator interface', () => {
    ...
    it('should support control modes', () => {
      const modes: Actuator['controlMode'][] = ['manual', 'automatic', 'scheduled'];
  ...
  describe('AlertRule interface', () => {
    ...
    it('should support all condition types', () => {
      const conditions: AlertRule['condition'][] = ['above', 'below', 'between', 'outside'];
  ...
  describe('SensorReadingsQuery interface', () => {
    ...
    it('should support different intervals', () => {
      const intervals: SensorReadingsQuery['interval'][] = ['1h', '1d', '1w', '1m'];
