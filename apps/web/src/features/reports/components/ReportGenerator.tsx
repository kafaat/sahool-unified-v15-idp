...
export const ReportGenerator: React.FC<ReportGeneratorProps> = ({
  ...
  const currentSections = config.type === 'field' ? FIELD_SECTIONS : SEASON_SECTIONS;
      ...
      if (config.type === 'field') {
        ...
        const request: GenerateSeasonReportRequest = {
          ...
          options: {
            ...
            format: config.format,
            language: config.language,
            titleAr: fieldNameAr ? `تقرير موسم - ${fieldNameAr}` : 'تقرير الموسم',
  ...
  const isGenerating = generateFieldReport.isPending || generateSeasonReport.isPending;
  ...
  return (
    <div className="space-y-6">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">تحليل شامل لأداء الحقل الحالي</p>
              </div>
              {config.type === 'field' && <CheckCircle className="w-6 h-6 text-green-500" />}
            </div>
          </button>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">ملخص كامل للموسم الزراعي</p>
              </div>
              {config.type === 'season' && <CheckCircle className="w-6 h-6 text-green-500" />}
            </div>
          </button>
        </div>
      </div>
      ...
      {/* Report Sections */}
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">أقسام التقرير</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setConfig((prev) => ({ ...prev, language: e.target.value as 'ar' | 'en' | 'both' }))
                }
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
              >
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  setConfig((prev) => ({ ...prev, includeMaps: e.target.checked }))
                }
                className="w-5 h-5 text-green-500 rounded focus:ring-green-500"
              />
              <span className="text-sm font-medium text-gray-700">تضمين الخرائط</span>
            </label>
          </div>
        </div>
      </div>
      ...
      {(generateFieldReport.isSuccess || generateSeasonReport.isSuccess) && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
          ...
          <p className="text-sm text-green-600 mt-1">Report generated successfully!</p>
        </div>
      )}
      ...
      {hasError && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
          ...
          <p className="text-sm text-red-600 mt-1">Failed to generate report. Please try again.</p>
        </div>
      )}
    </div>
  );
