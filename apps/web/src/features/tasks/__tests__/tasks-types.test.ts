...
describe('Tasks Types', () => {
  ...
  describe('TaskBoardColumn interface', () => {
    ...
    it('should support different status columns', () => {
      const columns: TaskBoardColumn[] = [
        { id: 'pending', title: 'Pending', title_ar: 'قيد الانتظار', tasks: [], color: '#FFA500' },
        { id: 'in_progress', title: 'In Progress', title_ar: 'قيد التنفيذ', tasks: [], color: '#2196F3' },
        { id: 'completed', title: 'Completed', title_ar: 'مكتمل', tasks: [], color: '#4CAF50' },
        { id: 'cancelled', title: 'Cancelled', title_ar: 'ملغي', tasks: [], color: '#9E9E9E' },
      ];
  ...
  describe('TaskStatistics interface', () => {
    ...
    it('should have consistent totals', () => {
      ...
      const statusSum = stats.open + stats.in_progress + stats.completed + stats.cancelled;
      expect(statusSum).toBe(stats.total);

      // Verify priority totals
      const prioritySum = stats.by_priority.high + stats.by_priority.medium + stats.by_priority.low;
