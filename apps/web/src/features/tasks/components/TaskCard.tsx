...
import { Calendar, MapPin, Flag, Clock, CheckCircle2, XCircle, Circle, AlertCircle } from 'lucide-react';
...
const priorityConfig: Record<string, { colors: string; label: string; labelAr: string; icon: React.ReactNode }> = {
  urgent: {
    colors: 'bg-red-200 text-red-800 border-red-300 dark:bg-red-900 dark:text-red-100',
    label: 'Urgent',
    labelAr: 'عاجلة',
    icon: <Flag className="w-3 h-3" aria-hidden="true" />
  },
  high: {
    colors: 'bg-red-100 text-red-700 border-red-200 dark:bg-red-800 dark:text-red-100',
    label: 'High',
    labelAr: 'عالية',
    icon: <Flag className="w-3 h-3" aria-hidden="true" />
  },
  medium: {
    colors: 'bg-yellow-100 text-yellow-700 border-yellow-200 dark:bg-yellow-800 dark:text-yellow-100',
    label: 'Medium',
    labelAr: 'متوسطة',
    icon: <Flag className="w-3 h-3" aria-hidden="true" />
  },
  low: {
    colors: 'bg-green-100 text-green-700 border-green-200 dark:bg-green-800 dark:text-green-100',
    label: 'Low',
    labelAr: 'منخفضة',
    icon: <Flag className="w-3 h-3" aria-hidden="true" />
  },
...
const statusConfig: Record<string, { colors: string; label: string; labelAr: string; icon: React.ReactNode }> = {
  open: {
  ...
  in_progress: {
    colors: 'border-blue-300 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-700',
    label: 'In Progress',
    labelAr: 'قيد التنفيذ',
    icon: <AlertCircle className="w-4 h-4 text-blue-600" aria-hidden="true" />
  },
  completed: {
    colors: 'border-green-300 bg-green-50 dark:bg-green-900/20 dark:border-green-700',
    label: 'Completed',
    labelAr: 'مكتملة',
    icon: <CheckCircle2 className="w-4 h-4 text-green-600" aria-hidden="true" />
  },
  cancelled: {
...
export const TaskCard: React.FC<TaskCardProps> = React.memo(({ task, onClick, draggable = false }) => {
  // Memoize computed values for performance
  const isOverdue = React.useMemo(() => {
    return task.due_date ? new Date(task.due_date) < new Date() && task.status !== 'completed' : false;
  ...
  const formattedDueDate = React.useMemo(() => {
    return task.due_date ? new Date(task.due_date).toLocaleDateString('ar-EG') : null;
  }, [task.due_date]);
