    ...
    return (
      <div className="bg-white rounded-xl border-2 border-gray-200 overflow-hidden">
        <div className="p-6 border-b-2 border-gray-200 bg-gray-50">
          <h3 className="text-xl font-bold text-gray-900 mb-2">مصفوفة الصلاحيات</h3>
          <p className="text-gray-600 text-sm">عرض الصلاحيات المتاحة لكل دور في النظام</p>
        </div>
                  ...
                  <tr key={category} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {roles.map((role) => {
                      ...
                      return (
                        <td
                          key={role}
                          className={`px-4 py-4 text-center ${isSelected ? 'bg-blue-50' : ''}`}
                        >
                          {allowed && action ? (
