...
export const ERROR_MESSAGES = {
  NETWORK_ERROR: { en: "Network error. Terrain service unavailable.", ar: "خطأ في الاتصال. خدمة التضاريس غير متاحة." },
  DEM_FAILED: { en: "Failed to process DEM data.", ar: "فشل في معالجة بيانات نموذج الارتفاع الرقمي." },
...
export const terrainApi = {
  analyzeDEM: async (fieldId: string, data?: FormData): Promise<DEMAnalysis> => {
    try {
      const response = await api.post(TERRAIN_ENDPOINTS.DEM, data || { field_id: fieldId }, data ? { headers: { "Content-Type": "multipart/form-data" } } : {});
  ...
  optimizeLeveling: async (fieldId: string, targetSlope?: number): Promise<LevelingPlan> => {
    try {
      const response = await api.post(TERRAIN_ENDPOINTS.LEVELING_OPTIMIZE, { field_id: fieldId, target_slope: targetSlope });
      return response.data.data || response.data;
    } catch (error) {
  ...
  calculateCutFill: async (fieldId: string, targetElevation?: number): Promise<CutFillResult> => {
    try {
      const response = await api.post(TERRAIN_ENDPOINTS.LEVELING_CUT_FILL, { field_id: fieldId, target_elevation: targetElevation });
      return response.data.data || response.data;
    } catch (error) {
