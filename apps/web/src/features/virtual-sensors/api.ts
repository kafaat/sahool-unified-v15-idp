...
export const ERROR_MESSAGES = {
  NETWORK_ERROR: { en: "Network error. Virtual sensors unavailable.", ar: "خطأ في الاتصال. الاستشعار الافتراضي غير متاح." },
  ...
  RECOMMENDATION_FAILED: { en: "Failed to get irrigation recommendation.", ar: "فشل في الحصول على توصية الري." },
};

export const virtualSensorsApi = {
  calculateET0: async (data: { temperature: number; humidity: number; windSpeed: number; solarRadiation: number; latitude: number; date?: string }): Promise<ET0Result> => {
    try {
  ...
  calculateETC: async (data: { cropType: string; growthStage?: string; et0?: number; latitude?: number; date?: string }): Promise<ETCResult> => {
    try {
  ...
  estimateSoilMoisture: async (data: { soilType: string; lastIrrigation?: string; et0?: number; rainfall?: number }): Promise<SoilMoistureEstimate> => {
    try {
  ...
  getIrrigationRecommendation: async (data: { fieldId: string; cropType: string; soilType?: string }): Promise<IrrigationRecommendation> => {
    try {
  ...
  quickIrrigationCheck: async (data: { cropType: string; soilMoisture: number; temperature: number }): Promise<IrrigationQuickCheck> => {
    try {
  ...
  getIrrigationMethods: async (): Promise<Array<{ id: string; name: string; nameAr: string; efficiency: number }>> => {
    try {
