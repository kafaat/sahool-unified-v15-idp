...
export const CurrentWeather = React.memo<CurrentWeatherProps>(
  ({ lat, lon, enabled }) => {
    const {
      data: weather,
      isLoading,
    ...
    const metricCards = useMemo(() => {
      ...
      if (weather.pressure) {
        cards.push({
          icon: (
            <Gauge className="w-5 h-5 text-purple-600" aria-hidden="true" />
          ),
          label: "الضغط",
      ...
      if (weather.uvIndex !== undefined) {
        cards.push({
          icon: (
            <Sunrise className="w-5 h-5 text-orange-600" aria-hidden="true" />
          ),
          label: "مؤشر UV",
