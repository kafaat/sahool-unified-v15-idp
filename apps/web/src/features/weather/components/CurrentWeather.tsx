...
const MetricCard = React.memo<MetricCardProps>(
  ({ icon, label, value, unit, extraInfo, ariaLabel }) => {
    return (
      <div
...
const getWeatherIcon = (condition?: string) => {
  ...
  if (lower.includes('rain'))
    return <CloudRain className="w-16 h-16 text-blue-500" aria-hidden="true" />;
  if (lower.includes('clear') || lower.includes('sunny'))
    return <Sun className="w-16 h-16 text-yellow-500" aria-hidden="true" />;
...
export const CurrentWeather = React.memo<CurrentWeatherProps>(({ lat, lon, enabled }) => {
  const {
    ...
    refetch,
    isRefetching,
  } = useCurrentWeather({ lat, lon, enabled });
    ...
    return (
      ...
      >
        <div className="text-center py-8">
          ...
          <p className="text-gray-700 mb-2">عذراً، حدث خطأ أثناء تحميل بيانات الطقس</p>
          ...
          >
            <RefreshCw
              ...
              aria-hidden="true"
            />
            <span>{isRefetching ? 'جاري المحاولة...' : 'إعادة المحاولة'}</span>
          </button>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            {weather.location && <p className="text-sm text-gray-500 mt-1">{weather.location}</p>}
          </div>
        </div>
      </div>

      {/* Weather Details Grid */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4" role="list" aria-label="تفاصيل الطقس">
