...
async function fetchCurrentWeather(lat?: number, lon?: number): Promise<WeatherData> {
  ...
  try {
    const response = await fetch(`${WEATHER_API_BASE}/current?lat=${latitude}&lon=${longitude}`, {
      method: 'GET',
      headers: {
...
async function fetchWeatherAlerts(lat?: number, lon?: number): Promise<WeatherAlert[]> {
  ...
  try {
    const response = await fetch(`${WEATHER_API_BASE}/alerts?lat=${latitude}&lon=${longitude}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json',
