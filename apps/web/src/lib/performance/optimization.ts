...
export function memoize<T extends (...args: Parameters<T>) => ReturnType<T>>(
  ...
  const {
    maxSize = 100,
    ttl,
    keyGenerator = ((...a: unknown[]) => JSON.stringify(a)) as (...args: Parameters<T>) => string,
  } = options;
...
export function getOptimalImageSize(containerWidth: number): number {
  ...
  return IMAGE_SIZES[IMAGE_SIZES.length - 1] ?? containerWidth; // Return max size or container width as fallback
}
...
export function observeElement(
  ...
  return () => {
    ...
    // Decrement ref count and cleanup if no more references
 ...
 * تنظيف جميع مراقبي التقاطع
 */
export function cleanupIntersectionObservers(): void {
