...
app.post("/api/v1/fields", async (req: Request, res: Response) => {
  try {
    ...
    if (coordinates && Array.isArray(coordinates) && coordinates.length >= 3) {
      ...
      const validationResult: GeoValidationResult = validatePolygonCoordinates([
        closedCoords,
      ]);
...
app.put("/api/v1/sync/status", async (req: Request, res: Response) => {
  try {
    const { deviceId, userId, tenantId, lastSyncVersion, deviceInfo, status } =
      req.body;
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const historyWithGeoJson = await Promise.all(
...
AppDataSource.initialize()
  .then(async () => {
    ...
    app.listen(PORT, "0.0.0.0", () => {
      ...
      console.log(
        "    GET  /api/v1/fields/:id/boundary-history  - Get history",
      );
      console.log(
