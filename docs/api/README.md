...
> **Snapshot status:** the checked-in `openapi.json` is a **v15.3.0 snapshot**
> (`info.version: "15.3.0"`) generated by an earlier aggregator run against
> the running services. The current authoritative gateway spec lives in
> [`api/gateway-openapi.yaml`](../../api/gateway-openapi.yaml) (v16.0.0) and
> the per-service specs under [`docs/api/openapi/`](./openapi/) are kept in
> sync manually.
>
> The snapshot **still lists deprecated services** (`satellite-service`,
> `agro-advisor`, `crop-health`, `field-core`, `field-ops`, `ndvi-engine`,
> `weather-core`, `weather-advanced`, `fertilizer-advisor`) — these have been
> migrated per the [Deprecated Services table in CLAUDE.md](../../CLAUDE.md)
> and moved to `archive/deprecated-services/`. Treat the snapshot as a
> historical reference until it is regenerated.
>
> **Regeneration procedure** (when you have all services running locally):
>
> ```bash
> cd docs/api
