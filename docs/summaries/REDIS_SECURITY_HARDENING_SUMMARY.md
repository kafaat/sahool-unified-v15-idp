...
3. **`helm/infra/templates/redis.yaml`**
   ...
   - **Impact:** ⚠️ Breaking change for existing deployments (see migration)
...
- `DEBUG` → (disabled)
- `SHUTDOWN` → `SAHOOL_SHUTDOWN_ADMIN_a9f3e7b1`
- `BGSAVE` → `SAHOOL_BGSAVE_ADMIN_d4b8f2c5`
- `BGREWRITEAOF` → `SAHOOL_BGREWRITEAOF_ADMIN_e7c3a9f2`
- `KEYS` → `SAHOOL_KEYS_SCAN_ONLY_f8d3b7e2` (use SCAN instead)
...
# maxmemory = (peak_usage × 1.5) + 256MB overhead
```
