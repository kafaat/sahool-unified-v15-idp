...
export class IrrigationDecisionService {
  ...
  private readonly cropParams: Map<string, CropKcParams> = new Map([
    ...
    ['STRAWBERRY', {
      Kc_ini: 0.40, Kc_mid: 0.85, Kc_end: 0.75,
    ...
    ['CUCUMBER', {
      Kc_ini: 0.60, Kc_mid: 1.00, Kc_end: 0.75,
  ...
  private readonly irrigationThresholds: Map<string, { [stage: string]: { start: number; stop: number } }> = new Map([
    ['WHEAT', {
      seedling: { start: 0.50, stop: 0.85 },
      vegetative: { start: 0.55, stop: 0.90 },
      flowering: { start: 0.45, stop: 0.85 },
      maturity: { start: 0.60, stop: 0.85 },
    }],
    ['CORN', {
      seedling: { start: 0.55, stop: 0.90 },
      vegetative: { start: 0.50, stop: 0.85 },
      flowering: { start: 0.40, stop: 0.80 }, // Critical stage - lower threshold
      maturity: { start: 0.60, stop: 0.85 },
    }],
    ['RICE', {
      seedling: { start: 0.80, stop: 1.00 }, // Flooded
      vegetative: { start: 0.80, stop: 1.00 },
      flowering: { start: 0.85, stop: 1.00 },
      maturity: { start: 0.70, stop: 0.90 },
    }],
    ['TOMATO', {
      seedling: { start: 0.50, stop: 0.85 },
      vegetative: { start: 0.45, stop: 0.80 },
      flowering: { start: 0.40, stop: 0.75 }, // Sensitive stage
      maturity: { start: 0.50, stop: 0.80 },
    }],
    ['GRAPE', {
      seedling: { start: 0.55, stop: 0.85 },
      vegetative: { start: 0.50, stop: 0.80 },
      flowering: { start: 0.55, stop: 0.85 },
      maturity: { start: 0.60, stop: 0.80 }, // Slight stress improves sugar
    }],
    ['STRAWBERRY', {
      seedling: { start: 0.60, stop: 0.90 },
      vegetative: { start: 0.55, stop: 0.85 },
      flowering: { start: 0.50, stop: 0.80 }, // Very sensitive
  ...
  selectOptimalMethod(scenario: ScenarioInput): MethodRecommendation {
    ...
    if (scenario.budget === 'high' && scenario.cropValue === 'high' && scenario.technicalCapability === 'advanced') {
      primaryMethod = 'crop_model';
      estimatedWaterSaving = 35;
      estimatedYieldIncrease = 15;
      implementationCost = 'high';
      reasoning = 'High budget and technical capability support advanced crop model system for maximum precision and yield optimization';
      reasoningAr = 'الميزانية العالية والقدرة التقنية تدعم نظام نماذج المحاصيل المتقدم لتحقيق أقصى دقة وتحسين الإنتاجية';
      technicalRequirements.push('Weather station', 'Soil moisture sensors (TDR/capacitance)', 'Cloud platform', 'DSSAT/APSIM model calibration');
      technicalRequirementsAr.push('محطة أرصاد جوية', 'مستشعرات رطوبة التربة', 'منصة سحابية', 'معايرة نموذج DSSAT/APSIM');
    }
    // Greenhouse → Crop Model (even with medium budget)
      ...
      reasoning = 'Greenhouse environment requires precise control; crop models prevent overwatering and disease';
      reasoningAr = 'بيئة الدفيئة تتطلب تحكماً دقيقاً؛ نماذج المحاصيل تمنع الإفراط في الري والأمراض';
      technicalRequirements.push('Temperature/humidity sensors', 'Automated valve control', 'Growth monitoring');
      technicalRequirementsAr.push('مستشعرات درجة الحرارة/الرطوبة', 'تحكم آلي بالصمامات', 'مراقبة النمو');
    }
    // Mountain terrain → Threshold Control
    else if (scenario.terrain === 'mountain' || scenario.terrain === 'terrace') {
      primaryMethod = 'threshold';
      estimatedWaterSaving = 25;
      estimatedYieldIncrease = 10;
      implementationCost = 'medium';
      reasoning = 'Complex terrain requires real-time soil moisture response; threshold control handles pressure variations';
      reasoningAr = 'التضاريس المعقدة تتطلب استجابة فورية لرطوبة التربة؛ التحكم بالعتبة يتعامل مع تقلبات الضغط';
      technicalRequirements.push('Soil moisture sensors per zone', 'Pressure-compensating emitters', 'PLC/SCADA controller');
      technicalRequirementsAr.push('مستشعرات رطوبة لكل منطقة', 'باعثات تعويض الضغط', 'وحدة تحكم PLC');
    }
    // Medium budget + Plain terrain → Hybrid (FAO-56 + Threshold)
      ...
      implementationCost = 'medium';
      reasoning = 'Hybrid approach: FAO-56 for baseline planning + threshold control for real-time execution';
      reasoningAr = 'النهج المختلط: FAO-56 للتخطيط الأساسي + التحكم بالعتبة للتنفيذ الفوري';
      technicalRequirements.push('Weather data access', 'Basic soil sensors', 'Timer-based controller');
      technicalRequirementsAr.push('الوصول لبيانات الطقس', 'مستشعرات تربة أساسية', 'وحدة تحكم بالوقت');
    }
    // Low budget or basic capability → FAO-56 with manual threshold
    else if (scenario.budget === 'low' || scenario.technicalCapability === 'minimal') {
      primaryMethod = 'fao56';
      estimatedWaterSaving = 15;
      estimatedYieldIncrease = 5;
      implementationCost = 'low';
      reasoning = 'FAO-56 provides theoretical baseline with minimal infrastructure; manual monitoring sufficient';
      reasoningAr = 'FAO-56 يوفر الأساس النظري مع الحد الأدنى من البنية التحتية؛ المراقبة اليدوية كافية';
      technicalRequirements.push('Weather data (online/station)', 'Kc tables', 'Simple calculation spreadsheet');
      technicalRequirementsAr.push('بيانات الطقس (أونلاين/محطة)', 'جداول Kc', 'جدول حسابات بسيط');
    }
    // Default: Threshold control
      ...
      implementationCost = 'medium';
      reasoning = 'Threshold control provides good balance of cost and water savings for general scenarios';
      reasoningAr = 'التحكم بالعتبة يوفر توازناً جيداً بين التكلفة وتوفير المياه للسيناريوهات العامة';
      technicalRequirements.push('Soil moisture sensors', 'Automatic valve controller', 'Basic programming');
      technicalRequirementsAr.push('مستشعرات رطوبة التربة', 'وحدة تحكم آلية بالصمامات', 'برمجة أساسية');
    }
    ...
    const methodNames: { [key: string]: { en: string; ar: string } } = {
      fao56: { en: 'FAO-56 Reference Evapotranspiration', ar: 'التبخر-نتح المرجعي FAO-56' },
      threshold: { en: 'Soil Moisture Threshold Control', ar: 'التحكم بعتبة رطوبة التربة' },
      crop_model: { en: 'Crop Growth Model (DSSAT/APSIM)', ar: 'نموذج نمو المحاصيل (DSSAT/APSIM)' },
  ...
  calculateETc(input: ETcCalculationInput): {
    ...
    const { stage, kc } = this.calculateKcForDay(input.daysAfterPlanting, cropParams);
    ...
    const adjustedEtc = etc * ks * (input.growthStageAdjustment ? soilFactor : 1.0);
  ...
  private calculateKcForDay(daysAfterPlanting: number, params: CropKcParams): { stage: string; kc: number } {
  ...
  evaluateThresholdControl(input: ThresholdControlInput): IrrigationRecommendation {
    ...
    if (!thresholds || !cropParams || !soilProps) {
      ...
      const defaultThreshold = { start: 0.50, stop: 0.85 };
      return this.makeDecision(input.currentSoilMoisture, defaultThreshold, soilProps || this.soilTypes.get('loam')!, input.rootDepth);
    }

    const stageThreshold = thresholds[input.growthStage] || { start: 0.50, stop: 0.85 };
    return this.makeDecision(input.currentSoilMoisture, stageThreshold, soilProps, input.rootDepth);
  }
  ...
  private makeDecision(
    ...
    const relativeSaturation = (currentMoisture - soilProps.wiltingPoint) /
                               (soilProps.fieldCapacity - soilProps.wiltingPoint);

    // Calculate irrigation amount needed
    const targetMoisture = threshold.stop * (soilProps.fieldCapacity - soilProps.wiltingPoint) + soilProps.wiltingPoint;
    const deficit = Math.max(0, targetMoisture - currentMoisture);
    const irrigationAmount = deficit * rootDepth * 1000; // Convert to mm

    let shouldIrrigate = false;
    let urgency: 'critical' | 'recommended' | 'optional' | 'not_needed' = 'not_needed';
    let reasoning = '';
    let reasoningAr = '';
    ...
    if (relativeSaturation <= 0.3) {
      shouldIrrigate = true;
      urgency = 'critical';
      reasoning = 'Soil moisture critically low - immediate irrigation required to prevent crop damage';
      reasoningAr = 'رطوبة التربة منخفضة بشكل حرج - الري الفوري مطلوب لمنع تلف المحصول';
    } else if (relativeSaturation <= threshold.start) {
      shouldIrrigate = true;
      urgency = 'recommended';
      reasoning = 'Soil moisture below threshold - irrigation recommended within 24 hours';
    ...
    // Add tips
    if (shouldIrrigate) {
      waterEfficiencyTips.push('Irrigate during early morning or late evening to reduce evaporation');
      waterEfficiencyTipsAr.push('الري في الصباح الباكر أو المساء المتأخر لتقليل التبخر');

      if (irrigationAmount > 30) {
        waterEfficiencyTips.push('Consider split irrigation to improve infiltration');
        waterEfficiencyTipsAr.push('فكر في تقسيم الري لتحسين التسرب');
      }
    }

    // Calculate next check time
    const hoursUntilCheck = urgency === 'critical' ? 6 :
                           urgency === 'recommended' ? 12 :
                           urgency === 'optional' ? 24 : 48;
  ...
  generateSmartSchedule(input: SmartScheduleInput): {
    ...
    const method = input.budget === 'high' ? 'crop_model' :
                   input.budget === 'medium' ? 'hybrid' : 'threshold';
    ...
    const TAW = (soilProps.fieldCapacity - soilProps.wiltingPoint) * rootDepth * 1000; // mm
    const RAW = TAW * cropParams.depletionFraction; // Readily Available Water

    for (const forecast of input.weatherForecast) {
      const forecastDate = new Date(forecast.date);
      const dayOfSeason = Math.floor((forecastDate.getTime() - sowingDate.getTime()) / (1000 * 60 * 60 * 24));
      ...
      currentMoisture = currentMoisture + effectivePrecipitation / (rootDepth * 1000) - etc / (rootDepth * 1000);
      currentMoisture = Math.max(soilProps.wiltingPoint, Math.min(soilProps.fieldCapacity, currentMoisture));

      // Check if irrigation needed
      const depletion = (soilProps.fieldCapacity - currentMoisture) * rootDepth * 1000;
    ...
    const waterSavings = Math.max(0, Math.round((faoIrrigation - totalIrrigation) / faoIrrigation * 100));

    const methodNames: { [key: string]: { en: string; ar: string } } = {
      crop_model: { en: 'Crop Growth Model Integration', ar: 'تكامل نموذج نمو المحاصيل' },
      hybrid: { en: 'Hybrid FAO-56 + Threshold', ar: 'مختلط FAO-56 + عتبة' },
  ...
  compareMethods(): {
    ...
    return {
      ...
      recommendation: 'Progressive adoption recommended: Start with FAO-56 for baseline, add threshold sensors, then integrate crop models as budget allows.',
      recommendationAr: 'يُوصى بالتبني التدريجي: ابدأ بـ FAO-56 للأساس، أضف مستشعرات العتبة، ثم ادمج نماذج المحاصيل حسب الميزانية.',
    };
  }
