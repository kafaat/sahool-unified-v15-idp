...
interface DataSource {
  ...
  nameEn: string;
  nameAr: string;
  type: 'weather' | 'market' | 'research' | 'satellite' | 'commodity' | 'news' | 'government';
  url: string;
  region: string[];
...
interface AgriculturalNews {
  ...
  url: string;
  publishedAt: string;
  category: 'policy' | 'market' | 'technology' | 'weather' | 'research' | 'pest';
  region: string;
...
export class WebDataCollectorService {
  ...
  private readonly sampleAlerts: WeatherAlert[] = [
    {
      ...
      descriptionEn: 'Temperatures expected to exceed 45°C. Increase irrigation frequency and provide shade for sensitive crops.',
      descriptionAr: 'يتوقع أن تتجاوز درجات الحرارة 45 درجة مئوية. زد معدل الري ووفر الظل للمحاصيل الحساسة.',
    ...
    },
    {
      ...
      descriptionEn: 'Moderate dust storm expected. Cover sensitive equipment and consider delaying pesticide applications.',
      descriptionAr: 'يتوقع عاصفة ترابية متوسطة. قم بتغطية المعدات الحساسة وفكر في تأجيل تطبيقات المبيدات.',
  ...
  private readonly sampleNews: AgriculturalNews[] = [
    {
      ...
      summary: 'The Ministry of Environment, Water and Agriculture has announced expanded support programs for precision agriculture adoption.',
      summaryAr: 'أعلنت وزارة البيئة والمياه والزراعة عن توسيع برامج الدعم لتبني الزراعة الدقيقة.',
      source: 'MEWA',
    ...
    },
    {
      ...
      summary: 'Agricultural researchers have developed a new date palm variety that shows significant yield improvements in hot climates.',
      summaryAr: 'طور الباحثون الزراعيون صنفاً جديداً من نخيل التمر يظهر تحسناً كبيراً في الإنتاجية في المناخات الحارة.',
      source: 'KACST',
    ...
    },
    {
      ...
      summary: 'International wheat prices have increased 8% this month due to production challenges in major exporting countries.',
      summaryAr: 'ارتفعت أسعار القمح الدولية بنسبة 8% هذا الشهر بسبب تحديات الإنتاج في الدول المصدرة الرئيسية.',
      source: 'FAO GIEWS',
  ...
  private readonly sampleResearch: ResearchPaper[] = [
    {
      ...
      abstract: 'This study evaluates deficit irrigation strategies for date palm cultivation in arid regions, demonstrating water savings of up to 30% without significant yield reduction.',
      journal: 'Agricultural Water Management',
      year: 2024,
      doi: '10.1016/j.agwat.2024.107XXX',
      keywords: ['date palm', 'deficit irrigation', 'water use efficiency', 'arid agriculture'],
    ...
    },
    {
      ...
      abstract: 'Comprehensive review of machine learning approaches for crop yield prediction, analyzing 567 studies from 2015-2024.',
      journal: 'Computers and Electronics in Agriculture',
      year: 2024,
      doi: '10.1016/j.compag.2024.108XXX',
      keywords: ['machine learning', 'yield prediction', 'precision agriculture', 'deep learning'],
  ...
  getDataSourcesByRegion(region: string): DataSource[] {
    return Array.from(this.dataSources.values()).filter(s =>
      s.region.includes(region) || s.region.includes('global'),
    );
  }
  ...
  getWeatherAlerts(region?: string): WeatherAlert[] {
    if (region) {
      return this.sampleAlerts.filter(a =>
        a.affectedRegions.some(r => r.toLowerCase().includes(region.toLowerCase())),
      );
    }
   ...
   */
  getAgriculturalNews(category?: AgriculturalNews['category'], limit?: number): AgriculturalNews[] {
    ...
    if (keywords && keywords.length > 0) {
      return this.sampleResearch.filter(p =>
        keywords.some(k =>
          ...
          p.relevanceToSahool.some(r => r.toLowerCase().includes(k.toLowerCase())),
        ),
      );
    }
