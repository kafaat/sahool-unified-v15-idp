...
export class DisasterService {
  ...
  async assessFieldDamage(fieldId: string, dto: DisasterAssessmentDto) {
    ...
    const estimatedLoss = dto.estimatedLossYER ?? Math.round(affectedArea * avgValuePerHectare * (damagePercentage / 100));

    // Generate recommendations based on disaster type and damage level
    const recommendations = this.generateRecommendations(dto.disasterId, damageLevel.level);
    ...
    return {
      ...
      insuranceClaimAmount: damagePercentage >= 30 ? Math.round(estimatedLoss * 0.7) : 0,
  ...
  async getStatistics(params: { year?: number; governorate?: string }) {
    ...
    return {
      ...
      governorateAr: params.governorate ? GOVERNORATE_AR[params.governorate] : 'جميع المحافظات',
      summary: {
        totalDisasters: 45,
        activeDisasters: 3,
      ...
      byType: [
        { type: DisasterType.FLOOD, typeAr: 'فيضان', count: 12, lossYER: 250000000 },
        { type: DisasterType.DROUGHT, typeAr: 'جفاف', count: 8, lossYER: 180000000 },
        { type: DisasterType.LOCUST, typeAr: 'جراد', count: 5, lossYER: 320000000 },
        { type: DisasterType.PEST, typeAr: 'آفات', count: 15, lossYER: 60000000 },
        { type: DisasterType.DISEASE, typeAr: 'أمراض', count: 5, lossYER: 40000000 },
      ],
  ...
  private generateAffectedFields(count: number) {
    return Array.from({ length: Math.min(count, 10) }, (_, i) => ({
      ...
      cropType: ['wheat', 'coffee', 'qat', 'sorghum'][Math.floor(Math.random() * 4)],
    }));
  }
  ...
  private generateRecommendations(disasterId: string, damageLevel: string) {
    ...
    return recommendationsByType[type] || {
      en: ['Document damage', 'Contact agricultural extension services', 'Apply for disaster relief'],
      ar: ['توثيق الأضرار', 'التواصل مع خدمات الإرشاد الزراعي', 'التقدم للحصول على إغاثة الكوارث'],
    };
  }
}
