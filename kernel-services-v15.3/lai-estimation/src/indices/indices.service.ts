...
const VEGETATION_INDICES: Record<string, IndexInfo> = {
  NDVI: {
    ...
    description: 'Most widely used vegetation index for monitoring plant health and density',
  ...
  EVI2: {
    ...
    description: 'Optimized to enhance vegetation signal with improved sensitivity in high biomass regions',
    descriptionAr: 'محسّن لتعزيز إشارة الغطاء النباتي مع حساسية محسنة في مناطق الكتلة الحيوية العالية',
    range: { min: -1, max: 1 },
  ...
  SAVI: {
    ...
    description: 'Minimizes soil brightness influences using a soil brightness correction factor (L=0.5)',
    descriptionAr: 'يقلل من تأثيرات سطوع التربة باستخدام معامل تصحيح سطوع التربة',
    range: { min: -1, max: 1 },
  ...
  MSAVI: {
    ...
    description: 'Self-adjusting soil factor to minimize soil background effects',
  ...
  NDRE: {
    ...
    description: 'Sensitive to chlorophyll content in moderately vegetated areas',
    descriptionAr: 'حساس لمحتوى الكلوروفيل في المناطق ذات الغطاء النباتي المتوسط',
    range: { min: -1, max: 1 },
  ...
  MTVI2: {
    ...
    formula: '1.5 × (1.2 × (NIR - Green) - 2.5 × (Red - Green)) / √((2 × NIR + 1)² - (6 × NIR - 5 × √Red) - 0.5)',
    description: 'Optimized LAI estimation index with reduced soil background sensitivity',
...
export class VegetationIndicesService {
  ...
  calculateAllIndices(bands: {
    ...
    const indices: Record<string, { value: number; status: string; statusAr: string }> = {};
    ...
    // MSAVI
    const msavi = (2 * nir + 1 - Math.sqrt((2 * nir + 1) ** 2 - 8 * (nir - red))) / 2;
    indices.MSAVI = {
    ...
    const mtvi2Den = Math.sqrt((2 * nir + 1) ** 2 - (6 * nir - 5 * Math.sqrt(Math.max(0, red))) - 0.5);
  ...
  analyzeVegetationHealth(bands: {
    ...
    if (overallHealth >= 80) {
      category = 'Excellent';
      categoryAr = 'ممتاز';
      recommendations.push('Vegetation is in excellent condition. Maintain current practices.');
      recommendationsAr.push('الغطاء النباتي في حالة ممتازة. استمر في الممارسات الحالية.');
    } else if (overallHealth >= 60) {
      category = 'Good';
      categoryAr = 'جيد';
      recommendations.push('Vegetation health is good. Monitor for any changes.');
      recommendationsAr.push('صحة الغطاء النباتي جيدة. راقب أي تغييرات.');
    } else if (overallHealth >= 40) {
      category = 'Moderate';
      categoryAr = 'متوسط';
      recommendations.push('Vegetation shows moderate stress. Consider irrigation or fertilization.');
      recommendationsAr.push('يظهر الغطاء النباتي إجهادًا متوسطًا. فكر في الري أو التسميد.');
    } else if (overallHealth >= 20) {
      category = 'Poor';
      categoryAr = 'ضعيف';
      recommendations.push('Vegetation is stressed. Immediate intervention recommended.');
      recommendationsAr.push('الغطاء النباتي متأثر. يُنصح بالتدخل الفوري.');
    } else {
      category = 'Critical';
      categoryAr = 'حرج';
      recommendations.push('Vegetation is in critical condition. Urgent action required.');
    ...
    if (indices.NDVI && indices.NDVI.value < 0.4) {
      recommendations.push('Low NDVI suggests sparse vegetation or poor health.');
      recommendationsAr.push('انخفاض NDVI يشير إلى غطاء نباتي متفرق أو صحة ضعيفة.');
    }

    if (indices.GNDVI && indices.GNDVI.value < 0.3) {
      recommendations.push('Low GNDVI indicates potential chlorophyll deficiency.');
