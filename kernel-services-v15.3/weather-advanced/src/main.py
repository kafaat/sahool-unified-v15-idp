...
YEMEN_LOCATIONS = {
    ...
    "hadramaut": {"lat": 15.9500, "lon": 48.7833, "name_ar": "حضرموت", "elevation": 650},
}
...
def calculate_evapotranspiration(temp: float, humidity: float, wind_speed: float, solar_radiation: float = 20) -> float:
    ...
    for i, day in enumerate(forecast):
        ...
        if day.temp_max_c >= 40:
            alerts.append(WeatherAlert(
                ...
                severity=AlertSeverity.WARNING if day.temp_max_c < 45 else AlertSeverity.EMERGENCY,
        ...
        if day.precipitation_total_mm >= 30:
            alerts.append(WeatherAlert(
                ...
                severity=AlertSeverity.WATCH if day.precipitation_total_mm < 50 else AlertSeverity.WARNING,
...
def get_spray_windows(hourly: List[HourlyForecast]) -> List[str]:
    ...
    for hour in hourly[:48]:  # Next 48 hours
        if (hour.wind_speed_kmh < 15 and
            hour.precipitation_probability < 20 and
            hour.temperature_c < 35 and
            hour.temperature_c > 15):
...
def get_forecast(
    ...
    for h in range(48):
        ...
        precip_prob = 0.6 if condition in [WeatherCondition.RAIN, WeatherCondition.HEAVY_RAIN, WeatherCondition.THUNDERSTORM] else random.uniform(0, 0.2)
    ...
    for d in range(days):
        ...
        condition = generate_weather_condition((temp_high + temp_low) / 2, humidity_avg, forecast_date.month)

        precip_total = 0
        precip_prob = 0
        if condition in [WeatherCondition.RAIN, WeatherCondition.HEAVY_RAIN, WeatherCondition.THUNDERSTORM]:
            precip_total = random.uniform(5, 40) if condition != WeatherCondition.HEAVY_RAIN else random.uniform(30, 80)
    ...
    if et0 > 6:
        irrig_ar = "💧 احتياج ري عالي اليوم ({} ملم) - ري صباحي ومسائي مطلوب".format(et0)
        irrig_en = "💧 High irrigation need today ({} mm) - morning and evening irrigation required".format(et0)
    elif et0 > 4:
        irrig_ar = "💧 احتياج ري متوسط ({} ملم) - ري واحد كافي".format(et0)
        irrig_en = "💧 Medium irrigation need ({} mm) - one irrigation sufficient".format(et0)
    else:
        irrig_ar = "💧 احتياج ري منخفض ({} ملم) - تقليل الري ممكن".format(et0)
        irrig_en = "💧 Low irrigation need ({} mm) - reduced irrigation possible".format(et0)
...
def get_agricultural_calendar(
    location_id: str,
    crop: str = Query(default="tomato", description="نوع المحصول")
):
