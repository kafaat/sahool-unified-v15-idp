...
class AlertService:
    ...
    async def _create_alert_from_event(
        ...
        if event_type == EventTypes.TASK_CREATED:
            ...
            priority = (
                AlertPriority(priority_str)
                if priority_str in [p.value for p in AlertPriority]
        ...
        for channel in alert.channels:
            try:
                if channel == AlertChannel.SMS and alert.recipient_phone:
                    sent = await self.sms.send(
                        alert.recipient_phone, f"{alert.title}\n{alert.message_ar}"
                    )
                elif channel == AlertChannel.PUSH:
                    sent = await self.push.send(
                        alert.recipient_id, alert.title, alert.message_ar
                    )
                elif channel == AlertChannel.WHATSAPP and alert.recipient_phone:
                    sent = await self.whatsapp.send(
                        alert.recipient_phone, f"*{alert.title}*\n{alert.message_ar}"
                    )
...
async def ready():
    return {
        "status": (
            "ready"
            if alert_service.nc and alert_service.nc.is_connected
            else "not_ready"
        )
    }
