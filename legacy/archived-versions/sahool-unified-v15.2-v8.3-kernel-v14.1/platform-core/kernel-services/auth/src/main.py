...
class Settings:
    ...
    DATABASE_URL = os.getenv(
        "DATABASE_URL", "postgresql+asyncpg://sahool:sahool@postgres:5432/sahool_auth"
    )
...
auth_requests = Counter(
    "auth_requests_total", "Total authentication requests", ["method", "status"]
)
token_operations = Counter(
    "token_operations_total", "Token operations", ["operation", "status"]
)
...
class AuthService:
    ...
    async def authenticate(
        ...
        async with self.db.session() as session:
            ...
            session_obj.refresh_token_hash = hashlib.sha256(
    ...
    async def refresh_tokens(self, refresh_token: str) -> tuple[str, str]:
        ...
        async with self.db.session() as session:
            ...
            # Update session
            user_session.refresh_token_hash = hashlib.sha256(
                new_refresh_token.encode()
            ).hexdigest()
            user_session.last_used = datetime.utcnow()
            user_session.expires_at = datetime.utcnow() + timedelta(
...
def require_permissions(*permissions: str):
    ...
    async def check_permissions(current_user: TokenPayload = Depends(get_current_user)):
        for perm in permissions:
            if (
                perm not in current_user.permissions
...
async def logout(
    data: TokenRefreshRequest, current_user: TokenPayload = Depends(get_current_user)
):
...
async def change_password(
    data: PasswordChangeRequest, current_user: TokenPayload = Depends(get_current_user)
):
...
async def setup_mfa(current_user: TokenPayload = Depends(get_current_user)):
    """Setup MFA for user"""
    from sqlalchemy import select
...
async def verify_mfa_setup(
    data: MFAVerifyRequest, current_user: TokenPayload = Depends(get_current_user)
):
...
async def create_api_key(
    data: APIKeyCreate, current_user: TokenPayload = Depends(get_current_user)
):
    """Create new API key"""
    # Generate API key
    ...
    async with db.session() as session:
        api_key = APIKey(
            ...
            scopes=data.scopes,
            expires_at=(
                datetime.utcnow() + timedelta(days=data.expires_in_days)
...
async def list_api_keys(current_user: TokenPayload = Depends(get_current_user)):
...
async def delete_api_key(
    key_id: uuid.UUID, current_user: TokenPayload = Depends(get_current_user)
):
    """Delete API key"""
    from sqlalchemy import select
    ...
    async with db.session() as session:
        result = await session.execute(
            select(APIKey).where(
                APIKey.id == key_id, APIKey.user_id == uuid.UUID(current_user.sub)
            )
        )
...
@app.delete(
    "/api/v1/auth/sessions/{session_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def revoke_session(
    session_id: uuid.UUID, current_user: TokenPayload = Depends(get_current_user)
):
...
async def revoke_all_sessions(current_user: TokenPayload = Depends(get_current_user)):
