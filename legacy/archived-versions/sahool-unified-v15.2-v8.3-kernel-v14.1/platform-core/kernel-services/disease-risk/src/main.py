...
DATABASE_URL = os.getenv(
    "DATABASE_URL", "postgresql://sahool:sahool@localhost:5432/disease_risk"
)
...
class DiseaseRiskFactor:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "optimal_range": {
                "min": self.optimal_range[0],
                "max": self.optimal_range[1],
            },
...
DISEASE_RISK_MODELS = {
    ...
    "bacterial_wilt": {
        ...
        "prevention_actions": ["تعقيم التربة", "تناوب المحاصيل", "استخدام شتلات سليمة"],
        "treatment_actions": [
...
class DiseaseRiskCalculator:
    ...
    def record_weather(self, region: str, weather_data: Dict):
        ...
        self.weather_history[region].append(
            {"timestamp": datetime.utcnow().isoformat(), "data": weather_data}
        )
    ...
    def record_disease_detection(self, region: str, disease_data: Dict):
        ...
        self.disease_detections[region].append(
            {"timestamp": datetime.utcnow().isoformat(), "data": disease_data}
        )
    ...
    def calculate_risk(
        self, region: str, current_weather: Dict, crop_type: Optional[str] = None
    ...
    def _calculate_single_disease_risk(
        self, disease_id: str, model: Dict, weather: Dict, region: str
        ...
        return DiseaseRiskAssessment(
            ...
            + (len(self.weather_history.get(region, [])) / 168) * 0.2,
    ...
    def _calculate_outbreak_probability(
        self, risk_score: float, detection_factor: float, region: str
    ) -> float:
        ...
        if len(history) > 24:
            ...
            favorable_hours = sum(
                1 for h in history[-48:] if h["data"].get("humidity_percent", 0) > 70
            )
...
class DiseaseRiskService:
    ...
    async def _publish_risk_assessment(
        self, assessment: DiseaseRiskAssessment, location: Dict, tenant_id: str
    ):
        ...
        event = create_event(
            ...
            payload={
                "disease": {
                    ...
                    "name": {
                        "ar": assessment.disease_name_ar,
                        "en": assessment.disease_name_en,
                    },
...
async def calculate_risk(request: RiskQueryRequest):
    ...
    assessments = disease_service.calculator.calculate_risk(
        request.region, weather, request.crop_type
    )
