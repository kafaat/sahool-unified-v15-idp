...
class IrrigationCalculator:
    ...
    def calculate_requirement(
        ...
        water_req = CROP_WATER_REQUIREMENTS.get(crop_type, DEFAULT_WATER_REQ).get(
            growth_stage, 3.5
        )
        ...
        duration = int(
            water_liters / (flow_rates[irrigation_method] * (field_area_sqm / 100))
        )
        duration = max(15, min(duration, 180))  # 15 min to 3 hours

        # Determine urgency
        urgency = self._calculate_urgency(water_req, temp, humidity, last_irrigation)
        ...
        wisdom = TRADITIONAL_WISDOM.get(
            self.current_star, TRADITIONAL_WISDOM["default"]
        )
        ...
        return IrrigationRecommendation(
            ...
            crop_type=crop_type,
            recommendation_type=(
                "scheduled" if urgency != UrgencyLevel.CRITICAL else "urgent"
            ),
            urgency=urgency,
    ...
    def check_irrigation_delay(
        self, field_id: str, last_irrigation: datetime, expected_frequency_days: int = 2
    ) -> Optional[Dict]:
        ...
        if days_since > expected_frequency_days:
            return {
                ...
                "severity": (
                    "critical" if days_since > expected_frequency_days + 2 else "high"
                ),
            }
...
class IrrigationAdvisorService:
    ...
    async def _handle_crop_stage_change(self, msg):
        ...
        try:
            ...
            # Update field state
            self.calculator.update_field_state(
                field_id,
                {
    ...
    async def _generate_and_publish_recommendation(
        self, field_id: str, state: Dict, tenant_id: str, urgent: bool = False
    ):
        ...
        try:
            ...
            logger.error(
                "recommendation_generation_failed", field_id=field_id, error=str(e)
            )
