...
class Settings:
    ...
    DATABASE_URL = os.getenv(
        "DATABASE_URL", "postgresql+asyncpg://sahool:sahool@postgres:5432/sahool_market"
    )
...
market_events = Counter(
    "market_events_total", "Market events", ["event_type", "status"]
)
...
class Market(DBBaseModel):
    ...
    # Market details
    market_type = Column(
        String(50), default="wholesale"
    )  # wholesale, retail, farm_gate
    operating_days = Column(
        ARRAY(String), default=["sat", "sun", "mon", "tue", "wed", "thu"]
    )
    ...
    # Relationships
    prices = relationship(
        "CropPrice", back_populates="market", cascade="all, delete-orphan"
    )
...
class Crop(DBBaseModel):
    ...
    )  # vegetables, fruits, grains, cash_crops
    ...
    # Relationships
    prices = relationship(
        "CropPrice", back_populates="crop", cascade="all, delete-orphan"
    )
...
class CropPrice(DBBaseModel):
    ...
    )  # scarce, low, moderate, abundant, surplus
...
class MarketAnalysisEngine:
    ...
    def get_selling_recommendation(
        ...
        price_vs_avg = (
            ((current_price - avg_price_30d) / avg_price_30d) * 100
            if avg_price_30d > 0
            else 0
        )
...
class MarketService:
    ...
    async def get_markets(
        self, governorate: Optional[str] = None, market_type: Optional[str] = None
    ) -> List[Market]:
    ...
    async def record_price(self, data: PriceCreate) -> CropPrice:
        ...
        async with self.db.session() as session:
            ...
            if prev_price:
                price_change = (
                    (data.price_avg - prev_price.price_avg) / prev_price.price_avg
                ) * 100
                if price_change > 5:
            ...
            price_updates.labels(
                crop=str(data.crop_id)[:8], market=str(data.market_id)[:8]
            ).inc()
    ...
    async def get_latest_prices(
        self, crop_id: uuid.UUID, market_id: Optional[uuid.UUID] = None
    ) -> List[CropPrice]:
    ...
    async def get_market_summary(self, crop_id: uuid.UUID) -> MarketSummary:
        ...
        async with self.db.session() as session:
            # Get crop info
            crop_result = await session.execute(select(Crop).where(Crop.id == crop_id))
            ...
            async def get_price_change(days: int) -> Optional[float]:
                ...
                result = await session.execute(
                    select(func.avg(CropPrice.price_avg)).where(
                        CropPrice.crop_id == crop_id, CropPrice.price_date == old_date
                    )
                )
            ...
            trend = (
                max(set(trends), key=trends.count)
                if trends
                else PriceTrend.STABLE.value
            )
    ...
    async def create_price_alert(
        self, tenant_id: uuid.UUID, user_id: uuid.UUID, data: PriceAlertCreate
    ) -> PriceAlert:
        """Create price alert"""
        async with self.db.session() as session:
            alert = PriceAlert(tenant_id=tenant_id, user_id=user_id, **data.dict())
            session.add(alert)
            await session.commit()
    ...
    async def check_alerts(
        self, crop_id: uuid.UUID, market_id: uuid.UUID, current_price: float
    ):
        ...
        async with self.db.session() as session:
            result = await session.execute(
                select(PriceAlert).where(
                    ...
                    PriceAlert.is_active == True,
                    (PriceAlert.market_id == market_id)
                    | (PriceAlert.market_id.is_(None)),
                )
            )
            ...
            for alert in alerts:
                ...
                if (
                    ...
                    and current_price > alert.threshold_value
                ):
                    should_trigger = True
                elif (
...
async def get_markets(
    governorate: Optional[str] = None, market_type: Optional[str] = None
):
...
# Crops (Internal)
@app.post(
    "/internal/crops", response_model=CropResponse, status_code=status.HTTP_201_CREATED
)
...
async def get_prices(
    ...
    prices = await market_service.get_prices(
        crop_id, market_id, start_date, end_date, limit
    )
    return prices
...
async def create_alert(
    tenant_id: uuid.UUID, user_id: uuid.UUID, data: PriceAlertCreate
):
