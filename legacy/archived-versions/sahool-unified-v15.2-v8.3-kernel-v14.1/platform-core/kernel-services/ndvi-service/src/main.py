...
class SatelliteImageryProvider:
    ...
    async def get_ndvi_for_field(
        ...
        try:
            ...
            ndvi_std = math.sqrt(
                sum((x - ndvi_mean) ** 2 for x in ndvi_values) / len(ndvi_values)
            )
    ...
    async def get_ndvi_timeseries(
        self, field_id: str, bbox: Tuple[float, float, float, float], days: int = 30
    ) -> List[dict]:
        """
        ...
        for i in range(days // 5):  # Every 5 days (Sentinel-2 revisit)
            ...
            timeseries.append({"date": date.isoformat(), "ndvi": round(ndvi, 3)})
...
class NDVIAnomalyDetector:
    ...
    def detect(
        self, current: NDVIReading, history: List[dict] = None, baseline: float = None
    ) -> List[NDVIAnomaly]:
        """
...
class NDVIService:
    ...
    async def start(self):
        ...
        self.scheduler.add_job(
            self._check_new_imagery, CronTrigger(hour=8, minute=0), id="imagery_check"
        )
    ...
    async def _analyze_field(self, field_id: str, field: dict):
        ...
        try:
            ...
            reading = await self.provider.get_ndvi_for_field(
                field_id=field_id, bbox=field["bbox"]
            )
            ...
            history = await self.provider.get_ndvi_timeseries(
                field_id=field_id, bbox=field["bbox"], days=30
            )
    ...
    async def _publish_ndvi_processed(
        self, field_id: str, field: dict, reading: NDVIReading, history: List[dict]
    ):
        """Publish NDVI processed event"""
        event = create_event(
    ...
    async def _publish_anomaly(
        self, field_id: str, field: dict, anomaly: NDVIAnomaly, reading: NDVIReading
    ):
    ...
    def _get_recommendations(self, reading: NDVIReading) -> List[dict]:
        ...
        if reading.vegetation_health == VegetationHealth.STRESSED:
            recommendations.append(
                {"action_ar": "فحص الري والتسميد", "priority": "high"}
            )
        elif reading.vegetation_health == VegetationHealth.DEAD:
            recommendations.append(
                {"action_ar": "فحص ميداني عاجل", "priority": "urgent"}
            )
        ...
        if reading.ndvi_std > 0.12:
            recommendations.append(
                {"action_ar": "توحيد المعاملات الزراعية", "priority": "medium"}
            )
