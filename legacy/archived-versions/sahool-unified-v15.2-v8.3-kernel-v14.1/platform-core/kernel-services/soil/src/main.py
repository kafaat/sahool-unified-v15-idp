...
class Settings:
    ...
    DATABASE_URL = os.getenv(
        "DATABASE_URL", "postgresql+asyncpg://sahool:sahool@postgres:5432/sahool_soil"
    )
...
class SoilSample(DBBaseModel):
    ...
    recommendations = relationship(
        "SoilRecommendation", back_populates="sample", cascade="all, delete-orphan"
    )
...
class SoilRecommendation(DBBaseModel):
    ...
    )  # fertilizer, amendment, irrigation, crop_selection
    ...
    def analyze(self, sample: SoilSample) -> SoilAnalysisResult:
        ...
        if sample.ph:
            ...
            if ph_status.status in ["very_acidic", "acidic"]:
                issues.append(
                    {"nutrient": "pH", "status": ph_status.status, "value": sample.ph}
                )
                ...
                recommendations.append(
                    {
                        "category": "amendment",
                        "priority": (
                            "high" if ph_status.status == "very_acidic" else "medium"
                        ),
                        **rec,
                    }
                )
                ...
                recommendations.append(
                    {
                        "category": "amendment",
                        "priority": (
                            "high" if ph_status.status == "very_alkaline" else "medium"
                        ),
                        **rec,
                    }
                )
        ...
        if sample.organic_matter_percent:
            om_status = self._get_nutrient_status(
                ...
                issues.append(
                    {
                        ...
                        "status": om_status.status,
                        "value": sample.organic_matter_percent,
                    }
                )
                ...
                recommendations.append(
                    {
                        "category": "amendment",
                        "priority": (
                            "high" if om_status.status == "very_low" else "medium"
                        ),
                        **rec,
                    }
                )
                ...
                issues.append(
                    {
                        ...
                        "value": sample.nitrogen_available,
                    }
                )
                ...
                recommendations.append(
                    {
                        "category": "fertilizer",
                        "priority": (
                            "high" if n_status.status == "deficient" else "medium"
                        ),
                        **rec,
                    }
                )
                ...
                recommendations.append(
                    {
                        "category": "fertilizer",
                        "priority": (
                            "high" if p_status.status == "deficient" else "medium"
                        ),
                        **rec,
                    }
                )
                ...
                recommendations.append(
                    {
                        "category": "fertilizer",
                        "priority": (
                            "high" if k_status.status == "deficient" else "medium"
                        ),
                        **rec,
                    }
                )
        ...
        return SoilAnalysisResult(
            ...
            ph_status=(
                self._get_nutrient_status("ph", sample.ph).status
                if sample.ph
                else "unknown"
            ),
            ...
            nitrogen_status=(
                self._get_nutrient_status("nitrogen", sample.nitrogen_available).status
                if sample.nitrogen_available
                else "unknown"
            ),
            phosphorus_status=(
                self._get_nutrient_status("phosphorus", sample.phosphorus).status
                if sample.phosphorus
                else "unknown"
            ),
            potassium_status=(
                self._get_nutrient_status("potassium", sample.potassium).status
                if sample.potassium
                else "unknown"
            ),
            issues=issues,
...
class SoilService:
    ...
    async def create_sample(
        self, tenant_id: uuid.UUID, data: SoilSampleCreate
    ) -> SoilSample:
        """Create soil sample and analyze"""
        async with self.db.session() as session:
            sample = SoilSample(tenant_id=tenant_id, **data.dict())
    ...
    async def get_sample(
        self, sample_id: uuid.UUID, tenant_id: uuid.UUID
    ) -> Optional[SoilSample]:
        """Get soil sample by ID"""
        async with self.db.session() as session:
            result = await session.execute(
                select(SoilSample).where(
                    SoilSample.id == sample_id, SoilSample.tenant_id == tenant_id
                )
            )
            return result.scalar_one_or_none()

    async def get_field_samples(
        self, field_id: uuid.UUID, tenant_id: uuid.UUID, limit: int = 10
    ) -> List[SoilSample]:
        ...
        async with self.db.session() as session:
            result = await session.execute(
                select(SoilSample)
                .where(
                    SoilSample.field_id == field_id, SoilSample.tenant_id == tenant_id
                )
    ...
    async def record_sensor_reading(
        self, tenant_id: uuid.UUID, data: SensorReadingCreate
    ) -> SoilSensorReading:
    ...
    async def calculate_fertility_trend(
        ...
        async with self.db.session() as session:
            ...
            om_values = [
                s.organic_matter_percent for s in samples if s.organic_matter_percent
            ]
            ...
            trend = SoilFertilityTrend(
                ...
                avg_organic_matter=(
                    sum(om_values) / len(om_values) if om_values else None
                ),
...
@app.post(
    "/internal/samples",
    response_model=SoilSampleResponse,
...
@app.get(
    "/internal/fields/{field_id}/sensor-readings",
...
async def get_sensor_readings(
    ...
    readings = await soil_service.get_sensor_readings(
        field_id, tenant_id, sensor_id, hours
    )
    return readings


@app.get(
    "/internal/fields/{field_id}/fertility-trend", response_model=FertilityTrendResponse
)
