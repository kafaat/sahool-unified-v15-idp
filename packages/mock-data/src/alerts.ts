...
export type AlertType = 'pest' | 'disease' | 'weather' | 'irrigation' | 'harvest' | 'soil';
...
const alertMessages: Record<AlertType, { title: string; messages: string[] }> = {
  ...
  disease: {
    title: 'تنبيه أمراض',
    messages: ['ظهور أعراض مرضية على المحصول', 'انتشار فطري محتمل', 'يُنصح بفحص النباتات المصابة'],
  },
  weather: {
    title: 'تنبيه طقس',
    messages: ['توقعات بموجة حر شديدة', 'احتمال هطول أمطار غزيرة', 'رياح قوية متوقعة'],
  },
  irrigation: {
    title: 'تنبيه ري',
    messages: ['مستوى الرطوبة منخفض جداً', 'الحقل يحتاج للري الفوري', 'كفاءة نظام الري منخفضة'],
  },
  harvest: {
    title: 'تنبيه حصاد',
    messages: [
  ...
  soil: {
    title: 'تنبيه تربة',
    messages: ['مستوى المغذيات منخفض', 'درجة حموضة التربة غير مناسبة', 'يُنصح بتحليل التربة'],
  },
};
 ...
 */
export function generateMockAlert(overrides: Partial<MockAlert> = {}): MockAlert {
