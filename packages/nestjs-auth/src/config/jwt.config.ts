...
export class JWTConfig {
   ...
   */
  static readonly RATE_LIMIT_ENABLED: boolean = process.env.RATE_LIMIT_ENABLED !== 'false';
  ...
  static validate(): void {
    ...
    if (env === 'production' || env === 'staging') {
      ...
      if (!this.SECRET) {
        throw new Error('JWT_SECRET must be set in development');
      }
      ...
      if (!this.SECRET) {
        throw new Error('JWT_SECRET must not be empty, even in test environment');
      }
    }
  }
