...
abstract class _$AppDatabase extends GeneratedDatabase {
  ...
  late final Index tasksTenantIdx = Index(
      'tasks_tenant_idx', 'CREATE INDEX tasks_tenant_idx ON tasks (tenant_id)');
  late final Index tasksFieldIdx = Index(
      'tasks_field_idx', 'CREATE INDEX tasks_field_idx ON tasks (field_id)');
  late final Index tasksStatusIdx = Index(
      'tasks_status_idx', 'CREATE INDEX tasks_status_idx ON tasks (status)');
  late final Index tasksSyncedIdx = Index(
      'tasks_synced_idx', 'CREATE INDEX tasks_synced_idx ON tasks (synced)');
  late final Index tasksTenantStatusIdx = Index('tasks_tenant_status_idx',
      'CREATE INDEX tasks_tenant_status_idx ON tasks (tenant_id, status)');
  late final Index tasksCreatedIdx = Index('tasks_created_idx',
      'CREATE INDEX tasks_created_idx ON tasks (created_at)');
  late final Index outboxTenantIdx = Index('outbox_tenant_idx',
      'CREATE INDEX outbox_tenant_idx ON outbox (tenant_id)');
  late final Index outboxSyncedIdx = Index('outbox_synced_idx',
      'CREATE INDEX outbox_synced_idx ON outbox (is_synced)');
  late final Index outboxEntityIdx = Index('outbox_entity_idx',
      'CREATE INDEX outbox_entity_idx ON outbox (entity_type, entity_id)');
  late final Index outboxCreatedIdx = Index('outbox_created_idx',
      'CREATE INDEX outbox_created_idx ON outbox (created_at)');
  late final Index outboxTenantSyncedIdx = Index('outbox_tenant_synced_idx',
      'CREATE INDEX outbox_tenant_synced_idx ON outbox (tenant_id, is_synced)');
  late final Index fieldsTenantIdx = Index('fields_tenant_idx',
      'CREATE INDEX fields_tenant_idx ON fields (tenant_id)');
  late final Index fieldsFarmIdx = Index(
      'fields_farm_idx', 'CREATE INDEX fields_farm_idx ON fields (farm_id)');
  late final Index fieldsSyncedIdx = Index(
      'fields_synced_idx', 'CREATE INDEX fields_synced_idx ON fields (synced)');
  late final Index fieldsDeletedIdx = Index('fields_deleted_idx',
      'CREATE INDEX fields_deleted_idx ON fields (is_deleted)');
  late final Index fieldsTenantDeletedIdx = Index('fields_tenant_deleted_idx',
      'CREATE INDEX fields_tenant_deleted_idx ON fields (tenant_id, is_deleted)');
  late final Index fieldsUpdatedIdx = Index('fields_updated_idx',
      'CREATE INDEX fields_updated_idx ON fields (updated_at)');
  late final Index fieldsRemoteIdx = Index('fields_remote_idx',
      'CREATE INDEX fields_remote_idx ON fields (remote_id)');
  ...
  late final Index syncEventsTenantReadIdx = Index(
      'sync_events_tenant_read_idx',
      'CREATE INDEX sync_events_tenant_read_idx ON sync_events (tenant_id, is_read)');
  ...
  List<DatabaseSchemaEntity> get allSchemaEntities => [
        ...
        tasksStatusIdx,
        tasksSyncedIdx,
        tasksTenantStatusIdx,
        tasksCreatedIdx,
        outboxTenantIdx,
        outboxSyncedIdx,
        outboxEntityIdx,
        outboxCreatedIdx,
        outboxTenantSyncedIdx,
        fieldsTenantIdx,
        fieldsFarmIdx,
        fieldsSyncedIdx,
        fieldsDeletedIdx,
        fieldsTenantDeletedIdx,
        fieldsUpdatedIdx,
        fieldsRemoteIdx,
        syncEventsTenantIdx,
        syncEventsReadIdx,
        syncEventsTenantReadIdx,
