 ...
 */
export type PackageTier = 'trial' | 'starter' | 'professional' | 'enterprise' | 'research';
 ...
 */
export function normalizeLegacyResponse<T>(legacy: LegacyApiResponse<T>): ApiResponse<T> {
  return {
 ...
 */
export function getServicesForTier(tier: PackageTier): Record<string, KongService> {
  return Object.fromEntries(
    Object.entries(KONG_SERVICES).filter(([_, service]) => service.tier.includes(tier))
  );
}
