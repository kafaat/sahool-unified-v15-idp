...
class Settings:
    ...
    DATABASE_URL = os.getenv(
        "DATABASE_URL",
        "postgresql+asyncpg://sahool:sahool@postgres:5432/sahool_advisor",
    )
...
advisory_requests = Counter(
    "advisory_requests_total", "Advisory requests", ["advisory_type", "status"]
)
...
class Advisory(DBBaseModel):
    ...
    recommended_actions = Column(
        JSON, default=[]
    )  # [{action, description_ar, description_en, due_date}]
...
class AdvisoryEngine:
    ...
    def _load_knowledge_base(self):
        ...
        self.proverbs = {
            ...
            "harvesting": ["اجمع محصولك قبل أن يجمعه غيرك", "الحصاد في وقته بركة"],
        }
                    ...
                    "recommended_actions": [
                        {
                            ...
                            "description_ar": "زيادة الري في الصباح الباكر والمساء",
                        },
                        {
                            "action": "shade_crops",
                            "description_ar": "توفير الظل للمحاصيل الحساسة",
                        },
                    ],
                    ...
                    "recommended_actions": [
                        {
                            ...
                            "description_ar": "تقليل الري لتجنب الإفراط في الماء",
                        },
                        {
                            "action": "check_drainage",
                            "description_ar": "تفقد نظام الصرف",
                        },
                    ],
                }
            )
                    ...
                    "recommended_actions": [
                        {
                            ...
                            "description_ar": "فحص الحقل للتحقق من المشاكل",
                        },
                        {
                            "action": "check_irrigation",
                            "description_ar": "التحقق من نظام الري",
                        },
                        {
                            "action": "soil_test",
                            "description_ar": "إجراء اختبار للتربة",
                        },
                    ],
                }
            )
                ...
                advisories.append(
                    {
                        ...
                        "traditional_practice": current_star.get(
                            "agricultural_advice_ar"
                        ),
                        "related_proverb": (
                            self.proverbs["planting"][0]
                            if self.proverbs["planting"]
                            else None
                        ),
                        "action_window_start": datetime.utcnow(),
                        "action_window_end": datetime.utcnow()
                        + timedelta(days=calendar.get("days_remaining", 7)),
                    }
                )
    ...
    async def chat_response(
        self, message: str, context: Dict[str, Any], language: str = "ar"
    ) -> Dict[str, Any]:
        ...
        if any(word in message_lower for word in ["ري", "irrigation", "water", "ماء"]):
            response["message_ar"] = (
                "بناءً على بيانات الطقس والتربة المتاحة، أنصح بالري في الصباح الباكر لتقليل التبخر. تأكد من أن التربة رطبة حتى عمق 15-20 سم."
            )
            response["message"] = (
                "Based on available weather and soil data, I recommend irrigation early morning to reduce evaporation. Ensure soil is moist to 15-20cm depth."
            )
            ...
            response["message_ar"] = (
                "للوقاية من الأمراض، تأكد من التهوية الجيدة بين النباتات وتجنب الري العلوي. إذا لاحظت أعراضاً، أرسل صورة للتشخيص."
            )
            response["message"] = (
                "For disease prevention, ensure good ventilation between plants and avoid overhead irrigation. If you notice symptoms, send an image for diagnosis."
            )
            response["suggested_actions"] = ["upload_image", "view_disease_guide"]

        elif any(
            word in message_lower for word in ["زراعة", "planting", "plant", "زرع"]
        ):
            response["message_ar"] = (
                "أفضل وقت للزراعة يعتمد على نوع المحصول والظروف المناخية. حسب التقويم الفلكي اليمني، الفترة الحالية مناسبة لـ..."
            )
            response["message"] = (
                "Best planting time depends on crop type and climate conditions. According to the Yemeni astronomical calendar, the current period is suitable for..."
            )
            response["suggested_actions"] = ["view_calendar", "check_crop_guide"]

        else:
            response["message_ar"] = (
                "شكراً على سؤالك. يمكنني مساعدتك في الري، مكافحة الأمراض، أوقات الزراعة، وتوصيات المحاصيل. كيف يمكنني مساعدتك؟"
            )
            response["message"] = (
                "Thank you for your question. I can help with irrigation, disease control, planting times, and crop recommendations. How can I help you?"
            )
...
class AdvisorService:
    ...
    async def get_advisory(
        self, advisory_id: uuid.UUID, tenant_id: Optional[uuid.UUID] = None
    ) -> Optional[Advisory]:
        """Get advisory by ID"""
        async with self.db.session() as session:
    ...
    async def list_advisories(
        ...
        async with self.db.session() as session:
            ...
            count_query = select(func.count(Advisory.id)).where(
            ...
            if status_filter:
                ...
                count_query = count_query.where(
                    Advisory.status != AdvisoryStatus.EXPIRED.value
                )
            ...
            query = (
                query.offset(offset)
                .limit(page_size)
                .order_by(Advisory.priority.desc(), Advisory.created_at.desc())
            )
    ...
    async def acknowledge_advisory(
        self, advisory_id: uuid.UUID, tenant_id: uuid.UUID, user_id: uuid.UUID
    ) -> Advisory:
        """Acknowledge advisory"""
        async with self.db.session() as session:
            result = await session.execute(
                select(Advisory).where(
                    Advisory.id == advisory_id, Advisory.tenant_id == tenant_id
                )
            )
    ...
    async def complete_advisory(
        self, advisory_id: uuid.UUID, tenant_id: uuid.UUID
    ) -> Advisory:
        ...
        async with self.db.session() as session:
            ...
            await self.event_bus.publish(
                ...
                {"advisory_id": str(advisory_id), "tenant_id": str(tenant_id)},
            )
    ...
    async def generate_advisories(
        self, request: AdvisoryGenerateRequest
    ) -> List[Advisory]:
        """Generate advisories based on context"""
        ...
        if request.include_ndvi:
            context["ndvi_data"] = {"ndvi_mean": 0.45, "ndvi_trend": "stable"}
        ...
        # Generate advisories
        advisory_data_list = await self.engine.generate_advisory(
        ...
        for advisory_data in advisory_data_list:
            advisory_create = AdvisoryCreate(
                tenant_id=request.tenant_id, field_id=request.field_id, **advisory_data
            )
        ...
        async with self.db.session() as session:
            ...
            if session_id:
                ...
                chat_session = ConversationSession(
                    tenant_id=tenant_id, user_id=user_id, field_id=field_id
                )
    ...
    async def get_dashboard_summary(self, tenant_id: uuid.UUID) -> Dict[str, Any]:
        ...
        async with self.db.session() as session:
            # Count by priority
            result = await session.execute(
                select(Advisory.priority, func.count(Advisory.id))
                .where(
                    Advisory.tenant_id == tenant_id,
            ...
            # Count by category
            result = await session.execute(
                select(Advisory.category, func.count(Advisory.id))
                .where(
                    Advisory.tenant_id == tenant_id,
                    Advisory.status == AdvisoryStatus.ACTIVE.value,
                )
...
async def lifespan(app: FastAPI):
    ...
    await event_bus.subscribe(
        "weather.forecast.received", lambda e: handle_weather_event(e, advisor_service)
    )
    await event_bus.subscribe(
        "ndvi.analysis.completed", lambda e: handle_ndvi_event(e, advisor_service)
    )
    await event_bus.subscribe(
        "disease.risk.assessed", lambda e: handle_disease_risk_event(e, advisor_service)
    )
...
# Advisory Actions
@app.post(
    "/api/v1/advisories/{advisory_id}/acknowledge", response_model=AdvisoryResponse
)
async def acknowledge_advisory(
    advisory_id: uuid.UUID, tenant_id: uuid.UUID, user_id: uuid.UUID
):
    """Acknowledge advisory"""
...
async def submit_feedback(
    advisory_id: uuid.UUID, tenant_id: uuid.UUID, data: FeedbackRequest
):
    """Submit feedback on advisory"""
    return await advisor_service.submit_feedback(
        advisory_id, tenant_id, data.feedback, data.notes
    )
...
async def get_advisory_categories():
    ...
    return {
        "categories": [
            ...
            {
                "code": "disease_management",
                "name_en": "Disease Management",
