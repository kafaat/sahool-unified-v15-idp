...
STAR_CROP_ACTIONS = {
    "star_alab": {
        "sorghum": {"action": ActionType.SOIL_PREP, "description_ar": "حرث الجنيد استعداداً للزراعة", "proverb": "ثورك والعلب"},
        "wheat": {"action": ActionType.MONITORING, "description_ar": "مراقبة نمو القمح", "proverb": None}
    },
    "star_saal": {
        "wheat": {"action": ActionType.PLANTING, "description_ar": "بدء زراعة القمح", "proverb": "سعل الضان والخضار"}
    },
    "star_batn_hoot": {
        "coffee": {"action": ActionType.HARVESTING, "description_ar": "بدء موسم حصاد البن", "proverb": None}
    },
    "star_sharatain": {
        "vegetables": {"action": ActionType.PLANTING, "description_ar": "زراعة الخضروات الشتوية", "proverb": "الشرط زراعة"}
    }
}
...
class CropDecisionEngine:
        ...
        for planting in affected_fields:
            if anomaly_type == "drought":
                recommendations.append(CropRecommendation(
                    field_id=planting.field_id, crop_type=planting.crop_type,
                    action=ActionType.IRRIGATION,
                    priority=ActionPriority.URGENT if severity in ["high", "critical"] else ActionPriority.HIGH,
    ...
    def process_ndvi_anomaly(self, event_data: dict) -> List[CropRecommendation]:
        ...
        if anomaly_type == "water_stress":
            recommendations.append(CropRecommendation(
                field_id=field_id, crop_type=planting.crop_type,
                action=ActionType.IRRIGATION,
                priority=ActionPriority.URGENT if severity == "critical" else ActionPriority.HIGH,
...
class CropLifecycleService:
    ...
    def _add_sample_plantings(self):
        samples = [
            CropPlanting("field_001", "wheat", "محلي", datetime.utcnow() - timedelta(days=45),
                        CropStage.VEGETATIVE, 15, datetime.utcnow() + timedelta(days=90), 0.75, datetime.utcnow(), "default"),
            CropPlanting("field_002", "coffee", "بني يمني", datetime.utcnow() - timedelta(days=365),
                        CropStage.FRUITING, 60, datetime.utcnow() + timedelta(days=60), 0.85, datetime.utcnow(), "default"),
            CropPlanting("field_003", "sorghum", "ذرة بيضاء", datetime.utcnow() - timedelta(days=30),
                        CropStage.GERMINATION, 10, datetime.utcnow() + timedelta(days=120), 0.80, datetime.utcnow(), "default"),
        ]
        for p in samples:
    ...
    async def _handle_event(self, msg, subject: str):
        try:
            ...
            if event_type == EventTypes.ASTRO_STAR_RISING:
                ...
                recommendations = self.decision_engine.process_weather_anomaly(event_data)
            ...
            for rec in recommendations:
                await self._publish_recommendation(rec, event_data.get("correlation_id"))
    ...
    async def _publish_recommendation(self, rec: CropRecommendation, correlation_id: str = None):
        event = create_event(
        ...
        EVENTS_PUBLISHED.labels(service=SERVICE_NAME, event_type=EventTypes.CROP_ACTION_RECOMMENDED, tenant_id="default").inc()
        event_logger.published(EventTypes.CROP_ACTION_RECOMMENDED, event_id=event["event_id"])
        logger.info("recommendation_published", field_id=rec.field_id, action=rec.action.value)
...
@app.get("/readyz")
async def ready():
    return {"status": "ready" if crop_service.nc and crop_service.nc.is_connected else "not_ready"}
...
async def list_plantings():
    return {"plantings": [p.to_dict() for p in crop_service.state_manager.plantings.values()]}
...
async def add_planting(request: AddPlantingRequest):
    planting = CropPlanting(
        request.field_id, request.crop_type, request.variety,
        datetime.utcnow(), CropStage.SEED, 0,
        datetime.utcnow() + timedelta(days=120), 1.0, datetime.utcnow(), request.tenant_id
    )
...
if __name__ == "__main__":
    uvicorn.run("main:app", host="0.0.0.0", port=int(os.getenv("CROP_LIFECYCLE_PORT", "8086")), reload=os.getenv("ENV") == "development")
