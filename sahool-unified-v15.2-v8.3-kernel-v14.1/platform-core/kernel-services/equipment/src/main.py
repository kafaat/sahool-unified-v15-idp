...
class Settings:
    ...
    DATABASE_URL = os.getenv(
        "DATABASE_URL",
        "postgresql+asyncpg://sahool:sahool@postgres:5432/sahool_equipment",
    )
...
equipment_operations = Counter(
    "equipment_operations_total", "Equipment operations", ["operation", "status"]
)
...
class Equipment(DBBaseModel):
    ...
    specifications = Column(
        JSON, default={}
    )  # {engine_power, fuel_type, capacity, etc.}
...
class MaintenanceRecord(DBBaseModel):
    ...
    parts_replaced = Column(
        JSON, default=[]
    )  # [{part_name, part_number, quantity, cost}]
...
class EquipmentUsageLog(DBBaseModel):
    ...
    )  # plowing, spraying, harvesting, etc.
    description = Column(Text, nullable=True)
...
class EquipmentService:
    ...
    async def _check_maintenance_due(self):
        ...
        async with self.db.session() as session:
                    ...
                    and_(
                        ...
                        or_(
                            ...
                            and_(
                                ...
                                >= Equipment.last_maintenance_hours
                                + Equipment.maintenance_interval_hours
                                - 10,
                            ),
                        ),
                    )
                ...
                await self.event_bus.publish(
                    "equipment.maintenance.due",
                    {
                        ...
                        "next_maintenance_due": (
                            equip.next_maintenance_due.isoformat()
                            if equip.next_maintenance_due
                            else None
                        ),
            ...
            logger.info(
                f"Maintenance check complete. {len(equipment_list)} equipment items need attention."
            )
    ...
    async def create_equipment(self, data: EquipmentCreate) -> Equipment:
        ...
        async with self.db.session() as session:
            ...
            if data.maintenance_interval_days:
                equipment.next_maintenance_due = date.today() + timedelta(
                    days=data.maintenance_interval_days
                )
            ...
            logger.info(
                "Equipment created", equipment_id=str(equipment.id), name=equipment.name
            )
            return equipment

    async def get_equipment(
        self, equipment_id: uuid.UUID, tenant_id: Optional[uuid.UUID] = None
    ) -> Optional[Equipment]:
        """Get equipment by ID"""
        async with self.db.session() as session:
    ...
    async def update_equipment(
        self, equipment_id: uuid.UUID, tenant_id: uuid.UUID, data: EquipmentUpdate
    ) -> Equipment:
        """Update equipment"""
        async with self.db.session() as session:
            result = await session.execute(
                select(Equipment).where(
                    Equipment.id == equipment_id, Equipment.tenant_id == tenant_id
                )
            )
    ...
    async def start_usage_session(
        self, equipment_id: uuid.UUID, tenant_id: uuid.UUID, data: UsageLogCreate
    ) -> EquipmentUsageLog:
        ...
        async with self.db.session() as session:
            ...
            if equipment.status not in [EquipmentStatus.OPERATIONAL.value]:
                raise HTTPException(
                    status_code=400,
                    detail=f"Equipment is {equipment.status}, cannot start usage",
                )
    ...
    async def end_usage_session(
        self, usage_log_id: uuid.UUID, tenant_id: uuid.UUID, data: UsageLogEnd
    ) -> EquipmentUsageLog:
        ...
        async with self.db.session() as session:
            ...
            await self.event_bus.publish(
                "equipment.maintenance.created",
                {
                    ...
                    "maintenance_type": data.maintenance_type,
                    "scheduled_date": (
                        data.scheduled_date.isoformat() if data.scheduled_date else None
                    ),
                },
            )
    ...
    async def complete_maintenance(
        self, maintenance_id: uuid.UUID, tenant_id: uuid.UUID, data: MaintenanceUpdate
    ) -> MaintenanceRecord:
        ...
        async with self.db.session() as session:
            ...
            maintenance.total_cost = (maintenance.labor_cost or 0) + (
    ...
    async def get_maintenance_history(
        self, equipment_id: uuid.UUID, tenant_id: uuid.UUID, limit: int = 50
        ...
        async with self.db.session() as session:
            result = await session.execute(
                select(
                    ...
                    .label("operational"),
                    func.count(Equipment.id)
                    .filter(Equipment.status == "in_use")
                    .label("in_use"),
                    func.count(Equipment.id)
                    .filter(Equipment.status == "maintenance")
...
async def update_equipment(
    equipment_id: uuid.UUID, tenant_id: uuid.UUID, data: EquipmentUpdate
):
    """Update equipment"""
...
# Location
@app.post(
    "/api/v1/equipment/{equipment_id}/location", status_code=status.HTTP_204_NO_CONTENT
)
async def update_location(
    equipment_id: uuid.UUID, tenant_id: uuid.UUID, data: LocationUpdate
):
    """Update equipment location (from GPS/IoT)"""
    await equipment_service.update_location(
        equipment_id, tenant_id, data.latitude, data.longitude
    )


# Usage Sessions
@app.post(
    "/api/v1/equipment/{equipment_id}/usage/start", response_model=UsageLogResponse
)
async def start_usage(
    equipment_id: uuid.UUID, tenant_id: uuid.UUID, data: UsageLogCreate
):
    """Start equipment usage session"""
...
@app.get(
    "/api/v1/equipment/{equipment_id}/usage", response_model=List[UsageLogResponse]
)
async def get_usage_history(
    equipment_id: uuid.UUID, tenant_id: uuid.UUID, limit: int = Query(50, ge=1, le=200)
):
    """Get equipment usage history"""
...
async def create_maintenance(
    ...
    """Create maintenance record"""
    return await equipment_service.create_maintenance(
        equipment_id, tenant_id, data, created_by
    )


@app.patch(
    "/api/v1/equipment/maintenance/{maintenance_id}", response_model=MaintenanceResponse
)
async def update_maintenance(
    maintenance_id: uuid.UUID, tenant_id: uuid.UUID, data: MaintenanceUpdate
):
...
async def get_maintenance_history(
    ...
    """Get equipment maintenance history"""
    return await equipment_service.get_maintenance_history(
        equipment_id, tenant_id, limit
    )
...
async def get_equipment_types():
    ...
    return {
        "types": [
            ...
            {
                "code": "sensor_station",
                "name_en": "Sensor Station",
                "name_ar": "محطة استشعار",
            },
