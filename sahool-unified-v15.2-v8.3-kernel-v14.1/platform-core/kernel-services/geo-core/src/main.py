...
class Settings:
    ...
    DATABASE_URL = os.getenv(
        "DATABASE_URL", "postgresql+asyncpg://sahool:sahool@postgres:5432/sahool_geo"
    )
    NATS_URL = os.getenv("NATS_URL", "nats://nats:4222")

    # Default SRID for Yemen
    DEFAULT_SRID = 4326  # WGS84
    YEMEN_BBOX = {"min_lat": 12.0, "max_lat": 19.0, "min_lon": 42.0, "max_lon": 55.0}
...
geo_operations = Counter(
    "geo_operations_total", "Geo operations", ["operation", "status"]
)
...
class Field(DBBaseModel):
    ...
    # Relationships
    zones = relationship(
        "FieldZone", back_populates="field", cascade="all, delete-orphan"
    )
    sensors = relationship(
        "FieldSensor", back_populates="field", cascade="all, delete-orphan"
    )
...
class FieldSensor(DBBaseModel):
    ...
    )  # soil_moisture, temperature, weather_station
...
class FieldHistory(DBBaseModel):
    ...
    change_type = Column(
        String(50), nullable=False
    )  # created, updated, boundary_changed, crop_changed
...
class GeoService:
    ...
    async def create_field(self, tenant_id: uuid.UUID, data: FieldCreate) -> Field:
        ...
        async with self.db.session() as session:
            ...
            logger.info(
                "Field created", field_id=str(field.id), area=field.area_hectares
            )
            return field

    async def get_field(
        self, field_id: uuid.UUID, tenant_id: uuid.UUID
    ) -> Optional[Field]:
        """Get field by ID"""
        async with self.db.session() as session:
    ...
    async def delete_field(self, field_id: uuid.UUID, tenant_id: uuid.UUID):
        ...
        async with self.db.session() as session:
            ...
            await self.event_bus.publish(
                "geo.field.deleted",
                {"field_id": str(field_id), "tenant_id": str(tenant_id)},
            )
    ...
    async def list_fields(
        ...
        async with self.db.session() as session:
            ...
            count_query = select(func.count(Field.id)).where(
                Field.tenant_id == tenant_id
            )
            area_query = select(func.sum(Field.area_hectares)).where(
                Field.tenant_id == tenant_id
            )
            ...
            query = (
                query.offset(offset).limit(page_size).order_by(Field.created_at.desc())
            )
    ...
    async def find_fields_by_point(
        self, tenant_id: uuid.UUID, point: GeoJSONPoint, buffer_meters: float = 0
    ) -> List[Field]:
        ...
        async with self.db.session() as session:
            ...
            if buffer_meters > 0:
                ...
                query = select(Field).where(
                    Field.tenant_id == tenant_id,
                    func.ST_Contains(
                        Field.boundary, func.ST_GeomFromText(point_wkt, 4326)
                    ),
                )
    ...
    async def find_fields_intersecting(
        self, tenant_id: uuid.UUID, polygon: GeoJSONPolygon
    ) -> List[Field]:
        ...
        async with self.db.session() as session:
            ...
            query = select(Field).where(
                Field.tenant_id == tenant_id,
                func.ST_Intersects(
                    Field.boundary, func.ST_GeomFromText(polygon_geom.wkt, 4326)
                ),
            )
    ...
    async def get_statistics(self, tenant_id: uuid.UUID) -> FieldStatistics:
        ...
        async with self.db.session() as session:
            ...
            result = await session.execute(
                select(func.count(Field.id), func.sum(Field.area_hectares)).where(
                    Field.tenant_id == tenant_id
                )
            )
            ...
            crop_result = await session.execute(
                select(Field.current_crop_name, func.count(Field.id))
                .where(
                    Field.tenant_id == tenant_id, Field.current_crop_name.isnot(None)
                )
            ...
            soil_result = await session.execute(
                select(Field.soil_type, func.count(Field.id))
                .where(Field.tenant_id == tenant_id)
                .group_by(Field.soil_type)
            )
            by_soil = {row[0]: row[1] for row in soil_result}

            # By irrigation
            irrigation_result = await session.execute(
                select(Field.irrigation_type, func.count(Field.id))
                .where(Field.tenant_id == tenant_id)
                .group_by(Field.irrigation_type)
            )
            by_irrigation = {row[0]: row[1] for row in irrigation_result}

            # By governorate
            gov_result = await session.execute(
                select(Field.governorate, func.count(Field.id))
                .where(Field.tenant_id == tenant_id, Field.governorate.isnot(None))
    ...
    async def create_zone(
        self, field_id: uuid.UUID, tenant_id: uuid.UUID, data: ZoneCreate
    ) -> FieldZone:
    ...
    async def create_sensor(
        self, field_id: uuid.UUID, tenant_id: uuid.UUID, data: SensorCreate
    ) -> FieldSensor:
    ...
    def _field_to_response(self, field: Field) -> FieldResponse:
        ...
        return FieldResponse(
            ...
            centroid=(
                self._shapely_to_geojson(centroid_shape) if centroid_shape else None
            ),
            area_hectares=field.area_hectares,
...
# Field CRUD
@app.post(
    "/api/v1/fields", response_model=FieldResponse, status_code=status.HTTP_201_CREATED
)
...
async def find_by_point(
    tenant_id: uuid.UUID, point: GeoJSONPoint, buffer_meters: float = 0
):
    """Find fields containing or near a point"""
...
# Sensors
@app.post(
    "/api/v1/fields/{field_id}/sensors",
