...
class DiagnosisResult:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "treatment": {
                "ar": self.treatment_ar,
                "en": self.treatment_en
            },
            "prevention": {
                "ar": self.prevention_ar,
                "en": self.prevention_en
            }
        }
...
DISEASE_DATABASE = {
    "tomato_late_blight": DiagnosisResult(
        ...
        symptoms=["بقع مائية على الأوراق", "بقع بنية داكنة", "عفن أبيض على السطح السفلي"],
...
class ImageAnalyzer:
    ...
    def __init__(self):
        self.supported_crops = ["tomato", "wheat", "coffee", "grape", "banana", "potato"]
    ...
    def _rule_based_analysis(
        ...
        if (features["green_ratio"] > 0.4 and 
            features["yellow_ratio"] < 0.1 and
            features["dark_ratio"] < 0.05):
        ...
        if features["dark_ratio"] > 0.05 and features["yellow_ratio"] > 0.1:
            ...
            blight.confidence = min(0.9, features["dark_ratio"] * 5 + features["yellow_ratio"] * 2)
        ...
        if features["green_ratio"] < 0.3 and features["yellow_ratio"] > 0.2:
            ...
            deficiency.severity = self._calculate_severity(deficiency.affected_area_percent)
                ...
                if result.confidence > 0.6:  # Only high confidence
                    disease_event = create_event(
                        ...
                        payload={
                            ...
                            "crop_type": crop_type,
                            "field_id": field_id,
                            "requires_action": result.severity in [Severity.HIGH, Severity.CRITICAL]
                        },
                        tenant_id=tenant_id
                    )
