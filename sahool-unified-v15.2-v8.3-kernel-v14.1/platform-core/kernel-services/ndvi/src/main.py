...
class AnomalyDetector:
    ...
    def detect_anomalies(
        self, analysis: NDVIAnalysis, previous_ndvi: Optional[float] = None
    ) -> List[NDVIAnomaly]:
        ...
        baseline = self.baselines.get(
            baseline_key, {"expected": 0.55, "threshold": 0.15}
        )
        ...
        if analysis.ndvi_std > 0.2:
            # Find problem zones
            problem_zones = [
                z for z in analysis.zones if z["health"] in ["poor", "bare_soil"]
            ]
            if problem_zones:
                anomalies.append(
                    NDVIAnomaly(
                        ...
                        change_percent=(
                            (problem_zones[0]["ndvi"] - analysis.ndvi_mean)
                            / analysis.ndvi_mean
                        )
                        * 100,
...
class NDVIService:
    ...
    async def start_scheduler(self):
        ...
        self.scheduler.add_job(
            self.process_all_fields,
            CronTrigger(day="*/5", hour=10),
            id="ndvi_processing",
        )
    ...
    async def process_field(
        self, field: Dict
    ) -> Tuple[NDVIAnalysis, List[NDVIAnomaly]]:
            ...
            try:
                ...
                for anomaly in anomalies:
                    ...
                    await self.js.publish(
                        ...
                        payload=json.dumps(anomaly_event).encode(),
                    )
                ...
                logger.info(
                    "field_processed",
                    field_id=field["id"],
                    ndvi_mean=round(analysis.ndvi_mean, 3),
                    anomalies=len(anomalies),
                )

            except Exception as e:
                logger.error(
                    "field_processing_failed", field_id=field["id"], error=str(e)
                )
...
app = FastAPI(
    title="NDVI Service",
    description="SAHOOL - Satellite NDVI Signal Producer (Layer 2)",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def ready():
    return {
        "status": (
            "ready" if ndvi_service.nc and ndvi_service.nc.is_connected else "not_ready"
        )
    }
