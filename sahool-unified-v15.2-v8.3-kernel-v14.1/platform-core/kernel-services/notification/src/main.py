...
class Settings:
    ...
    DATABASE_URL = os.getenv("DATABASE_URL", "postgresql+asyncpg://sahool:sahool@postgres:5432/sahool_notifications")
...
notifications_sent = Counter('notifications_sent_total', 'Notifications sent', ['channel', 'status', 'template'])
notification_latency = Histogram('notification_latency_seconds', 'Notification delivery latency', ['channel'])
...
class NotificationService:
        ...
        if template:
            ...
            if channel == "email":
                if user_language == "ar" and template.email_subject_ar:
                    title = self.email_provider.render_template(template.email_subject_ar, variables)
                elif template.email_subject:
                    title = self.email_provider.render_template(template.email_subject, variables)
                
                if user_language == "ar" and template.email_body_html_ar:
                    body = self.email_provider.render_template(template.email_body_html_ar, variables)
                elif template.email_body_html:
                    body = self.email_provider.render_template(template.email_body_html, variables)
                
                channel_data["body_text"] = self.email_provider.render_template(
                    template.email_body_text_ar if user_language == "ar" else template.email_body_text,
                    variables
                ) if (template.email_body_text_ar or template.email_body_text) else ""
                
                recipient = user_email
                
            elif channel == "sms":
                if user_language == "ar" and template.sms_body_ar:
                    body = self.email_provider.render_template(template.sms_body_ar, variables)
                elif template.sms_body:
                    body = self.email_provider.render_template(template.sms_body, variables)
                title = ""
                recipient = user_phone
                
            elif channel == "push":
                if user_language == "ar" and template.push_title_ar:
                    title = self.email_provider.render_template(template.push_title_ar, variables)
                elif template.push_title:
                    title = self.email_provider.render_template(template.push_title, variables)
                
                if user_language == "ar" and template.push_body_ar:
                    body = self.email_provider.render_template(template.push_body_ar, variables)
                elif template.push_body:
                    body = self.email_provider.render_template(template.push_body, variables)
                ...
                if user_language == "ar" and template.in_app_title_ar:
                    title = self.email_provider.render_template(template.in_app_title_ar, variables)
                elif template.in_app_title:
                    title = self.email_provider.render_template(template.in_app_title, variables)
                
                if user_language == "ar" and template.in_app_body_ar:
                    body = self.email_provider.render_template(template.in_app_body_ar, variables)
                elif template.in_app_body:
                    body = self.email_provider.render_template(template.in_app_body, variables)
    ...
    async def _deliver_notification(self, notification: Notification):
        ...
        try:
            ...
            logger.error("Notification delivery failed", notification_id=str(notification.id), error=str(e))
    ...
    async def get_templates(self, event_type: Optional[str] = None) -> List[NotificationTemplate]:
    ...
    try:
        await service.send_notification(
            ...
            request=SendNotificationRequest(
                ...
                entity_type="alert",
                entity_id=uuid.UUID(event["alert_id"]) if event.get("alert_id") else None,
                priority="high" if event.get("severity") in ["high", "critical"] else "normal"
            )
        )
    except Exception as e:
...
async def lifespan(app: FastAPI):
    ...
    # Subscribe to events
    await event_bus.subscribe("alert.created", lambda e: handle_alert_created(e, notification_service))
    await event_bus.subscribe("task.assigned", lambda e: handle_task_assigned(e, notification_service))
