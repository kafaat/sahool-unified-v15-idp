...
async def list_saga_definitions():
    ...
    return {
        "sagas": [s.to_dict() for s in SAGA_DEFINITIONS.values()]
    }
...
async def trigger_saga(request: TriggerSagaRequest):
    ...
    await pm.js.publish(
        subject=first_step.trigger_event,
