...
class TaskService:
    ...
    async def _create_task_from_recommendation(self, event_data: dict):
        ...
        priority = priority_map.get(
            payload.get("priority", "medium"), TaskPriority.MEDIUM
        )
    ...
    async def _publish_task_overdue(self, task: Task):
        event = create_event(
            ...
            payload={
                ...
                "field_id": task.field_id,
                "due_date": task.due_date.isoformat(),
                "overdue_hours": (datetime.utcnow() - task.due_date).total_seconds()
                / 3600,
                "assignee_id": task.assignee_id,
            },
            tenant_id=task.tenant_id,
...
async def ready():
    return {
        "status": (
            "ready" if task_service.nc and task_service.nc.is_connected else "not_ready"
        )
    }
...
async def create_task(request: CreateTaskRequest):
    ...
    priority = (
        ...
        if request.priority in [p.value for p in TaskPriority]
