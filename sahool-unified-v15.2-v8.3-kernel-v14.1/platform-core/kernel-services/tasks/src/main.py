...
class Task:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "assignee": (
                {"id": self.assignee_id, "name": self.assignee_name}
                if self.assignee_id
                else None
            ),
...
class TaskManager:
    ...
    def create_task_from_recommendation(self, event: Dict) -> Task:
        ...
        TASKS_PENDING.labels(
            tenant_id=task.tenant_id, priority=task.priority.value
        ).inc()
    ...
    def complete_task(
        self, task_id: str, completed_by: str, notes: Optional[str] = None
    ) -> Optional[Task]:
        ...
        return sorted(
            ...
            key=lambda t: (
                (
                    ...
                    else 1 if t.priority == TaskPriority.HIGH else 2
                ),
                t.due_date,
            ),
        )
...
class TasksService:
    ...
    async def _publish_task_created(self, task: Task, source_event: Dict):
        ...
        await self.js.publish(
            subject=EventTypes.TASK_CREATED, payload=json.dumps(event).encode()
        )
    ...
    async def _publish_task_assigned(self, task: Task):
        ...
        await self.js.publish(
            subject=EventTypes.TASK_ASSIGNED, payload=json.dumps(event).encode()
        )
    ...
    async def _publish_task_completed(self, task: Task):
        ...
        await self.js.publish(
            subject=EventTypes.TASK_COMPLETED, payload=json.dumps(event).encode()
        )
    ...
    async def _publish_task_overdue(self, task: Task):
        ...
        await self.js.publish(
            subject=EventTypes.TASK_OVERDUE, payload=json.dumps(event).encode()
        )
...
app = FastAPI(
    title="Tasks Service",
    description="SAHOOL - Task Execution Service (Layer 4)",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def ready():
    return {
        "status": (
            "ready"
            if tasks_service.nc and tasks_service.nc.is_connected
            else "not_ready"
        )
    }
...
async def assign_task(task_id: str, request: AssignTaskRequest):
    """Assign task to user"""
    task = tasks_service.manager.assign_task(
        task_id, request.assignee_id, request.assignee_name
    )
    if not task:
        raise HTTPException(404, "Task not found")
...
async def complete_task(task_id: str, request: CompleteTaskRequest):
    """Mark task as completed"""
    task = tasks_service.manager.complete_task(
        task_id, request.completed_by, request.notes
    )
    if not task:
        raise HTTPException(404, "Task not found")
...
if __name__ == "__main__":
    uvicorn.run(
        "main:app",
        host="0.0.0.0",
        port=int(os.getenv("TASKS_PORT", "8089")),
