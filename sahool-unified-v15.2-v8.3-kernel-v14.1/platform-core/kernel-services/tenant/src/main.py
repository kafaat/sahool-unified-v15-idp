...
from sqlalchemy import Column, String, Boolean, DateTime, Text, JSON, ForeignKey, Integer, Float, Date, select, update, func
...
class Settings:
    ...
    DATABASE_URL = os.getenv("DATABASE_URL", "postgresql+asyncpg://sahool:sahool@postgres:5432/sahool_tenants")
    ...
    PLAN_LIMITS = {
        ...
        "professional": {
            ...
            "features": ["weather", "tasks", "alerts", "ndvi", "reports", "ai_advisor", "irrigation"]
        },
        "enterprise": {
...
class Tenant(DBBaseModel):
    ...
    organization_type = Column(String(50), default="farm")  # farm, cooperative, company, research
...
class TenantService:
    ...
    async def _create_default_features(self, session, tenant: Tenant):
        ...
        plan_config = settings.PLAN_LIMITS.get(tenant.plan, settings.PLAN_LIMITS["free"])
    ...
    async def check_limit(self, tenant_id: uuid.UUID, resource: str, requested: int = 1) -> bool:
    ...
    async def has_feature(self, tenant_id: uuid.UUID, feature_code: str) -> bool:
        ...
        async with self.db.session() as session:
            ...
            if tenant.custom_features:
                if "all" in tenant.custom_features or feature_code in tenant.custom_features:
                    return True
            
            # Check plan features
            plan_config = settings.PLAN_LIMITS.get(tenant.plan, settings.PLAN_LIMITS["free"])
            if "all" in plan_config["features"] or feature_code in plan_config["features"]:
                return True
    ...
    async def update_subscription(self, tenant_id: uuid.UUID, data: SubscriptionUpdate) -> Tenant:
...
@app.post("/api/v1/tenants/{tenant_id}/activate", status_code=status.HTTP_204_NO_CONTENT)
