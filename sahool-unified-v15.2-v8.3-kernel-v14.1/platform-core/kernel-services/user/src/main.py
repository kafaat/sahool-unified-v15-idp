...
class Settings:
    ...
    DATABASE_URL = os.getenv(
        "DATABASE_URL", "postgresql+asyncpg://sahool:sahool@postgres:5432/sahool_users"
    )
...
user_operations = Counter(
    "user_operations_total", "User operations", ["operation", "status"]
)
...
class User(DBBaseModel):
    ...
    assigned_fields = relationship(
        "UserFieldAssignment", back_populates="user", cascade="all, delete-orphan"
    )
...
class UserActivity(DBBaseModel):
    ...
    )  # login, task_completed, field_viewed, etc.
...
class UserNotificationPreference(DBBaseModel):
    ...
    )  # task.assigned, alert.created, etc.
...
class UserService:
    ...
    async def remove_field_assignment(self, user_id: uuid.UUID, field_id: uuid.UUID):
        ...
        async with self.db.session() as session:
            ...
            await self.event_bus.publish(
                ...
                {"user_id": str(user_id), "field_id": str(field_id)},
            )
    ...
    async def get_activities(
        self, user_id: uuid.UUID, limit: int = 50, activity_type: Optional[str] = None
    ) -> List[UserActivity]:
        """Get user activities"""
        async with self.db.session() as session:
    ...
    async def set_notification_preferences(
        self, user_id: uuid.UUID, preferences: List[NotificationPreferenceCreate]
    ):
...
async def lifespan(app: FastAPI):
    ...
    await event_bus.subscribe(
        "auth.user.registered", lambda e: handle_user_registered(e, user_service)
    )
...
@app.post(
    "/api/v1/users/{user_id}/fields",
    response_model=FieldAssignmentResponse,
    status_code=status.HTTP_201_CREATED,
)
async def assign_field_to_user(
    user_id: uuid.UUID, data: FieldAssignmentCreate, assigned_by: uuid.UUID = Query(...)
):
    """Assign field to user"""
...
@app.delete(
    "/api/v1/users/{user_id}/fields/{field_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
async def set_user_notification_preferences(
    user_id: uuid.UUID, preferences: List[NotificationPreferenceCreate]
):
