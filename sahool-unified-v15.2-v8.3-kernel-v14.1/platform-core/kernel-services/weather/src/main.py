...
YEMEN_THRESHOLDS = {
    "المرتفعات": {
        "temp_high": 38, "temp_low": 5,
        "humidity_high": 85, "humidity_low": 15,
        "wind_high": 50, "rain_heavy": 30
    },
    "تهامة": {
        "temp_high": 45, "temp_low": 18,
        "humidity_high": 95, "humidity_low": 20,
        "wind_high": 60, "rain_heavy": 50
    },
    "حضرموت": {
        "temp_high": 48, "temp_low": 10,
        "humidity_high": 80, "humidity_low": 10,
        "wind_high": 70, "rain_heavy": 25
    }
}

# Monitoring Locations
MONITORING_LOCATIONS = [
    {"lat": 15.3694, "lon": 44.1910, "region": "المرتفعات", "name": "صنعاء", "name_en": "Sanaa"},
    {"lat": 14.7979, "lon": 42.9540, "region": "تهامة", "name": "الحديدة", "name_en": "Hodeidah"},
    {"lat": 15.9543, "lon": 48.9902, "region": "حضرموت", "name": "المكلا", "name_en": "Mukalla"},
    {"lat": 13.5775, "lon": 44.0178, "region": "المرتفعات", "name": "تعز", "name_en": "Taiz"},
    {"lat": 12.7797, "lon": 45.0358, "region": "تهامة", "name": "عدن", "name_en": "Aden"},
    {"lat": 16.9389, "lon": 43.8650, "region": "المرتفعات", "name": "صعدة", "name_en": "Saada"},
    {"lat": 14.5425, "lon": 49.1260, "region": "حضرموت", "name": "سيئون", "name_en": "Seiyun"},
    {"lat": 13.9792, "lon": 44.1739, "region": "المرتفعات", "name": "إب", "name_en": "Ibb"},
]
...
class OpenMeteoClient:
    ...
    def parse_response(self, data: dict, location: dict) -> Optional[WeatherData]:
        ...
        return WeatherData(
            ...
            uv_index=round(daily.get("uv_index_max", [0])[0] if daily.get("uv_index_max") else 0, 1),
...
class AnomalyDetector:
    ...
    def detect(self, weather: WeatherData) -> List[WeatherAnomaly]:
        ...
        if weather.temperature_c > thresholds["temp_high"]:
            severity = "critical" if weather.temperature_c > thresholds["temp_high"] + 7 else "high"
...
class WeatherService:
    ...
    async def check_all_locations(self):
        ...
        for location in MONITORING_LOCATIONS:
            try:
                ...
                logger.error("location_check_failed", 
    ...
    async def publish_anomaly(self, weather: WeatherData, anomaly: WeatherAnomaly, location: dict):
            ...
            try:
                ...
                event = create_event(
                    ...
                    payload={
                        ...
                        "agricultural_impact": self._assess_agricultural_impact(weather),
                ...
                event_logger.published(EventTypes.WEATHER_DAILY_SUMMARY, location=location["name"])
                await asyncio.sleep(1)
                
            except Exception as e:
                logger.error("daily_summary_failed", location=location["name"], error=str(e))
    
    def _assess_agricultural_impact(self, weather: WeatherData) -> dict:
        """Assess agricultural impact of current weather"""
        return {
            "irrigation_needed": weather.precipitation_mm < 5 and weather.temperature_c > 25,
            "spray_conditions_good": (
                weather.wind_speed_kmh < 15 and
                weather.precipitation_mm == 0 and
            ...
            "disease_risk_high": weather.humidity_percent > 85 and weather.temperature_c > 20,
            "pollination_conditions": 20 < weather.temperature_c < 30 and weather.wind_speed_kmh < 20
        }
...
async def internal_status():
    ...
    return {
        ...
        "scheduler_running": weather_service.scheduler.running if weather_service.scheduler else False
    }
...
async def internal_trigger():
    ...
    return {"message": "Weather check triggered", "locations": len(MONITORING_LOCATIONS)}
