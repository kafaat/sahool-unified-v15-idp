...
def validate_registry(registry_path: Path = REGISTRY_PATH) -> list[str]:
    ...
    for spec_entry in registry.get("specs", []):
        ...
        for approved_operation in approved.get("operations", []):
            ...
            pagination_and_sorting_params = {"limit", "offset", "page", "sort", "sortBy", "order", "orderBy"}
            if approved_operation.get("requiresFiltering") and query_params <= pagination_and_sorting_params:
