    ...
    data: [
      {
        ...
        coordinates: [[44.191, 15.369], [44.195, 15.369], [44.195, 15.365], [44.191, 15.365]],
      },
      {
        id: 'field-002',
        ...
        coordinates: [[44.185, 15.369], [44.189, 15.369], [44.189, 15.365], [44.185, 15.365]],
      },
    ],
  },
    ...
    data: [
      {
        ...
        status: 'active',
        field_id: 'field-001',
        last_reading: { value: 42.5, unit: '%', timestamp: new Date().toISOString() },
      },
      {
        id: 'sensor-002',
        ...
        status: 'active',
        field_id: 'field-001',
        last_reading: { value: 28.3, unit: '°C', timestamp: new Date().toISOString() },
      },
      {
        id: 'sensor-003',
        ...
        status: 'active',
        field_id: 'field-001',
        last_reading: { value: 45, unit: '%', timestamp: new Date().toISOString() },
      },
    ],
  },
...
class APISimulator {
  ...
  private async simulateRequest(endpoint: string): Promise<SimulationResult> {
    ...
    const mockKey = Object.keys(mockResponses).find((key) => endpoint.startsWith(key));
  ...
  async runSimulation(): Promise<void> {
    ...
    for (const endpoint of endpoints) {
      ...
      console.log(`${color}${icon}\x1b[0m ${endpoint} (${result.latency_ms}ms)`);
    }
  ...
  private printSummary(): void {
    ...
    const avgLatency = this.results.reduce((acc, r) => acc + r.latency_ms, 0) / this.results.length;
  ...
  async testFullFlow(): Promise<void> {
    ...
    const flows = [
      { name: 'Login Flow', endpoints: ['/api/v1/auth/login', '/api/v1/auth/me'] },
      { name: 'Dashboard Load', endpoints: ['/api/v1/fields', '/api/v1/weather/current', '/api/v1/tasks'] },
      { name: 'Field Details', endpoints: ['/api/v1/fields', '/api/v1/ndvi', '/api/v1/sensors'] },
      { name: 'Irrigation Planning', endpoints: ['/api/v1/irrigation', '/api/v1/weather/forecast'] },
    ...
    for (const flow of flows) {
      ...
      console.log(`  \x1b[90mFlow completed in ${Date.now() - flowStart}ms\x1b[0m`);
    }
