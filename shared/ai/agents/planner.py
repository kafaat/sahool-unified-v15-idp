...
"""
    ...
    def _get_growth_stages(
        ...
        if crop == "wheat":
            return [
                {"stage": "germination", "stage_ar": "الإنبات", "date": (plant_date + timedelta(days=7)).strftime("%Y-%m-%d")},
                {"stage": "tillering", "stage_ar": "التفريع", "date": (plant_date + timedelta(days=30)).strftime("%Y-%m-%d")},
                {"stage": "stem_extension", "stage_ar": "استطالة الساق", "date": (plant_date + timedelta(days=60)).strftime("%Y-%m-%d")},
                {"stage": "heading", "stage_ar": "التسنبل", "date": (plant_date + timedelta(days=90)).strftime("%Y-%m-%d")},
                {"stage": "grain_fill", "stage_ar": "امتلاء الحبوب", "date": (plant_date + timedelta(days=120)).strftime("%Y-%m-%d")},
                {"stage": "maturity", "stage_ar": "النضج", "date": (plant_date + timedelta(days=150)).strftime("%Y-%m-%d")},
            ]
        return []
    ...
    async def _assess_risk_category(
        ...
        return RiskAssessment(
            ...
            risk_score=50,
            mitigation_strategies=[
                {"en": "Monitor and adapt", "ar": "المراقبة والتكيف"}
            ],
        )
