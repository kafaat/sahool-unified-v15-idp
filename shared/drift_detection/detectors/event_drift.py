...
class EventDriftDetector(BaseDriftDetector):
    ...
    async def _check_event_catalog(self) -> None:
        ...
        if isinstance(event_list, list):
            for event in event_list:
                ...
                # Validate category
                valid_categories = {
                    "field", "ndvi", "alert", "weather", "irrigation",
                    "crop_health", "yield", "system", "vision", "tenant",
                    "user", "equipment", "task", "billing", "notification",
                }
    ...
    async def _check_idempotency(self) -> None:
        ...
        for hf in handler_files:
            ...
            has_handler = any(pat in content for pat in [
                "subscribe", "on_message", "message_handler", "event_handler",
                "nats_handler", "jetstream",
            ])
            ...
            has_idempotency = any(pat in content for pat in [
                "idempotency", "event_id", "dedup", "already_processed",
                "processed_events", "upsert", "ON CONFLICT",
            ])
            ...
            for py_file in events_dir.rglob("*.py"):
                try:
                    ...
                    if any(pat in content for pat in [
                        "EventEnvelope", "event_envelope", "BaseEvent",
