...
SECRET_PATTERNS = [
    ...
    (re.compile(r'(?:aws_access_key_id|aws_secret_access_key)\s*=\s*["\'][^"\']+["\']', re.IGNORECASE), "AWS credentials"),
...
EXCLUDE_SECRET_PATHS = {
    "test", "tests", "fixtures", "mock", "example", "__pycache__",
    "node_modules", ".venv", "archive",
}
            ...
            for file_path in root.rglob(f"*{ext}"):
                ...
                for pattern, pattern_name in SECRET_PATTERNS:
                    ...
                    if matches:
                        ...
                        safe = any(safe_pat in content[:200] for safe_pat in [
                            "test", "example", "placeholder", "dummy",
                            "mock", "<", "your_", "change_me",
                        ])
                        if safe:
                            continue
    ...
    async def _check_auth_patterns(self) -> None:
        ...
        for src_dir in service_dirs:
            ...
            for api_file in api_files:
                ...
                # Check for routes
                has_routes = any(pat in content for pat in [
                    "@router.", "@app.", "@Controller", "router.get",
                ...
                # Check for auth
                has_auth = any(pat in content for pat in [
                    "get_current_user", "Depends(", "Guards(",
                    "@UseGuards", "AuthGuard", "JwtAuthGuard",
                    "authenticate", "verify_token", "require_auth",
                ])

                # Skip health/public endpoints
                is_public = any(pat in content for pat in [
                    "healthz", "readyz", "/public/", "login", "register",
                    "callback", "webhook",
                ])
