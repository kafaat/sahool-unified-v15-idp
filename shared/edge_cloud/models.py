    ...
    def validate_interval_range(cls, v: int, info) -> int:
        ...
        if info.data.get("min_interval") and v < info.data["min_interval"]:
            raise ValueError(
                ...
                "الحد الأقصى يجب أن يكون >= الحد الأدنى"
            )
        return v
...
class DeviceConfig(BaseModel):
    ...
    protocol: DeviceProtocol = Field(
        ..., description="Communication protocol | بروتوكول الاتصال"
    )

    manufacturer: DeviceManufacturer = Field(
        default=DeviceManufacturer.GENERIC,
        description="Device manufacturer | الشركة المصنعة"
    )

    model: str = Field(
        default="", description="Device model | طراز الجهاز"
    )

    sensor_types: list[SensorType] = Field(
        default_factory=list,
        description="Types of sensors on device | أنواع المستشعرات على الجهاز"
    )

    # Connection settings
    host: str = Field(
        default="", description="Device host/IP address | عنوان المضيف/IP"
    )

    port: int = Field(
        default=0, ge=0, le=65535, description="Connection port | منفذ الاتصال"
    )

    username: str = Field(
        default="", description="Authentication username | اسم المستخدم للمصادقة"
    )

    # Note: Password should be stored securely, not in config
    use_tls: bool = Field(
        default=True, description="Use TLS encryption | استخدام تشفير TLS"
    )

    # Sampling
    sampling_config: SamplingConfig = Field(
        default_factory=SamplingConfig,
        description="Sampling configuration | تكوين أخذ العينات"
    )
    ...
    zone_id: str = Field(
        default="", description="Field zone identifier | معرف منطقة الحقل"
    )

    # Metadata
    is_active: bool = Field(
        default=True, description="Whether device is active | هل الجهاز نشط"
    )
...
class SensorReading(BaseModel):
    ...
    id: UUID = Field(
        default_factory=uuid4, description="Unique reading ID | معرف القراءة الفريد"
    )

    device_id: str = Field(
        ..., description="Source device identifier | معرف الجهاز المصدر"
    )

    sensor_type: SensorType = Field(
        ..., description="Type of sensor | نوع المستشعر"
    )

    value: float = Field(
        ..., description="Sensor value | قيمة المستشعر"
    )

    unit: str = Field(
        default="", description="Unit of measurement | وحدة القياس"
    )
    ...
    quality: DataQuality = Field(
        ...
        description="Data quality assessment | تقييم جودة البيانات"
    )

    confidence: float = Field(
        default=1.0, ge=0.0, le=1.0,
        description="Confidence in reading (0-1) | الثقة في القراءة"
    )
...
class EdgeDecision(BaseModel):
    ...
    id: UUID = Field(
        default_factory=uuid4, description="Unique decision ID | معرف القرار الفريد"
    )

    gateway_id: str = Field(
        default="", description="Edge gateway identifier | معرف بوابة الحافة"
    )

    decision_type: DecisionType = Field(
        ..., description="Type of decision | نوع القرار"
    )
    ...
    offline_mode: bool = Field(
        default=False,
        description="Decision made while offline | القرار اتخذ في وضع عدم الاتصال"
    )
    ...
    confidence: float = Field(
        ...
        description="Confidence in decision (0-1) | الثقة في القرار"
    )

    priority: int = Field(
        default=5, ge=1, le=10,
        description="Decision priority (1=highest) | أولوية القرار"
    )
    ...
    executed_at: datetime | None = Field(
        None, description="Execution timestamp | وقت التنفيذ"
    )

    execution_result: str = Field(
        default="", description="Execution result | نتيجة التنفيذ"
    )
    ...
    synced_at: datetime | None = Field(
        None, description="Sync timestamp | وقت المزامنة"
    )
...
class CloudInference(BaseModel):
    ...
    model_name: str = Field(
        ..., description="Name of AI model used | اسم نموذج الذكاء الاصطناعي"
    )

    model_version: str = Field(
        default="1.0.0", description="Model version | إصدار النموذج"
    )

    prediction: str = Field(
        ..., description="Prediction result | نتيجة التنبؤ"
    )

    prediction_ar: str = Field(
        default="", description="Prediction (Arabic) | التنبؤ (عربي)"
    )

    confidence: float = Field(
        ..., ge=0.0, le=1.0,
        description="Prediction confidence (0-1) | ثقة التنبؤ"
    )
    ...
    class_probabilities: dict[str, float] = Field(
        ...
        description="Probability distribution over classes | توزيع الاحتمالات"
    )
    ...
    error_margin: float = Field(
        ...
        description="Estimated error margin | هامش الخطأ المقدر"
    )
    ...
    completed_at: datetime | None = Field(
        None, description="Completion timestamp | وقت الإكمال"
    )

    # Metadata
    farm_id: str = Field(
        default="", description="Farm identifier | معرف المزرعة"
    )

    field_id: str = Field(
        default="", description="Field identifier | معرف الحقل"
    )
...
class Recommendation(BaseModel):
    ...
    title: str = Field(
        ..., description="Recommendation title | عنوان التوصية"
    )

    title_ar: str = Field(
        default="", description="Title (Arabic) | العنوان (عربي)"
    )

    description: str = Field(
        ..., description="Detailed description | الوصف التفصيلي"
    )

    description_ar: str = Field(
        default="", description="Description (Arabic) | الوصف (عربي)"
    )

    category: str = Field(
        default="general", description="Recommendation category | فئة التوصية"
    )

    priority: int = Field(
        default=5, ge=1, le=10,
        description="Priority level (1=highest) | مستوى الأولوية"
    )
    ...
    suggested_action: str = Field(
        default="", description="Suggested action | الإجراء المقترح"
    )
    ...
    confidence: float = Field(
        default=0.8, ge=0.0, le=1.0,
        description="Confidence in recommendation | الثقة في التوصية"
    )
...
class PestDetection(BaseModel):
    ...
    id: UUID = Field(
        default_factory=uuid4, description="Unique detection ID | معرف الكشف الفريد"
    )

    pest_type: str = Field(
        ..., description="Type/name of pest | نوع/اسم الآفة"
    )

    pest_type_ar: str = Field(
        default="", description="Pest type (Arabic) | نوع الآفة (عربي)"
    )

    category: PestCategory = Field(
        ..., description="Pest category | فئة الآفة"
    )

    confidence: float = Field(
        ..., ge=0.0, le=1.0,
        description="Detection confidence | ثقة الكشف"
    )

    severity: str = Field(
        default="medium",
        description="Severity level: low/medium/high/critical | مستوى الشدة"
    )

    # Bounding box
    bbox_x: float | None = Field(
        None, description="Bounding box X coordinate | إحداثي X للمربع"
    )

    bbox_y: float | None = Field(
        None, description="Bounding box Y coordinate | إحداثي Y للمربع"
    )

    bbox_width: float | None = Field(
        None, description="Bounding box width | عرض المربع"
    )

    bbox_height: float | None = Field(
        None, description="Bounding box height | ارتفاع المربع"
    )

    # Context
    affected_area_percent: float | None = Field(
        None, ge=0, le=100,
        description="Percentage of affected area | نسبة المنطقة المتضررة"
    )
    ...
    image_id: str = Field(
        default="", description="Source image identifier | معرف الصورة المصدر"
    )
...
class MoisturePrediction(BaseModel):
    ...
    id: UUID = Field(
        default_factory=uuid4, description="Unique prediction ID | معرف التنبؤ الفريد"
    )
    ...
    prediction_horizon_days: int = Field(
        ...
        description="Number of days predicted | عدد أيام التنبؤ"
    )

    # Error metrics
    error_margin: float = Field(
        default=0.03, ge=0.0, le=1.0,
        description="Expected error margin (~3%) | هامش الخطأ المتوقع"
    )

    confidence_intervals: list[tuple[float, float]] = Field(
        default_factory=list,
        description="Confidence intervals for each day | فترات الثقة لكل يوم"
    )

    # Input context
    initial_moisture: float = Field(
        default=0.0, ge=0, le=100,
        description="Initial soil moisture (%) | الرطوبة الأولية"
    )

    weather_factors: dict[str, Any] = Field(
        default_factory=dict,
        description="Weather factors used | عوامل الطقس المستخدمة"
    )

    soil_type: str = Field(
        default="", description="Soil type | نوع التربة"
    )

    crop_type: str = Field(
        default="", description="Crop type | نوع المحصول"
    )

    # Model info
    model_name: str = Field(
        default="moisture_lstm", description="Model used | النموذج المستخدم"
    )
    ...
    zone_id: str = Field(
        default="", description="Zone identifier | معرف المنطقة"
    )
...
class YieldEstimation(BaseModel):
    ...
    crop_type_ar: str = Field(
        default="", description="Crop type (Arabic) | نوع المحصول (عربي)"
    )

    growth_stage: str = Field(
        default="", description="Current growth stage | مرحلة النمو الحالية"
    )

    # Yield estimates
    estimated_yield_kg_ha: float = Field(
        ..., ge=0,
        description="Estimated yield in kg/hectare | الإنتاجية المقدرة كجم/هكتار"
    )

    yield_curve: list[float] = Field(
        default_factory=list,
        description="15-day yield curve | منحنى الإنتاجية لـ 15 يوم"
    )

    min_yield_kg_ha: float | None = Field(
        None, ge=0,
        description="Minimum yield estimate | الحد الأدنى للإنتاجية"
    )

    max_yield_kg_ha: float | None = Field(
        None, ge=0,
        description="Maximum yield estimate | الحد الأقصى للإنتاجية"
    )

    # Confidence
    confidence: float = Field(
        default=0.8, ge=0.0, le=1.0,
        description="Estimation confidence | ثقة التقدير"
    )

    # Factors
    contributing_factors: dict[str, float] = Field(
        default_factory=dict,
        description="Factors and their weights | العوامل وأوزانها"
    )

    risk_factors: list[str] = Field(
        default_factory=list,
        description="Identified risk factors | عوامل الخطر المحددة"
    )

    risk_factors_ar: list[str] = Field(
        default_factory=list,
        description="Risk factors (Arabic) | عوامل الخطر (عربي)"
    )

    # Input data
    field_area_ha: float = Field(
        default=0.0, ge=0,
        description="Field area in hectares | مساحة الحقل بالهكتار"
    )

    weather_forecast_used: bool = Field(
        default=True,
        description="Whether weather forecast was used | هل تم استخدام توقعات الطقس"
    )

    # Model info
    model_name: str = Field(
        default="yield_prediction", description="Model used | النموذج المستخدم"
    )
    ...
    forecast_horizon_days: int = Field(
        default=15, ge=1, le=90,
        description="Forecast horizon in days | أفق التنبؤ بالأيام"
    )
...
class SystemMetrics(BaseModel):
    ...
    edge_latency_ms: float = Field(
        default=0.0, ge=0,
        description="Average edge layer latency (ms) | متوسط زمن استجابة الحافة"
    )
    ...
    total_latency_ms: float = Field(
        default=0.0, ge=0,
        description="Total system latency (ms) | إجمالي زمن استجابة النظام"
    )

    # Accuracy metrics
    edge_accuracy: float = Field(
        default=0.0, ge=0.0, le=1.0,
        description="Edge layer accuracy | دقة طبقة الحافة"
    )

    cloud_accuracy: float = Field(
        default=0.0, ge=0.0, le=1.0,
        description="Cloud layer accuracy | دقة طبقة السحابة"
    )
    ...
    uptime_percent: float = Field(
        ...
        description="System uptime percentage | نسبة وقت التشغيل"
    )

    edge_uptime_percent: float = Field(
        default=0.0, ge=0.0, le=100.0,
        description="Edge layer uptime | وقت تشغيل طبقة الحافة"
    )

    cloud_uptime_percent: float = Field(
        default=0.0, ge=0.0, le=100.0,
        description="Cloud layer uptime | وقت تشغيل طبقة السحابة"
    )

    # Throughput
    readings_per_minute: float = Field(
        default=0.0, ge=0,
        description="Sensor readings per minute | القراءات في الدقيقة"
    )

    decisions_per_minute: float = Field(
        default=0.0, ge=0,
        description="Decisions per minute | القرارات في الدقيقة"
    )

    inferences_per_minute: float = Field(
        default=0.0, ge=0,
        description="Cloud inferences per minute | الاستدلالات في الدقيقة"
    )

    # Device metrics
    total_devices: int = Field(
        default=0, ge=0,
        description="Total registered devices | إجمالي الأجهزة المسجلة"
    )

    active_devices: int = Field(
        default=0, ge=0,
        description="Currently active devices | الأجهزة النشطة حالياً"
    )

    offline_devices: int = Field(
        default=0, ge=0,
        description="Offline devices | الأجهزة غير المتصلة"
    )

    # Sync metrics
    sync_success_rate: float = Field(
        default=0.0, ge=0.0, le=1.0,
        description="Edge-cloud sync success rate | معدل نجاح المزامنة"
    )

    pending_sync_count: int = Field(
        default=0, ge=0,
        description="Pending items to sync | العناصر المعلقة للمزامنة"
    )
    ...
    measurement_period_seconds: int = Field(
        default=60, ge=1,
        description="Measurement period in seconds | فترة القياس بالثواني"
    )
...
class RuleCondition(BaseModel):
    ...
    threshold: float = Field(
        ..., description="Threshold value | قيمة العتبة"
    )
...
class RuleAction(BaseModel):
    ...
    action_type_ar: str = Field(
        default="", description="Action type (Arabic) | نوع الإجراء (عربي)"
    )

    parameters: dict[str, Any] = Field(
        default_factory=dict,
        description="Action parameters | معلمات الإجراء"
    )

    delay_seconds: int = Field(
        default=0, ge=0,
        description="Delay before execution | التأخير قبل التنفيذ"
    )
...
class IFTTTRule(BaseModel):
    ...
    id: UUID = Field(
        default_factory=uuid4, description="Unique rule ID | معرف القاعدة الفريد"
    )

    name: str = Field(
        ..., min_length=1, description="Rule name | اسم القاعدة"
    )

    name_ar: str = Field(
        default="", description="Rule name (Arabic) | اسم القاعدة (عربي)"
    )

    description: str = Field(
        default="", description="Rule description | وصف القاعدة"
    )
    ...
    conditions: list[RuleCondition] = Field(
        ..., min_length=1,
        description="Conditions that must all be true | الشروط التي يجب أن تتحقق"
    )

    condition_logic: str = Field(
        default="AND",
        description="Logic between conditions (AND/OR) | المنطق بين الشروط"
    )

    actions: list[RuleAction] = Field(
        ..., min_length=1,
        description="Actions to execute | الإجراءات المراد تنفيذها"
    )

    # Execution settings
    is_active: bool = Field(
        default=True, description="Whether rule is active | هل القاعدة نشطة"
    )

    priority: int = Field(
        default=5, ge=1, le=10,
        description="Rule priority (1=highest) | أولوية القاعدة"
    )
    ...
    max_daily_triggers: int = Field(
        default=10, ge=1,
        description="Maximum triggers per day | الحد الأقصى للتفعيلات اليومية"
    )

    # Time constraints
    active_hours_start: int | None = Field(
        None, ge=0, le=23,
        description="Start hour for rule (0-23) | ساعة بدء القاعدة"
    )

    active_hours_end: int | None = Field(
        None, ge=0, le=23,
        description="End hour for rule (0-23) | ساعة نهاية القاعدة"
    )
    ...
    trigger_count: int = Field(
        default=0, ge=0,
        description="Total trigger count | إجمالي عدد التفعيلات"
    )
