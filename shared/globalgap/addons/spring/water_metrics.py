...
class WaterUsageMetric(BaseModel):
    ...
    @classmethod
    def calculate_flow_rate(cls, v: float | None, info: ValidationInfo) -> float | None:
        """Calculate flow rate if not provided / حساب معدل التدفق إذا لم يتم توفيره"""
        if v is None and info.data.get("volume_cubic_meters") and info.data.get("duration_hours"):
            if info.data["duration_hours"] > 0:
...
class IrrigationEfficiency(BaseModel):
    ...
    @classmethod
    def calculate_efficiency(cls, v: float | None, info: ValidationInfo) -> float | None:
        """Calculate efficiency if not provided / حساب الكفاءة إذا لم يتم توفيرها"""
        if (
            (v is None or v == 0)
            and info.data.get("water_applied_m3")
            and info.data.get("water_stored_in_root_zone_m3")
            and info.data["water_applied_m3"] > 0
        ):
...
class RainfallHarvesting(BaseModel):
    ...
    @field_validator("collected_volume_m3", mode="before")
    @classmethod
    def estimate_collected_volume(cls, v: float | None, info: ValidationInfo) -> float | None:
        """Estimate collected volume if not provided / تقدير الحجم المجمع إذا لم يتم توفيره"""
        if v is None or v == 0:
            if info.data.get("rainfall_mm") and info.data.get("collection_area_m2"):
...
class WaterEfficiencyScore(BaseModel):
    ...
    @classmethod
    def calculate_water_per_hectare(cls, v: float | None, info: ValidationInfo) -> float | None:
        """Calculate water use per hectare / حساب استخدام المياه لكل هكتار"""
        if v is None or v == 0:
            if info.data.get("total_water_used_m3") and info.data.get("total_irrigated_area_ha"):
                if info.data["total_irrigated_area_ha"] > 0:
                    return info.data["total_water_used_m3"] / info.data["total_irrigated_area_ha"]
        return v

    @field_validator("rainwater_percentage", mode="before")
    @classmethod
    def calculate_rainwater_percentage(cls, v: float | None, info: ValidationInfo) -> float | None:
