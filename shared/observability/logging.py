...
class SensitiveDataMasker:
    ...
    PATTERNS = {
        ...
        'access_token': re.compile(r'(["\']?(?:access[-_]?token|token)["\']?\s*[:=]\s*["\']?)([a-zA-Z0-9_\-\.]{20,})(["\']?)', re.IGNORECASE),
        ...
        # Google Cloud Keys
        'gcp_key': re.compile(r'(["\']?private[-_]?key["\']?\s*[:=]\s*["\']?)(-----BEGIN PRIVATE KEY-----[^-]+-----END PRIVATE KEY-----)(["\']?)', re.IGNORECASE),
    ...
    def mask_dict(cls, data: Dict[str, Any], deep: bool = True) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if any(sensitive in key.lower() for sensitive in cls.SENSITIVE_FIELDS):
                ...
                result[key] = [
                    cls.mask_dict(item, deep=True) if isinstance(item, dict)
