...
class DatabaseOptimizer:
    ...
    async def analyze_query(self, sql: str, *params) -> dict[str, Any]:
        ...
        async with self.pool.acquire() as conn:
            ...
            # nosemgrep: asyncpg-sqli -- input validated: only SELECT queries allowed (checked above)
...
async def batch_insert(
    ...
    for batch_idx in range(num_batches):
        ...
        async with pool.acquire() as conn:
            result = await conn.execute(sql, *params)  # nosemgrep: asyncpg-sqli,sqlalchemy-execute-raw-query,formatted-sql-query -- sql built from validated whitelist columns; values parameterized
