...
const [buyerWalletRows, sellerWalletRows] = await Promise.all([
  tx.$queryRaw<any[]>`SELECT * FROM wallets WHERE id = ${escrow.buyerWalletId}::uuid FOR UPDATE`,
  tx.$queryRaw<any[]>`SELECT * FROM wallets WHERE id = ${escrow.sellerWalletId}::uuid FOR UPDATE`
]);
```
