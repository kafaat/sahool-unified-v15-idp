...
async def test_complete_payment_workflow(
    ...
    assert subscription_response.status_code in (201, 200, 401, 422), \
    ...
    assert payment_intent_response.status_code in (200, 201, 401, 422), \
    ...
    if payment_intent_response.status_code in (200, 201):
        ...
        payment_status_response = await workflow_client.get(
            f"http://localhost:8089/api/v1/payments/{payment_id}",
            headers=e2e_headers
        )

        assert payment_status_response.status_code in (200, 401, 404), \
    ...
    invoices_response = await workflow_client.get(
        "http://localhost:8089/api/v1/invoices",
        headers=e2e_headers
    )

    assert invoices_response.status_code in (200, 401), \
        "Invoices should be accessible"
    ...
    if invoices_response.status_code == 200:
        ...
        if isinstance(invoices, list) and len(invoices) > 0:
            invoice = invoices[0]
            assert "id" in invoice or "invoice_id" in invoice, \
...
async def test_tharwatt_payment_integration(
    ...
    assert payment_response.status_code in (200, 201, 401, 422, 503), \
    ...
    if invoices_response.status_code == 200:
        ...
        if isinstance(invoices, list) and len(invoices) > 0:
            ...
            if invoice_id:
                ...
                assert invoice_detail_response.status_code in (200, 401, 404), \
...
async def test_payment_provider_configuration(
    ...
    providers_response = await workflow_client.get(
        "http://localhost:8104/api/v1/providers",
        headers=e2e_headers
    )

    assert providers_response.status_code in (200, 401, 404), \
...
async def test_billing_field_ops_integration(
    ...
    assert field_response.status_code in (201, 200, 401, 403, 422), \
...
async def test_payment_webhook_handling(
    ...
    assert webhook_response.status_code in (200, 400, 401, 404), \
