...
============================
اختبارات سلامة البيانات عبر المنصة
...
for p in (str(ROOT), str(ROOT / "apps" / "services")):
...
def _parse_ts_object(filepath: Path, object_name: str) -> dict[str, Any]:
    ...
    block = match.group(1)
    result: dict[str, Any] = {}
    # Match lines like:  KEY: 1234, or KEY: 'string',
    for m in re.finditer(
        ...
        if m.group(2) is not None:
            ...
            # template literal — resolve ${API_PREFIX}
            val = m.group(5).strip("`")
...
def _read_requirements(service_dir: Path) -> dict[str, str]:
    ...
    for line in req_file.read_text(encoding="utf-8").splitlines():
        ...
        if not line or line.startswith("#") or line.startswith("-"):
            continue
        # Parse name==version or name>=version,<version etc.
...
class TestPydanticModelValidation:
    ...
    def test_equipment_create_valid(self):
        ...
        from apps.services.equipment_service_models import EquipmentCreate, EquipmentType
    ...
    def test_equipment_type_enum_values(self):
        """EquipmentType enum has expected agricultural values."""
        from apps.services.equipment_service_models import EquipmentType

        expected = {"tractor", "pump", "drone", "harvester", "sprayer", "pivot", "sensor", "vehicle", "other"}
        assert {e.value for e in EquipmentType} == expected

    def test_equipment_status_enum_values(self):
        """EquipmentStatus enum covers operational states."""
        from apps.services.equipment_service_models import EquipmentStatus

        expected = {"operational", "maintenance", "inactive", "repair"}
        assert {e.value for e in EquipmentStatus} == expected

    def test_alert_create_valid(self):
        """AlertCreate accepts valid data with all required fields."""
        from apps.services.alert_service_models import AlertCreate, AlertType, AlertSeverity
    ...
    def test_alert_type_enum_completeness(self):
        ...
        from apps.services.alert_service_models import AlertType
        ...
        assert {e.value for e in AlertType} == expected

    def test_alert_severity_ordering(self):
        """AlertSeverity includes all levels from critical to info."""
        from apps.services.alert_service_models import AlertSeverity

        expected = {"critical", "high", "medium", "low", "info"}
        assert {e.value for e in AlertSeverity} == expected

    def test_alert_status_transitions(self):
        """AlertStatus covers the full lifecycle."""
        from apps.services.alert_service_models import AlertStatus

        expected = {"active", "acknowledged", "dismissed", "resolved", "expired"}
        assert {e.value for e in AlertStatus} == expected

    def test_billing_subscription_valid(self):
        """Billing Subscription model accepts valid data."""
        from apps.services.billing_core_models import Subscription
        ...
        assert obj.tenant_id == "tenant-001"
        assert obj.plan_id == "plan-starter"

    def test_billing_invoice_required_fields(self):
        """Invoice model requires invoice_id, tenant_id, and line_items."""
        from apps.services.billing_core_models import Invoice, InvoiceLineItem
        ...
        assert obj.invoice_number.startswith("SAH-")

    def test_billing_payment_required_fields(self):
        """Payment model requires payment_id, invoice_id, tenant_id."""
        from apps.services.billing_core_models import Payment
...
class TestServicePortUniqueness:
    ...
    def test_all_service_ports_are_unique(self):
        ...
        # Collect only numeric ports (skip infrastructure like NATS, REDIS)
    ...
    def test_service_port_count_minimum(self):
        ...
        ports = _parse_ts_object(ports_file, "SERVICE_PORTS")
        port_values = [v for v in ports.values() if isinstance(v, int)]
        assert len(port_values) >= 40, (
            f"Expected >=40 service ports, found {len(port_values)}"
        )

    def test_service_registry_ports_match(self):
        """SERVICE_REGISTRY ports must match SERVICE_PORTS values."""
        ports_file = CONTRACTS_DIR / "service-ports.ts"
        text = ports_file.read_text(encoding="utf-8")
        # Every registry entry references SERVICE_PORTS.<KEY>
        # Verify the port field matches in each registry entry
        ports = _parse_ts_object(ports_file, "SERVICE_PORTS")
        registry_ports = re.findall(
            r"port:\s*SERVICE_PORTS\.(\w+)", text
        )
        for key in registry_ports:
            assert key in ports, (
                f"SERVICE_REGISTRY references SERVICE_PORTS.{key} which does not exist"
            )
...
class TestErrorCodeUniqueness:
    ...
    def test_error_code_count_minimum(self):
        ...
        codes = _parse_ts_error_codes(codes_file)
        assert len(codes) >= 30, (
            f"Expected >=30 error codes, found {len(codes)}"
        )
    ...
    def test_vision_error_codes_follow_pattern(self):
        ...
        for key, value in vision_codes.items():
            assert re.match(r"^E\d{4}$", value), (
                f"Vision code {key}={value} does not follow Exxxx pattern"
            )
    ...
    def test_weather_error_codes_follow_pattern(self):
        ...
        for key, value in weather_codes.items():
            assert re.match(r"^W\d{4}$", value), (
                f"Weather code {key}={value} does not follow Wxxxx pattern"
            )
...
class TestAPIEndpointConsistency:
    ...
    def test_all_endpoints_follow_api_v1_pattern(self):
        ...
        assert violations == [], (
            f"Endpoints not following /api/v1/ pattern:\n"
            + "\n".join(violations)
        )

    def test_no_duplicate_endpoint_paths(self):
        """No two endpoints should resolve to the exact same path."""
        endpoints_file = CONTRACTS_DIR / "api-endpoints.ts"
        groups = _parse_ts_all_endpoint_groups(endpoints_file)
        all_paths: list[str] = []
        ...
        # Only check static paths (no {param} placeholders — those can overlap)
        static_paths = [p for p in all_paths if "{" not in p]
        duplicates = {p: c for p, c in Counter(static_paths).items() if c > 1}
        # Minor: some endpoints intentionally share paths (e.g., LIST and CREATE both use /api/v1/tasks)
        # Allow up to 2 occurrences for RESTful patterns (GET list + POST create)
        severe_duplicates = {p: c for p, c in duplicates.items() if c > 2}
        assert severe_duplicates == {}, (
            f"Endpoint paths with >2 occurrences: {severe_duplicates}"
        )
    ...
    def test_endpoint_group_count(self):
        ...
        assert len(groups) >= 15, (
            f"Expected >=15 endpoint groups, found {len(groups)}"
        )
...
class TestTenantIdCoverage:
    ...
    @pytest.mark.parametrize(
        "service_name,model_file_pattern",
        [
    ...
    def test_service_has_tenant_id_field(self, service_name: str, model_file_pattern: str):
        """Service models must include tenant_id for multi-tenancy."""
        model_path = SERVICES_DIR / service_name / model_file_pattern
        assert model_path.exists(), f"Missing {model_path}"
        content = model_path.read_text(encoding="utf-8")
        assert "tenant_id" in content, (
            f"{service_name} missing tenant_id in {model_file_pattern}"
        )

    def test_equipment_model_tenant_id_is_required(self):
        """Equipment model tenant_id must be a required str (not Optional)."""
        model_path = SERVICES_DIR / "equipment-service" / "src" / "main.py"
        content = model_path.read_text(encoding="utf-8")
        # The Equipment model should have tenant_id: str (not str | None)
        # Find the Equipment class definition
        ...
        assert "tenant_id: str" in equip_block, (
            "Equipment.tenant_id should be a required str"
        )

    def test_billing_subscription_tenant_id_is_required(self):
        """Subscription model tenant_id must be a required str."""
        model_path = SERVICES_DIR / "billing-core" / "src" / "main.py"
        content = model_path.read_text(encoding="utf-8")
        sub_match = re.search(
        ...
        assert "tenant_id: str" in sub_block, (
            "Subscription.tenant_id should be a required str"
        )
...
class TestConfigurationConsistency:
    ...
    def test_jwt_algorithm_consistent(self):
        ...
        for svc in services:
            ...
            # Check for explicit JWT_ALGORITHM assignments
            for m in re.finditer(r'JWT_ALGORITHM\s*[=:]\s*["\'](\w+)["\']', content):
                if m.group(1) != "HS256":
                    non_hs256.append(f"{svc.name}: {m.group(1)}")
        assert non_hs256 == [], (
            f"Services using non-HS256 JWT_ALGORITHM: {non_hs256}"
        )
    ...
    def test_nats_subjects_use_sahool_prefix(self):
        ...
        assert violations == [], (
            f"NATS subjects not starting with 'sahool.':\n" + "\n".join(violations)
        )
        ...
        for svc in services:
            ...
            for m in re.finditer(r'version\s*=\s*["\']([^"\']+)["\']', content):
                ...
                if ver != "16.0.0":
                    ...
                    break
        # Allow some services to be behind; but most should be 16.0.0
        if wrong_version:
            pytest.skip(
                f"Some services have non-16.0.0 version (informational): {wrong_version}"
            )
...
class TestNATSSubjectConventions:
    ...
    def test_subject_count_minimum(self):
        ...
        content = subjects_file.read_text(encoding="utf-8")
        subjects = re.findall(
            r'^SAHOOL_\w+\s*=\s*"sahool\.[^"]*"',
            content,
            re.MULTILINE,
        )
        assert len(subjects) >= 50, (
            f"Expected >=50 NATS subjects, found {len(subjects)}"
        )
    ...
    def test_no_invalid_characters_in_subjects(self):
        ...
        for m in re.finditer(
            ...
            name, value = m.group(1), m.group(2)
            # NATS subjects: allow alphanumeric, dots, underscores, hyphens,
            # and wildcards (* and >)
            if re.search(r"[^a-zA-Z0-9._\-*>]", value):
                violations.append(f"{name} = {value}")
        assert violations == [], (
            f"NATS subjects with invalid characters:\n" + "\n".join(violations)
        )
    ...
    def test_wildcard_subjects_end_correctly(self):
        ...
        for m in re.finditer(
            ...
            if not (value.endswith(".*") or value.endswith(".>")):
                violations.append(f"{name} = {value}")
        assert violations == [], (
            f"Wildcard subjects not ending with .* or .>:\n" + "\n".join(violations)
        )
    ...
    def test_get_tenant_subject_function_exists(self):
        ...
        subjects_file = SHARED_DIR / "events" / "subjects.py"
        content = subjects_file.read_text(encoding="utf-8")
        assert "def get_tenant_subject" in content, (
            "Missing get_tenant_subject function in subjects.py"
        )
    ...
    def test_helm_ports_match_contracts(self):
        ...
        for helm_svc, contract_key in self.HELM_SERVICE_MAP.items():
            ...
            if contract_port is not None and helm_port != contract_port:
                mismatches.append(
                    f"{helm_svc}: helm={helm_port}, contract={contract_port}"
                )

        assert mismatches == [], (
            f"Port mismatches between Helm and contracts:\n"
            + "\n".join(mismatches)
        )
...
class TestRequirementsConsistency:
    ...
    CRITICAL_PACKAGES = ["fastapi", "pydantic", "uvicorn"]

    def test_fastapi_version_consistent(self):
        """All services should use the same fastapi version."""
        services = _find_python_services()
        ...
        for svc in services:
            ...
            if "fastapi" in reqs:
                ver = reqs["fastapi"]
                versions.setdefault(ver, []).append(svc.name)

        assert len(versions) > 0, "No services with fastapi requirement found"
        if len(versions) > 1:
            detail = "; ".join(f"{v}: {svcs}" for v, svcs in versions.items())
            pytest.fail(f"Inconsistent fastapi versions: {detail}")

    def test_pydantic_version_consistent(self):
        """All services should use the same pydantic version."""
        services = _find_python_services()
        versions: dict[str, list[str]] = {}
        for svc in services:
            reqs = _read_requirements(svc)
        ...
        assert len(versions) > 0, "No services with pydantic requirement found"
        if len(versions) > 1:
            detail = "; ".join(f"{v}: {svcs}" for v, svcs in versions.items())
            pytest.fail(f"Inconsistent pydantic versions: {detail}")

    def test_nats_py_version_consistent(self):
        """Services using nats-py should have the same version."""
        services = _find_python_services()
        versions: dict[str, list[str]] = {}
        for svc in services:
            reqs = _read_requirements(svc)
        ...
        if len(versions) > 1:
            detail = "; ".join(f"{v}: {svcs}" for v, svcs in versions.items())
            pytest.fail(f"Inconsistent nats-py versions: {detail}")
...
class TestHealthEndpointConsistency:
    ...
    def test_all_services_have_healthz(self):
        """Every Python service's main.py must define a /healthz endpoint."""
        services = _find_python_services()
        ...
        for svc in services:
            ...
            if '"/healthz"' not in content and "'/healthz'" not in content:
                missing.append(svc.name)
        assert missing == [], (
            f"Services missing /healthz endpoint: {missing}"
        )

    def test_all_services_have_readyz(self):
        """Every Python service's main.py must define a /readyz endpoint."""
        services = _find_python_services()
        missing: list[str] = []
        for svc in services:
            main_py = svc / "src" / "main.py"
            ...
            if '"/readyz"' not in content and "'/readyz'" not in content:
                missing.append(svc.name)
        assert missing == [], (
            f"Services missing /readyz endpoint: {missing}"
        )

    def test_health_endpoints_return_status_ok(self):
        """Health endpoint implementations should return status 'ok'."""
        services = _find_python_services()
        no_status_ok: list[str] = []
        for svc in services[:10]:  # Sample first 10
            main_py = svc / "src" / "main.py"
            content = main_py.read_text(encoding="utf-8")
            if '"/healthz"' in content or "'/healthz'" in content:
                # Check that somewhere near healthz there's a status: ok
                if '"ok"' not in content and "'ok'" not in content:
                    no_status_ok.append(svc.name)
        assert no_status_ok == [], (
            f"Services with /healthz not returning 'ok' status: {no_status_ok}"
        )
