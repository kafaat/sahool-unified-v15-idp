...
export default function () {
    ...
    group('Weather Forecast', () => {
      ...
      if (result.success && result.data) {
        check(result.data, {
          ...
          'forecast has correct days': (d) => d.daily_forecast && d.daily_forecast.length <= days,
        });
      }
  ...
  if (Math.random() < 0.2) {
    group('Satellite Analysis', () => {
      ...
      if (!fieldId) {
        ...
        const createUrl = `${config.fieldServiceUrl}/fields`;
        const createResponse = authenticatedRequest('POST', createUrl, fieldData, authData.token);
        const result = validateResponse(createResponse, 201, 'create field for satellite');
      ...
      if (fieldId) {
        ...
        const response = authenticatedRequest('POST', url, analysisRequest, authData.token);
  ...
  if (Math.random() < 0.35 && fieldId) {
    group('Field Operations', () => {
      ...
      if (Math.random() < 0.5) {
        ...
        const createResponse = authenticatedRequest('POST', createUrl, operationData, authData.token);

        const result = validateResponse(createResponse, 201, 'create operation');
        ...
        if (result.success && result.data) {
          ...
          if (Math.random() < 0.3 && operationId) {
            ...
            authenticatedRequest('POST', completeUrl, { notes: 'Load test completion' }, authData.token);
            sleep(0.5);
          }
        }
      }
  ...
  if (Math.random() < 0.3) {
    group('Task Management', () => {
      ...
      if (Math.random() < 0.4 && fieldId) {
        ...
        const createResponse = authenticatedRequest('POST', createUrl, taskData, authData.token);
  ...
  if (Math.random() < 0.15) {
    group('Equipment Management', () => {
      ...
      if (Math.random() < 0.3) {
        const equipmentData = generateRandomEquipment(fieldId, authData.tenantId);
        const createUrl = `${config.equipmentUrl}/api/v1/equipment`;
        const createResponse = authenticatedRequest('POST', createUrl, equipmentData, authData.token);

        const result = validateResponse(createResponse, 201, 'create equipment');
        ...
        if (result.success && result.data) {
          ...
          if (Math.random() < 0.6 && equipmentId) {
            ...
            const telemetryData = {
              ...
              lon: 44.1910 + (Math.random() - 0.5) * 0.1,
            };
            authenticatedRequest('POST', telemetryUrl, telemetryData, authData.token);
            sleep(0.5);
          }
        }
      }
  ...
  if (Math.random() < 0.25 && fieldId) {
    group('Irrigation Calculation', () => {
      ...
      const response = authenticatedRequest('POST', url, irrigationRequest, authData.token);
  ...
  if (Math.random() < 0.1) {
    group('Billing Operations', () => {
      ...
      const quotaResponse = authenticatedRequest('GET', quotaUrl, null, authData.token);
