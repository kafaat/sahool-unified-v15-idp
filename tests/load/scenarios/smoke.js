...
export default function () {
  ...
  group('Authentication', () => {
    ...
    check(authData, {
      ...
      'token is present': (data) => data.token !== null && data.token !== undefined,
  ...
  group('List Fields', () => {
    ...
    if (result.success && result.data) {
      check(result.data, {
        'fields list has items array': (data) => Array.isArray(data.items) || Array.isArray(data.data),
      });
    }
  ...
  group('Create Field', () => {
    ...
    const response = authenticatedRequest('POST', url, fieldData, authData.token);
    ...
    if (result.success && result.data) {
      ...
      check(result.data, {
        'created field has id': (data) => data.id !== undefined || data.field_id !== undefined,
    ...
    group('Get Field by ID', () => {
      ...
      if (result.success && result.data) {
        check(result.data, {
          ...
          'field has geometry': (data) => data.geometry !== undefined || data.boundary !== undefined,
        });
      }
  ...
  if (fieldId) {
    group('Create Operation', () => {
      ...
      const response = authenticatedRequest('POST', url, operationData, authData.token);
      ...
      if (result.success && result.data) {
        ...
        check(result.data, {
          'operation has id': (data) => data.id !== undefined || data.operation_id !== undefined,
          'operation type matches': (data) => data.operation_type === 'irrigation',
        });
      }
  ...
  group('Weather Forecast', () => {
    ...
    if (result.success && result.data) {
      check(result.data, {
        ...
        'forecast has daily data': (data) => data.daily_forecast !== undefined && Array.isArray(data.daily_forecast),
      });
    }
  ...
  group('Billing Plans', () => {
    ...
    if (result.success && result.data) {
      check(result.data, {
        'plans list exists': (data) => data.plans !== undefined && Array.isArray(data.plans),
  ...
  if (operationId) {
    group('Delete Operation', () => {
      ...
      const response = authenticatedRequest('DELETE', url, null, authData.token);
