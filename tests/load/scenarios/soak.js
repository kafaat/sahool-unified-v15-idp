...
export default function (data) {
  ...
  if (elapsedMinutes > 5 && isBaselinePeriod) {
    ...
    if (!baselineCompleted) {
      ...
      console.log('✅ Baseline period completed. Starting long-term stability monitoring...');
    }
  }

  logIteration(iteration, `VU ${currentVU} - Soak test minute ${elapsedMinutes.toFixed(1)}`);

  // Authenticate
  const authData = authenticate();

  if (!authData || !authData.success) {
    console.error(`[VU ${currentVU}] Authentication failed at ${elapsedMinutes.toFixed(1)}m`);
    sleep(5);
    return;
  }
  ...
  group('Field Operations Cycle', () => {
    ...
    const listResponse = authenticatedRequest('GET', listUrl, null, authData.token);
    ...
    check(listResponse, {
      ...
      'field list performance stable': () => {
        ...
        const degradation = listDuration / (baselineResponseTimes.fieldList || listDuration);
    ...
    if (iteration % 3 === 0) {
      ...
      const createResponse = authenticatedRequest('POST', createUrl, fieldData, authData.token);
      ...
      if (fieldId) {
        ...
        const operationData = generateRandomOperation(fieldId, authData.tenantId);
  ...
  group('Weather Data Cycle', () => {
    ...
    const currentResponse = authenticatedRequest('GET', currentUrl, null, authData.token);
    ...
    if (iteration % 2 === 0) {
      ...
      const forecastResponse = authenticatedRequest('GET', forecastUrl, null, authData.token);
  ...
  group('Billing Stability', () => {
    ...
    const startTime = Date.now();
    const response = authenticatedRequest('GET', quotaUrl, null, authData.token);
  ...
  if (iteration % 5 === 0) {
    group('Task Lifecycle', () => {
      ...
      const statsResponse = authenticatedRequest('GET', statsUrl, null, authData.token);
  ...
  if (iteration % 8 === 0) {
    group('Equipment Telemetry Cycle', () => {
      ...
      const alertsResponse = authenticatedRequest('GET', alertsUrl, null, authData.token);
  ...
  if (iteration % 20 === 0) {
    group('Memory Leak Detection', () => {
      console.log(`[VU ${currentVU}] Memory check at ${elapsedMinutes.toFixed(1)}m (iteration ${iteration})`);
      ...
      for (let i = 0; i < 5; i++) {
        const fieldData = generateRandomField(authData.tenantId);
        const createUrl = `${config.fieldServiceUrl}/fields`;
        const response = authenticatedRequest('POST', createUrl, fieldData, authData.token);
        ...
        'cleanup successful': (r) => {
          if (r.status === 200) {
            try {
              ...
              return !items.some(item => createdFields.includes(item.id || item.field_id));
            } catch {
              return false;
            }
          }
  ...
  if (iteration % 30 === 0) {
    group('Long-running Operation', () => {
      console.log(`[VU ${currentVU}] Long-running operation test at ${elapsedMinutes.toFixed(1)}m`);
      ...
      if (fieldId) {
        ...
        const startTime = Date.now();
        const analysisResponse = authenticatedRequest('POST', analysisUrl, analysisRequest, authData.token);
        const analysisDuration = Date.now() - startTime;

        check(analysisResponse, {
          'satellite analysis completes': (r) => r.status === 200 || r.status === 202,
          'satellite analysis time reasonable': () => analysisDuration < 30000, // 30 seconds
        });

        if (analysisDuration > 10000) {
          longRunningOps.add(1);
          console.log(`[VU ${currentVU}] Long operation detected: ${analysisDuration}ms`);
        }
  ...
  if (iteration % 50 === 0) {
    ...
    console.log(`⏱️  Soak test progress: ${progress}% (${elapsedMinutes.toFixed(1)}/120 minutes)`);
  }
}
