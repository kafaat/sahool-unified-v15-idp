...
export default function (data) {
  ...
  if (!authData || !authData.success) {
    console.error(`[VU ${currentVU}] Authentication failed during ${phase} phase`);
    sleep(2);
    return;
  }
  ...
  if (Math.random() < 0.8) {
    group('Weather Forecast', () => {
      ...
      check(response, {
        'weather forecast accessible': (r) => r.status === 200 || r.status === 429 || r.status === 503,
  ...
  group('Quota Check', () => {
    ...
    check(response, {
      'quota check responsive': (r) => r.status === 200 || r.status === 429 || r.status === 503,
      'quota cached properly': (r) => {
        if (r.status === 200) {
          return r.headers['Cache-Control'] !== undefined || r.headers['X-Cache'] !== undefined;
        }
        return true;
      },
    });
  ...
  group('System Health', () => {
    ...
    if (!result && phase === 'spike') {
      console.warn(`[VU ${currentVU}] Health endpoint degraded during spike (${response.status})`);
    }
  });
  ...
  if (phase === 'spike' && Math.random() < 0.3) {
    group('Concurrent Request Burst', () => {
      ...
      check(responses, {
        ...
        'not all concurrent requests fail': () => successCount < 5 || responses.filter(r => r.status >= 500).length === 0,
      });
    });
  }
