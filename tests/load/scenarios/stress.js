...
export default function (data) {
  ...
  if (Math.random() < 0.7) {
    group('Aggressive Field Operations', () => {
      ...
      for (let i = 0; i < randomInt(1, 3); i++) {
        ...
        check(response, {
          'field creation handled': (r) => r.status === 201 || r.status === 429 || r.status === 503,
        });
      ...
      for (let i = 0; i < randomInt(3, 6); i++) {
        ...
        const response = authenticatedRequest('GET', listUrl, null, authData.token);

        check(response, {
          'list request handled': (r) => r.status === 200 || r.status === 429 || r.status === 503,
        });
  ...
  if (Math.random() < 0.5) {
    group('Heavy Weather Requests', () => {
      ...
      for (let i = 0; i < randomInt(2, 5); i++) {
        ...
        check(response, {
          'weather request handled': (r) => r.status === 200 || r.status === 429 || r.status === 503,
  ...
  if (Math.random() < 0.4) {
    group('Concurrent Operations', () => {
      ...
      const fieldResponse = authenticatedRequest('POST', fieldUrl, fieldData, authData.token);

      const fieldResult = validateResponse(fieldResponse, 201, 'field creation');
      ...
      if (fieldId) {
        ...
        for (let i = 0; i < randomInt(2, 5); i++) {
          ...
          const opResponse = authenticatedRequest('POST', opUrl, operationData, authData.token);

          check(opResponse, {
            'operation creation handled': (r) => r.status === 201 || r.status === 429 || r.status === 503,
          });
  ...
  if (Math.random() < 0.3) {
    group('Quota Check Storm', () => {
      ...
      for (let i = 0; i < randomInt(5, 10); i++) {
        ...
        check(response, {
          'quota check handled': (r) => r.status === 200 || r.status === 429 || r.status === 503,
        });
  ...
  if (Math.random() < 0.6) {
    group('Mixed Operations', () => {
      const operations = [
        // Read operations
        () => authenticatedRequest('GET', `${config.fieldServiceUrl}/fields?tenant_id=${authData.tenantId}`, null, authData.token),
        () => authenticatedRequest('GET', `${config.weatherUrl}/v1/forecast/sanaa`, null, authData.token),
        () => authenticatedRequest('GET', `${config.billingUrl}/v1/plans`, null, authData.token),
        // Write operations
        () => {
          const field = generateRandomField(authData.tenantId);
          return authenticatedRequest('POST', `${config.fieldServiceUrl}/fields`, field, authData.token);
        },
      ];
  ...
  if (Math.random() < 0.1) {
    group('Health Check Storm', () => {
      ...
      for (let i = 0; i < randomInt(10, 20); i++) {
        ...
        check(response, {
          'health endpoint responsive': (r) => r.status === 200 || r.status === 503,
  ...
  group('Error Monitoring', () => {
    ...
    const errorChecks = {
      ...
      'rate limiting working': (r) => r.status !== 429 || r.headers['Retry-After'] !== undefined,
    ...
    if (!result) {
      console.warn(`[VU ${currentVU}] Error detected: Status ${response.status}, Duration ${response.timings.duration}ms`);
    }
  });
