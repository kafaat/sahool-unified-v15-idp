...
export function fieldScenario() {
    ...
    group('Fields: CRUD Operations', () => {
        ...
        const listRes = http.get(`${BASE_URL}/api/fields?tenant_id=tenant_simulation&limit=20`, {
            headers: getHeaders(token),
            timeout: '30s',
        });
        ...
        const fieldData = {
            ...
            geometry: {
                ...
                coordinates: [[
                    ...
                    [44.20 + Math.random() * 0.1, 15.37 + Math.random() * 0.1],
                    [44.20 + Math.random() * 0.1, 15.38 + Math.random() * 0.1],
        ...
        if (fieldId) {
            ...
            const updateRes = http.patch(`${BASE_URL}/api/fields/${fieldId}`, JSON.stringify(updateData), {
                headers: getHeaders(token),
                timeout: '30s',
            });
...
export function iotScenario() {
    ...
    group('IoT: Sensor Data', () => {
        ...
        const iotRes = http.post(`${BASE_URL}/api/iot/readings`, JSON.stringify(sensorData), {
            headers: getHeaders(token),
            timeout: '30s',
        });
        ...
        const success = check(iotRes, {
            'iot data status OK': (r) => r.status === 201 || r.status === 200 || r.status === 401,
        });
        ...
        // Get sensor history
        http.get(`${BASE_URL}/api/iot/readings?device_id=${sensorData.device_id}&limit=10`, {
            headers: getHeaders(token),
            timeout: '30s',
        });
    });
    ...
    const summary = {
        ...
        scenarios: {
            ...
            fields: {
                ...
                create_duration: data.metrics.field_create_duration_ms?.values?.avg || 0,
            },
            weather: {
