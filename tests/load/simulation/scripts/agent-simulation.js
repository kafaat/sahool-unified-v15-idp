...
export default function (data) {
  ...
  group("Phase 4: Field Operations", () => {
    ...
    const listRes = http.get(
      `${BASE_URL}/api/fields?tenant_id=tenant_simulation&limit=10`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "X-Agent-ID": agentId,
...
export function handleSummary(data) {
  const summary = {
    ...
    metrics: {
      ...
      session_persistence:
        data.metrics.session_persistence_rate?.values?.rate || 0,
    },
    errors: {
...
function textSummary(data, opts) {
  ...
  const metrics = [
    {
      name: "Total Requests",
      value: data.metrics.http_reqs?.values?.count || 0,
    },
    {
      name: "Failed Rate",
      value:
    ...
    },
    {
      name: "Avg Duration",
      value:
        (data.metrics.http_req_duration?.values?.avg || 0).toFixed(2) + "ms",
    },
    {
      name: "P95 Duration",
      value:
  ...
  out +=
    indent +
    `    Connection Pool Errors: ${data.metrics.connection_pool_errors?.values?.count || 0}\n`;
  out +=
    indent +
    `    Session Loss Errors: ${data.metrics.session_loss_errors?.values?.count || 0}\n`;
  out +=
    indent +
    `    Race Condition Errors: ${data.metrics.race_condition_errors?.values?.count || 0}\n`;
  out +=
    indent +
    `    Timeout Errors: ${data.metrics.timeout_errors?.values?.count || 0}\n`;
