...
const CHAOS_CONFIGS = {
    ...
    high: {
        failureRate: 0.30,      // 30% failure injection
        maxRetries: 5,
        retryDelay: 300,
        stages: [
    ...
    extreme: {
        failureRate: 0.50,      // 50% failure injection
        maxRetries: 5,
        retryDelay: 200,
        stages: [
...
export default function () {
    ...
    group('Chaos: Service Failure Recovery', () => {
        ...
        check(result.response, {
            'service recovered or available': (r) => r && (r.status === 200 || r.status === 503),
        });
    });
    ...
    group('Chaos: Session Resilience', () => {
        ...
        const sessions = ['valid_token', 'invalid_token', '', null, 'expired_token'];
    ...
    group('Chaos: Concurrent Writes', () => {
        ...
        for (let i = 0; i < 3; i++) {
            requests.push(
                http.post(`${BASE_URL}/api/fields`, JSON.stringify({
                    tenant_id: 'tenant_chaos',
...
export function handleSummary(data) {
    const summary = {
        ...
        resilience_metrics: {
            ...
            failover_success_rate: data.metrics.failover_success_rate?.values?.rate || 0,
            graceful_degradation_rate: data.metrics.graceful_degradation_rate?.values?.rate || 0,
        },
        chaos_events: {
