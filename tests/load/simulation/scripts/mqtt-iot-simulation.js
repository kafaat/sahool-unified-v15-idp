...
// Device distribution
const SOIL_SENSOR_RATIO = 0.4; // 40% soil moisture sensors
const WEATHER_STATION_RATIO = 0.25; // 25% weather stations
const IRRIGATION_RATIO = 0.2; // 20% irrigation controllers
...
const SAUDI_FARM_LOCATIONS = [
  { name: "Al-Kharj Farm", lat: 24.15, lng: 47.3, region: "Riyadh" },
  { name: "Al-Ahsa Oasis", lat: 25.38, lng: 49.59, region: "Eastern" },
  {
    name: "Tabuk Agricultural Zone",
    lat: 28.3838,
    lng: 36.555,
    region: "Tabuk",
  },
  { name: "Wadi Al-Dawasir", lat: 20.491, lng: 44.78, region: "Riyadh" },
  { name: "Jizan Farm", lat: 16.8892, lng: 42.5511, region: "Jizan" },
  { name: "Al-Jouf Agricultural", lat: 29.785, lng: 40.1, region: "Al-Jouf" },
];
...
export function irrigationControllerFlow() {
  ...
  group("Irrigation: Publish Status", () => {
    const token = authenticateDevice(
      deviceId,
...
export function setup() {
  ...
  console.log(
    `  Soil Moisture Sensors (40%): ${Math.ceil(TOTAL_DEVICES * SOIL_SENSOR_RATIO)} devices`,
  );
  console.log(
    `  Weather Stations (25%):       ${Math.ceil(TOTAL_DEVICES * WEATHER_STATION_RATIO)} devices`,
  );
  console.log(
    `  Irrigation Controllers (20%): ${Math.ceil(TOTAL_DEVICES * IRRIGATION_RATIO)} devices`,
  );
  console.log(
    `  GPS Trackers (15%):           ${Math.ceil(TOTAL_DEVICES * GPS_TRACKER_RATIO)} devices`,
  );
  console.log("");
...
export function handleSummary(data) {
  const summary = {
    ...
    mqtt_metrics: {
      messages_published:
        data.metrics.mqtt_messages_published?.values?.count || 0,
      messages_received:
        data.metrics.mqtt_messages_received?.values?.count || 0,
      publish_success_rate:
        (
          (data.metrics.iot_publish_success_rate?.values?.rate || 0) * 100
        ).toFixed(2) + "%",
      subscribe_success_rate:
        (
          (data.metrics.iot_subscribe_success_rate?.values?.rate || 0) * 100
        ).toFixed(2) + "%",
    },
    device_success_rates: {
      soil_sensors:
        ((data.metrics.soil_sensor_success?.values?.rate || 0) * 100).toFixed(
          2,
        ) + "%",
      weather_stations:
        (
          (data.metrics.weather_station_success?.values?.rate || 0) * 100
        ).toFixed(2) + "%",
      irrigation:
        ((data.metrics.irrigation_success?.values?.rate || 0) * 100).toFixed(
          2,
        ) + "%",
      gps_trackers:
        ((data.metrics.gps_tracker_success?.values?.rate || 0) * 100).toFixed(
          2,
        ) + "%",
    },
    latency: {
      publish_p95:
        (data.metrics.mqtt_publish_latency_ms?.values?.["p(95)"] || 0).toFixed(
          0,
        ) + "ms",
      publish_avg:
        (data.metrics.mqtt_publish_latency_ms?.values?.avg || 0).toFixed(0) +
        "ms",
      auth_p95:
        (data.metrics.device_auth_latency_ms?.values?.["p(95)"] || 0).toFixed(
          0,
        ) + "ms",
    },
    errors: {
...
`;
  ...
  return {
    stdout: textSummary,
    "./results/mqtt-iot-simulation-summary.json": JSON.stringify(
      summary,
      null,
      2,
    ),
...
export default function () {
  ...
  if (rand < SOIL_SENSOR_RATIO) {
    ...
    rand <
    SOIL_SENSOR_RATIO + WEATHER_STATION_RATIO + IRRIGATION_RATIO
  ) {
