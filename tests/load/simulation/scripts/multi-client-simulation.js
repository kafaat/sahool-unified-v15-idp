...
const MOBILE_RATIO = 0.8; // 80% mobile traffic (iOS + Android) - الأعلى
...
const WEB_SUCCESS_TARGET = 0.9; // 90% success rate for web
...
export const options = {
  scenarios: {
    ...
    dashboard_admins: {
      executor: "constant-vus",
      vus: Math.ceil(DASHBOARD_VUS * 0.5),
      duration: TEST_DURATION,
    ...
    dashboard_analysts: {
      executor: "constant-vus",
      vus: Math.ceil(DASHBOARD_VUS * 0.3),
      duration: TEST_DURATION,
    ...
    dashboard_super_admins: {
      executor: "constant-vus",
      vus: Math.ceil(DASHBOARD_VUS * 0.2),
      duration: TEST_DURATION,
    ...
    iot_sensors: {
      ...
      rate: Math.max(1, Math.ceil(API_VUS * 0.5)) * 10,
      timeUnit: "1m",
      duration: TEST_DURATION,
    ...
    weather_stations: {
      executor: "constant-arrival-rate",
      rate: Math.max(1, Math.ceil(API_VUS * 0.3)) * 5,
      timeUnit: "1m",
      duration: TEST_DURATION,
    ...
    erp_sync: {
      executor: "constant-arrival-rate",
      rate: Math.max(1, Math.ceil(API_VUS * 0.2)) * 2,
      timeUnit: "1m",
      duration: TEST_DURATION,
...
export function iosFieldWorkerFlow() {
  ...
  group("iOS: Login", () => {
    ...
    const success = check(res, {
      "iOS login ok": (r) => r.status === 200 || r.status === 201,
    });
  ...
  for (let i = 0; i < 3; i++) {
    group("iOS: Field Operations", () => {
      ...
      const fieldsRes = http.get(
        `${BASE_URL}/api/fields?nearby=true&lat=${gps.latitude}&lng=${gps.longitude}`,
        {
          headers,
          tags: { name: "ios_fields" },
        },
      );
      recordRequest("mobile", Date.now() - fieldsStart);
      const fieldsSuccess = check(fieldsRes, {
        "fields ok": (r) => r.status === 200 || r.status === 404,
      });
...
export function androidFieldWorkerFlow() {
  ...
  group("Android: Login", () => {
    ...
    recordRequest("mobile", Date.now() - start);
    const success = check(res, {
      "Android login ok": (r) => r.status === 200 || r.status === 201,
    });
    recordSuccess("mobile", success);
    overallLoginSuccess.add(success ? 1 : 0);
  ...
  for (let i = 0; i < 3; i++) {
    group("Android: Field Operations", () => {
      ...
      recordSuccess(
        "mobile",
        check(weatherRes, {
          "weather ok": (r) => r.status === 200 || r.status === 404,
        }),
      );
      ...
      const tasksRes = http.get(
        `${BASE_URL}/api/workers/${__VU}/tasks?status=pending`,
        {
          headers,
          tags: { name: "android_tasks" },
        },
      );
      recordRequest("mobile", Date.now() - tasksStart);
      const tasksSuccess = check(tasksRes, {
        "tasks ok": (r) => r.status === 200 || r.status === 404,
      });
...
export function webManagerFlow() {
  ...
  group("Web Manager: Dashboard", () => {
    ...
    const res = http.get(`${BASE_URL}/api/dashboard/summary`, {
      headers,
      tags: { name: "web_dashboard" },
    });
    recordRequest("web", Date.now() - start);
    recordSuccess(
      "web",
      check(res, {
        "dashboard ok": (r) => r.status === 200 || r.status === 404,
      }),
    );
  });
  ...
  group("Web Manager: Fields", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/fields?page=1&limit=25`, {
      headers,
      tags: { name: "web_fields" },
    });
    recordRequest("web", Date.now() - start);
    const success = check(res, {
      "fields ok": (r) => r.status === 200 || r.status === 404,
    ...
    const detailRes = http.get(
      `${BASE_URL}/api/fields/${randomIntBetween(1, 25)}`,
      {
        headers,
        tags: { name: "web_field_detail" },
      },
    );
    ...
    recordSuccess(
      "web",
      check(detailRes, {
        "field detail ok": (r) => r.status === 200 || r.status === 404,
      }),
    );
  });
  ...
  group("Web Manager: Workers", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/workers?status=active`, {
      headers,
      tags: { name: "web_workers" },
    });
    recordRequest("web", Date.now() - start);
    recordSuccess(
      "web",
      check(res, { "workers ok": (r) => r.status === 200 || r.status === 404 }),
    );

    sleep(randomIntBetween(2, 4));

    const locStart = Date.now();
    const locRes = http.get(`${BASE_URL}/api/workers/locations`, {
      headers,
      tags: { name: "web_locations" },
    });
    recordRequest("web", Date.now() - locStart);
    recordSuccess(
      "web",
      check(locRes, {
        "locations ok": (r) => r.status === 200 || r.status === 404,
      }),
    );
  });
...
export function webAdminFlow() {
  ...
  group("Web Admin: Dashboard", () => {
    ...
    recordSuccess(
      "web",
      check(res, {
        "admin dash ok": (r) => r.status === 200 || r.status === 404,
      }),
    );
  });
  ...
  group("Web Admin: Users", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/admin/users?page=1&limit=50`, {
      headers,
      tags: { name: "admin_users" },
    });
    recordRequest("web", Date.now() - start);
    const success = check(res, {
      "users ok": (r) => r.status === 200 || r.status === 404,
    });
    recordSuccess("web", success);
    if (!success) recordError("web");
  });
  ...
  group("Web Admin: Audit", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/admin/audit-logs?page=1&limit=100`, {
      headers,
      tags: { name: "admin_audit" },
    });
    recordRequest("web", Date.now() - start);
    recordSuccess(
      "web",
      check(res, { "audit ok": (r) => r.status === 200 || r.status === 404 }),
    );
  });
...
export function webAnalystFlow() {
  ...
  group("Web Analyst: Analytics", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/analytics/overview?period=30d`, {
      headers,
      tags: { name: "analytics" },
    });
    recordRequest("web", Date.now() - start);
    recordSuccess(
      "web",
      check(res, {
        "analytics ok": (r) => r.status === 200 || r.status === 404,
      }),
    );
  });
...
export function webUserFlow() {
  ...
  group("Web User: Login", () => {
    ...
    recordRequest("web", Date.now() - start);
    const success = check(res, {
      "user login ok": (r) => r.status === 200 || r.status === 201,
    });
    recordSuccess("web", success);
    overallLoginSuccess.add(success ? 1 : 0);
  ...
  group("Web User: Browse", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/fields?page=1&limit=10`, {
      headers,
      tags: { name: "web_browse" },
    });
    recordRequest("web", Date.now() - start);
    recordSuccess(
      "web",
      check(res, { "browse ok": (r) => r.status === 200 || r.status === 404 }),
    );
  });
  ...
  group("Web User: Weather", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/weather/forecast`, {
      headers,
      tags: { name: "web_weather" },
    });
    recordRequest("web", Date.now() - start);
    recordSuccess(
      "web",
      check(res, { "weather ok": (r) => r.status === 200 || r.status === 404 }),
    );
  });
...
export function webViewerFlow() {
  ...
  group("Web Viewer: Dashboard", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/dashboard/public`, {
      headers,
...
export function dashboardAdminFlow() {
  ...
  group("Dashboard Admin: Overview", () => {
    ...
    recordSuccess(
      "dashboard",
      check(res, {
        "overview ok": (r) => r.status === 200 || r.status === 404,
      }),
    );
  });
  ...
  group("Dashboard Admin: Settings", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/admin/settings`, {
      headers,
    ...
    recordSuccess(
      "dashboard",
      check(res, {
        "settings ok": (r) => r.status === 200 || r.status === 404,
      }),
    );
  });
...
export function dashboardAnalystFlow() {
  ...
  group("Dashboard Analyst: Analytics", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/analytics/dashboard?period=30d`, {
      headers,
      tags: { name: "analyst_analytics" },
    });
    recordRequest("dashboard", Date.now() - start);
    recordSuccess(
      "dashboard",
...
export function dashboardSuperAdminFlow() {
  ...
  group("Dashboard SuperAdmin: Login", () => {
    ...
    recordRequest("dashboard", Date.now() - start);
    const success = check(res, {
      "super admin login ok": (r) => r.status === 200 || r.status === 201,
    });
    recordSuccess("dashboard", success);
    overallLoginSuccess.add(success ? 1 : 0);
  ...
  group("Dashboard SuperAdmin: System", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/admin/system/overview`, {
      headers,
      tags: { name: "system_overview" },
    });
    recordRequest("dashboard", Date.now() - start);
    recordSuccess(
      "dashboard",
      check(res, { "system ok": (r) => r.status === 200 || r.status === 404 }),
    );
  });
  ...
  group("Dashboard SuperAdmin: Security", () => {
    const start = Date.now();
    const res = http.get(`${BASE_URL}/api/admin/security/settings`, {
      headers,
      tags: { name: "security_settings" },
    });
    recordRequest("dashboard", Date.now() - start);
    const success = check(res, {
      "security ok": (r) => r.status === 200 || r.status === 404,
    });
    recordSuccess("dashboard", success);
    if (!success) recordError("dashboard");
  });
...
export function weatherStationFlow() {
  ...
  group("Weather: Station Data", () => {
    const start = Date.now();
    const res = http.post(
      `${BASE_URL}/api/weather/stations/data`,
      JSON.stringify({
...
export function erpSyncFlow() {
  ...
  group("ERP: Sync", () => {
    ...
    const fetchRes = http.get(
      `${BASE_URL}/api/integration/erp/pending?limit=100`,
      {
        headers,
        tags: { name: "erp_fetch" },
      },
    ...
    recordSuccess(
      "api",
      check(fetchRes, {
        "ERP fetch ok": (r) => r.status === 200 || r.status === 404,
      }),
    );
    ...
    const syncRes = http.post(
      `${BASE_URL}/api/integration/erp/sync`,
      JSON.stringify({
...
export function setup() {
  ...
  console.log(
    `  📱 Mobile (80%):     ${MOBILE_VUS} VUs  [Target: ${MOBILE_SUCCESS_TARGET * 100}% success]`,
  );
  console.log(
    `     ├─ 🍎 iOS:        ${Math.ceil(MOBILE_VUS * 0.45)} VUs (field workers)`,
  );
  console.log(
    `     └─ 🤖 Android:    ${Math.ceil(MOBILE_VUS * 0.55)} VUs (field workers)`,
  );
  console.log("");
  console.log(
    `  🌐 Web Browser (12%): ${WEB_VUS} VUs  [Target: ${WEB_SUCCESS_TARGET * 100}% success]`,
  );
  console.log(`     ├─ 👤 Users:      ${Math.ceil(WEB_VUS * 0.5)} VUs`);
  console.log(`     ├─ 👔 Managers:   ${Math.ceil(WEB_VUS * 0.3)} VUs`);
  console.log(`     └─ 👁️  Viewers:    ${Math.ceil(WEB_VUS * 0.2)} VUs`);
  console.log("");
  console.log(
    `  🖥️  Dashboard (5%):   ${DASHBOARD_VUS} VUs  [Target: ${DASHBOARD_SUCCESS_TARGET * 100}% success]`,
  );
  console.log(`     ├─ 👑 Admins:     ${Math.ceil(DASHBOARD_VUS * 0.5)} VUs`);
  console.log(`     ├─ 📊 Analysts:   ${Math.ceil(DASHBOARD_VUS * 0.3)} VUs`);
  console.log(`     └─ ⭐ SuperAdmins: ${Math.ceil(DASHBOARD_VUS * 0.2)} VUs`);
  console.log("");
  console.log(
    `  🔌 API (3%):         ${API_VUS} VUs  [Target: ${API_SUCCESS_TARGET * 100}% success]`,
  );
  console.log(`     ├─ 🌡️  IoT:        ${Math.ceil(API_VUS * 0.5)} VUs`);
  console.log(`     ├─ ☀️  Weather:    ${Math.ceil(API_VUS * 0.3)} VUs`);
  console.log(`     └─ 🏢 ERP:        ${Math.ceil(API_VUS * 0.2)} VUs`);
  console.log("");
  console.log(
    "  ────────────────────────────────────────────────────────────────────────────",
  );
  ...
  console.log(
    "  🚀 Maximum Thresholds: Mobile 95% | Web 90% | Dashboard 85% | API 99%",
  );
  console.log(
    "═══════════════════════════════════════════════════════════════════════════════",
  );
...
export function handleSummary(data) {
  ...
  const summary = {
    ...
    traffic_distribution: {
      mobile: {
        ...
        percentage:
          totalReqs > 0
            ? ((mobileReqs / totalReqs) * 100).toFixed(1) + "%"
            : "0%",
      },
      web: {
        requests: webReqs,
        percentage:
          totalReqs > 0 ? ((webReqs / totalReqs) * 100).toFixed(1) + "%" : "0%",
      },
      dashboard: {
        requests: dashboardReqs,
        percentage:
          totalReqs > 0
            ? ((dashboardReqs / totalReqs) * 100).toFixed(1) + "%"
            : "0%",
      },
      api: {
        requests: apiReqs,
        percentage:
          totalReqs > 0 ? ((apiReqs / totalReqs) * 100).toFixed(1) + "%" : "0%",
      },
    },
    success_rates: {
      overall:
        (data.metrics.overall_success_rate?.values?.rate * 100 || 0).toFixed(
          2,
        ) + "%",
      login:
        (data.metrics.overall_login_success?.values?.rate * 100 || 0).toFixed(
          2,
        ) + "%",
      mobile:
        (data.metrics.mobile_success_rate?.values?.rate * 100 || 0).toFixed(2) +
        "%",
      web:
        (data.metrics.web_success_rate?.values?.rate * 100 || 0).toFixed(2) +
        "%",
      dashboard:
        (data.metrics.dashboard_success_rate?.values?.rate * 100 || 0).toFixed(
          2,
        ) + "%",
      api:
        (
          data.metrics.api_integration_success_rate?.values?.rate * 100 || 0
        ).toFixed(2) + "%",
    },
    response_times: {
      mobile_p95:
        (data.metrics.response_time_mobile?.values?.["p(95)"] || 0).toFixed(0) +
        "ms",
      mobile_avg:
        (data.metrics.response_time_mobile?.values?.avg || 0).toFixed(0) + "ms",
      web_p95:
        (data.metrics.response_time_web?.values?.["p(95)"] || 0).toFixed(0) +
        "ms",
      web_avg:
        (data.metrics.response_time_web?.values?.avg || 0).toFixed(0) + "ms",
      dashboard_p95:
        (data.metrics.response_time_dashboard?.values?.["p(95)"] || 0).toFixed(
          0,
        ) + "ms",
      dashboard_avg:
        (data.metrics.response_time_dashboard?.values?.avg || 0).toFixed(0) +
        "ms",
      api_p95:
        (data.metrics.response_time_api?.values?.["p(95)"] || 0).toFixed(0) +
        "ms",
      api_avg:
        (data.metrics.response_time_api?.values?.avg || 0).toFixed(0) + "ms",
    },
    errors: {
  ...
  const dashboardBar = "█".repeat(
    Math.round((dashboardReqs / totalReqs || 0) * 25),
  );
...
`;
  ...
  return {
    stdout: textSummary,
    "./results/multi-client-max-performance-v3.json": JSON.stringify(
      summary,
      null,
      2,
    ),
