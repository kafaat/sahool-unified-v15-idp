...
const BROWSERS = [
    { name: 'Chrome', ua: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36', share: 0.65 },
    { name: 'Safari', ua: 'Mozilla/5.0 (Macintosh; Intel Mac OS X 14_2) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.2 Safari/605.1.15', share: 0.20 },
    { name: 'Firefox', ua: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:121.0) Gecko/20100101 Firefox/121.0', share: 0.10 },
    { name: 'Edge', ua: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 Edg/120.0.0.0', share: 0.05 },
];
...
const USER_ROLES = [
    { role: 'admin', weight: 0.15, pages: ['dashboard', 'users', 'settings', 'reports', 'audit'] },
    { role: 'manager', weight: 0.35, pages: ['dashboard', 'fields', 'workers', 'reports', 'weather'] },
    { role: 'analyst', weight: 0.25, pages: ['dashboard', 'reports', 'analytics', 'exports'] },
    { role: 'viewer', weight: 0.25, pages: ['dashboard', 'fields', 'weather'] },
];

const DASHBOARD_WIDGETS = [
    { id: 'active_fields', api: '/api/widgets/active-fields', priority: 'high' },
    { id: 'weather_overview', api: '/api/widgets/weather-overview', priority: 'high' },
    { id: 'iot_status', api: '/api/widgets/iot-status', priority: 'medium' },
    { id: 'worker_locations', api: '/api/widgets/worker-locations', priority: 'medium' },
    { id: 'alerts_summary', api: '/api/widgets/alerts-summary', priority: 'high' },
    { id: 'crop_health', api: '/api/widgets/crop-health', priority: 'low' },
    { id: 'water_usage', api: '/api/widgets/water-usage', priority: 'medium' },
    { id: 'recent_activities', api: '/api/widgets/recent-activities', priority: 'low' },
];
...
export function adminFlow() {
    ...
    group('Admin: Dashboard', () => {
        ...
        responses.forEach((res, i) => {
            const ok = check(res, {
                [`widget ${DASHBOARD_WIDGETS[i].id} loaded`]: (r) => r.status === 200 || r.status === 404,
            });
    ...
    group('Admin: User Management', () => {
        ...
        const usersRes = http.get(`${BASE_URL}/api/admin/users?page=1&limit=50`, {
            headers, tags: { name: 'list_users' }
        });
        webApiSuccess.add(check(usersRes, { 'users listed': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);

        simulateUserThinking();

        // View user details
        const userDetailRes = http.get(`${BASE_URL}/api/admin/users/${randomIntBetween(1, 100)}`, {
            headers, tags: { name: 'user_detail' }
        });
        webApiSuccess.add(check(userDetailRes, { 'user detail loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
    });
    ...
    group('Admin: Settings', () => {
        ...
        webApiSuccess.add(check(settingsRes, { 'settings loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);

        // Audit logs
        const auditRes = http.get(`${BASE_URL}/api/admin/audit-logs?page=1&limit=100`, {
            headers, tags: { name: 'audit_logs' }
        });
        webApiSuccess.add(check(auditRes, { 'audit logs loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
    });
...
export function managerFlow() {
    ...
    group('Manager: Dashboard', () => {
        ...
        const summaryRes = http.get(`${BASE_URL}/api/dashboard/summary`, { headers, tags: { name: 'dashboard_summary' } });
        webApiSuccess.add(check(summaryRes, { 'summary loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
    ...
    group('Manager: Field Operations', () => {
        ...
        const fieldsRes = http.get(`${BASE_URL}/api/fields?page=1&limit=25&sort=name`, {
            headers, tags: { name: 'fields_list' }
        });
        dataTableLoadTime.add(Date.now() - tableStart);
        webApiSuccess.add(check(fieldsRes, { 'fields listed': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
        ...
        const fieldDetailRes = http.get(`${BASE_URL}/api/fields/${fieldId}`, {
            headers, tags: { name: 'field_detail' }
        });
        webApiSuccess.add(check(fieldDetailRes, { 'field detail loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);

        // Get field sensors
        const sensorsRes = http.get(`${BASE_URL}/api/fields/${fieldId}/sensors`, {
            headers, tags: { name: 'field_sensors' }
        });
        webApiSuccess.add(check(sensorsRes, { 'sensors loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);

        // Get field history chart data
        const chartStart = Date.now();
        const historyRes = http.get(`${BASE_URL}/api/fields/${fieldId}/history?days=30`, {
            headers, tags: { name: 'field_history' }
        });
        chartRenderTime.add(Date.now() - chartStart);
        webApiSuccess.add(check(historyRes, { 'history loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
    });
    ...
    group('Manager: Workers', () => {
        const workersRes = http.get(`${BASE_URL}/api/workers?status=active`, {
            headers, tags: { name: 'workers_list' }
        });
        webApiSuccess.add(check(workersRes, { 'workers listed': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);

        // Worker locations map data
        const locationsRes = http.get(`${BASE_URL}/api/workers/locations`, {
            headers, tags: { name: 'worker_locations' }
        });
        webApiSuccess.add(check(locationsRes, { 'locations loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
    });
    ...
    group('Manager: Weather', () => {
        const weatherRes = http.get(`${BASE_URL}/api/weather/forecast?days=7`, {
            headers, tags: { name: 'weather_forecast' }
        });
        webApiSuccess.add(check(weatherRes, { 'weather loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
    });
...
export function analystFlow() {
    ...
    group('Analyst: Dashboard', () => {
        ...
        webApiSuccess.add(check(dashRes, { 'analytics loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
    });
    ...
    group('Analyst: Report Generation', () => {
        ...
        const genRes = http.post(`${BASE_URL}/api/reports/generate`, JSON.stringify({
        ...
        const success = check(genRes, {
            'report generation started': (r) => r.status === 200 || r.status === 201 || r.status === 202 || r.status === 404,
        });
        ...
        if (success && (genRes.status === 200 || genRes.status === 202)) {
            ...
            if (reportId) {
                ...
                for (let i = 0; i < 5; i++) {
                    sleep(2);
                    const statusRes = http.get(`${BASE_URL}/api/reports/${reportId}/status`, {
                        headers, tags: { name: 'report_status' }
                    });
                    try {
    ...
    group('Analyst: Data Export', () => {
        ...
        const exportRes = http.post(`${BASE_URL}/api/exports/create`, JSON.stringify({
        ...
        const success = check(exportRes, {
            'export created': (r) => r.status === 200 || r.status === 201 || r.status === 202 || r.status === 404,
        });
...
export function viewerFlow() {
    ...
    group('Viewer: Dashboard', () => {
        const dashStart = Date.now();
        const dashRes = http.get(`${BASE_URL}/api/dashboard/summary`, { headers, tags: { name: 'viewer_dashboard' } });
        dashboardLoadTime.add(Date.now() - dashStart);
        webPageLoadSuccess.add(check(dashRes, { 'dashboard loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
    });
    ...
    group('Viewer: Browse Fields', () => {
        const fieldsRes = http.get(`${BASE_URL}/api/fields?page=1&limit=20`, {
            headers, tags: { name: 'browse_fields' }
        });
        webApiSuccess.add(check(fieldsRes, { 'fields loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);

        simulateUserThinking();

        // Click on a field
        const fieldRes = http.get(`${BASE_URL}/api/fields/${randomIntBetween(1, 20)}`, {
            headers, tags: { name: 'view_field' }
        });
        webApiSuccess.add(check(fieldRes, { 'field viewed': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
    });
    ...
    group('Viewer: Weather', () => {
        const weatherRes = http.get(`${BASE_URL}/api/weather/current`, {
            headers, tags: { name: 'current_weather' }
        });
        webApiSuccess.add(check(weatherRes, { 'weather loaded': (r) => r.status === 200 || r.status === 404 }) ? 1 : 0);
    });
...
export function realtimePollingFlow() {
    ...
    group('Realtime: Polling', () => {
        ...
        endpoints.forEach(endpoint => {
            ...
            check(res, { 'poll successful': (r) => r.status === 200 || r.status === 404 });
        });
    });
}
...
export function handleSummary(data) {
    const summary = {
        ...
        success_rates: {
            login: (data.metrics.web_login_success?.values?.rate * 100 || 0).toFixed(2) + '%',
            page_load: (data.metrics.web_page_load_success?.values?.rate * 100 || 0).toFixed(2) + '%',
            api: (data.metrics.web_api_success?.values?.rate * 100 || 0).toFixed(2) + '%',
            report_generation: (data.metrics.web_report_generation_success?.values?.rate * 100 || 0).toFixed(2) + '%',
            data_export: (data.metrics.web_data_export_success?.values?.rate * 100 || 0).toFixed(2) + '%',
        },
        performance: {
            dashboard_load_p95: (data.metrics.web_dashboard_load_time?.values?.['p(95)'] || 0).toFixed(2) + 'ms',
            report_gen_p95: (data.metrics.web_report_generation_time?.values?.['p(95)'] || 0).toFixed(2) + 'ms',
            data_table_p95: (data.metrics.web_data_table_load_time?.values?.['p(95)'] || 0).toFixed(2) + 'ms',
            chart_render_p95: (data.metrics.web_chart_render_time?.values?.['p(95)'] || 0).toFixed(2) + 'ms',
        },
        activities: {
