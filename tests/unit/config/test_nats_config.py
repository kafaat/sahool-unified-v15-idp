...
class TestNatsSecurityConfig:
    ...
    def test_default_permissions_restricted(self, nats_config_content):
        ...
        default_perms_match = re.search(
            r'default_permissions\s*=\s*\{[\s\S]*?(?=users\s*=)',
            nats_config_content
        )
...
class TestNatsJetStreamConfig:
    ...
    def test_jetstream_store_dir_configured(self, nats_config_content):
        ...
        assert "store_dir:" in nats_config_content, \
            "JetStream store_dir should be configured"
...
class TestNatsServerConfig:
    ...
    def test_server_name_set(self, nats_config_content):
        ...
        assert "server_name:" in nats_config_content, \
            "Server name should be configured"
        assert "sahool" in nats_config_content.lower(), \
            "Server name should contain 'sahool'"

    def test_listen_port_configured(self, nats_config_content):
        """Test that listen port is configured."""
        assert "listen:" in nats_config_content, \
            "Listen address should be configured"
        # Check for standard NATS port
        assert "4222" in nats_config_content, \
            "Standard NATS port 4222 should be configured"
    ...
    def test_http_monitoring_port_configured(self, nats_config_content):
        ...
        assert "8222" in nats_config_content, \
            "Standard monitoring port 8222 should be configured"

    def test_max_connections_set(self, nats_config_content):
        """Test that max connections limit is set."""
        assert "max_connections:" in nats_config_content, \
            "max_connections should be configured"

    def test_max_payload_set(self, nats_config_content):
        """Test that max payload size is set."""
        assert "max_payload:" in nats_config_content, \
            "max_payload should be configured"
...
class TestNatsLoggingConfig:
    ...
    def test_logtime_enabled(self, nats_config_content):
        ...
        assert "logtime:" in nats_config_content, \
            "logtime should be configured"

    def test_log_file_configured(self, nats_config_content):
        """Test that log file is configured."""
        assert "log_file:" in nats_config_content, \
            "log_file should be configured"
