...
# ─────────────────────────────────────────────────────────────────────────────
# Health Check Tests - اختبارات فحص الصحة
# ─────────────────────────────────────────────────────────────────────────────


@pytest.mark.unit
class TestEventSubscriberHealthCheck:
    """Tests for EventSubscriber health_check functionality."""

    @pytest.mark.asyncio
    async def test_health_check_not_connected(self, subscriber):
        """Test health check when not connected returns unhealthy."""
        health = await subscriber.health_check()

        assert health["status"] == "unhealthy"
        assert health["nats_connected"] is False
        assert "Not connected to NATS" in health["details"].get("nats", "")

    @pytest.mark.asyncio
    async def test_health_check_connected_no_jetstream(self, subscriber):
        """Test health check when connected but JetStream disabled."""
        mock_nc = AsyncMock()
        subscriber._nc = mock_nc
        subscriber._connected = True
        subscriber._js = None
        subscriber.config.enable_jetstream = False

        health = await subscriber.health_check()

        assert health["status"] == "healthy"
        assert health["nats_connected"] is True
        assert health["jetstream_enabled"] is False

    @pytest.mark.asyncio
    async def test_health_check_jetstream_enabled(self, subscriber):
        """Test health check with JetStream enabled and healthy."""
        mock_nc = AsyncMock()
        mock_js = AsyncMock()

        # Mock JetStream account info
        mock_account_info = MagicMock()
        mock_account_info.streams = 5
        mock_account_info.consumers = 10
        mock_account_info.memory = 1024 * 1024
        mock_account_info.storage = 10 * 1024 * 1024
        mock_js.account_info = AsyncMock(return_value=mock_account_info)

        subscriber._nc = mock_nc
        subscriber._js = mock_js
        subscriber._connected = True
        subscriber.config.enable_jetstream = True
        subscriber.config.enable_dlq = False
        ...
        assert health["details"]["jetstream"]["status"] == "connected"
        assert health["details"]["jetstream"]["streams"] == 5
        assert health["details"]["jetstream"]["consumers"] == 10

    @pytest.mark.asyncio
    async def test_health_check_jetstream_error(self, subscriber):
        """Test health check when JetStream check fails."""
        mock_nc = AsyncMock()
        mock_js = AsyncMock()
        mock_js.account_info = AsyncMock(side_effect=Exception("JetStream error"))
        ...
        assert "JetStream check failed" in health["details"]["jetstream"]
    ...
    async def test_health_check_dlq_healthy(self, subscriber):
        ...
        mock_account_info.streams = 5
        mock_account_info.consumers = 10
        mock_account_info.memory = 1024
        mock_account_info.storage = 1024
        mock_js.account_info = AsyncMock(return_value=mock_account_info)

        # Mock DLQ stream info
        mock_stream_state = MagicMock()
        mock_stream_state.messages = 50
        mock_stream_state.bytes = 1024
        mock_stream_state.first_seq = 1
        mock_stream_state.last_seq = 50
        ...
        subscriber._js = mock_js
        subscriber._connected = True
        subscriber._dlq_initialized = True
        subscriber.config.enable_jetstream = True
        subscriber.config.enable_dlq = True
    ...
    async def test_health_check_dlq_warning(self, subscriber):
        ...
        # Mock DLQ stream with many messages (>1000)
        mock_stream_state = MagicMock()
        mock_stream_state.messages = 5000
        mock_stream_state.bytes = 1024 * 1024
        mock_stream_state.first_seq = 1
        mock_stream_state.last_seq = 5000
    ...
    async def test_health_check_dlq_not_initialized(self, subscriber):
        """Test health check when DLQ enabled but not initialized."""
        mock_nc = AsyncMock()
        mock_js = AsyncMock()
        ...
        subscriber._js = mock_js
        subscriber._connected = True
        subscriber._dlq_initialized = False
        subscriber.config.enable_jetstream = True
        subscriber.config.enable_dlq = True
        ...
        assert health["status"] == "degraded"
        assert "DLQ not initialized" in health["details"]["dlq"]

    @pytest.mark.asyncio
    async def test_health_check_includes_stats(self, subscriber):
        """Test health check includes error and dlq counts."""
        mock_nc = AsyncMock()
        subscriber._nc = mock_nc
        subscriber._connected = True
        subscriber._error_count = 5
        subscriber._dlq_count = 2
        subscriber.config.enable_jetstream = False
        subscriber.config.enable_dlq = False
